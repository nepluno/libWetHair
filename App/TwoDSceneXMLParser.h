...
class TwoDSceneXMLParser {
 public:
  void loadExecutableSimulation(
      ...
      libwethair::scalar& max_time, libwethair::scalar& steps_per_sec_cap,
      libwethair::renderingutils::Color& bgcolor, std::string& description,
      std::string& scenetag, bool& cam_inited, bool& view_inited);

  // TODO: NEED AN EIGEN_ALIGNED_THING_HERE ?
 private:
  template <int DIM>
  void loadParticleSimulation(GLFWwindow* window, bool simulate_comparison,
  ...
  void loadDERSimulation(GLFWwindow* window, bool simulate_comparison,
                         bool rendering_enabled, ExecutableSimulation** execsim,
                         libwethair::renderingutils::Viewport& view,
                         libwethair::renderingutils::Color& bgcolor,
                         rapidxml::xml_node<>* node,
  ...
  template <int DIM>
  void loadFluidSim(rapidxml::xml_node<>* node,
                    libwethair::TwoDScene<DIM>& twoscene);
  ...
  template <int DIM>
  void loadParticleEdges(rapidxml::xml_node<>* node,
                         libwethair::TwoDScene<DIM>& twodscene);

  void loadStrandParticleEdges(rapidxml::xml_node<>* node,
                               libwethair::TwoDScene<3>& scene);

  template <int DIM>
  void loadFlow(rapidxml::xml_node<>* node,
                libwethair::TwoDScene<DIM>& twodscene);
  ...
  void loadScripts(rapidxml::xml_node<>* node,
                   std::vector<libwethair::Script>& scripts);
  ...
  template <int DIM>
  void loadStrandForces(rapidxml::xml_node<>* node,
                        libwethair::TwoDScene<DIM>& scene);

  void loadMaxTime(rapidxml::xml_node<>* node, libwethair::scalar& max_t);

  void loadMaxSimFrequency(rapidxml::xml_node<>* node,
                           libwethair::scalar& max_freq);

  bool loadViewport(rapidxml::xml_node<>* node,
                    libwethair::renderingutils::Viewport& view);

  void loadBackgroundColor(rapidxml::xml_node<>* node,
                           libwethair::renderingutils::Color& color);

  void loadParticleColors(
      rapidxml::xml_node<>* node,
