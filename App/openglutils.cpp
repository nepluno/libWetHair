...
void draw_arrow3d(const Vector3s& start, const Vector3s& end,
                  const Vector3s& view_dir, scalar arrow_head_len) {
...
void write_trimesh3d(std::ostream& o, const std::vector<Vector3s>& vertices,
                     const std::vector<Vector3i>& tris, const Vector3s& center,
                     const Vector3s& scaling,
                     const Eigen::Quaternion<scalar>& rot, bool double_sided) {
  for (size_t i = 0; i < vertices.size(); ++i) {
    Eigen::Quaternion<scalar> rotp(0.0, vertices[i](0), vertices[i](1),
                                   vertices[i](2));
    Vector3s rot_x =
        (rot * rotp * rot.inverse()).vec().cwiseProduct(scaling) + center;
  ...
  if (double_sided) {
    for (size_t i = 0; i < vertices.size(); ++i) {
      Eigen::Quaternion<scalar> rotp(0.0, vertices[i](0), vertices[i](1),
                                     vertices[i](2));
      Vector3s rot_x =
          (rot * rotp * rot.inverse()).vec().cwiseProduct(scaling * 1.01) +
          center;
...
void draw_trimesh3d(const std::vector<Vector3s>& vertices,
  ...
  glRotated(rotp.angle() * 180.0 / M_PI, rotp.axis()(0), rotp.axis()(1),
            rotp.axis()(2));
...
void write_box3d(std::ostream& o, const Vector3s& dimensions,
                 const Vector3s& center, const Eigen::Quaternion<scalar>& rot,
                 bool double_sided) {
  ...
  write_trimesh3d(o, verts, tris, center,
                  Vector3s(width * 0.5, height * 0.5, depth * 0.5), rot,
                  double_sided);
}

void draw_box3d(const Vector3s& dimensions, const Vector3s& center,
                const Eigen::Quaternion<scalar>& rot) {
  glMatrixMode(GL_MODELVIEW);
  glPushMatrix();
  // glLoadIdentity();
