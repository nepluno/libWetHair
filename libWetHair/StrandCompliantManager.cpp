...
template <int DIM>
StrandCompliantManager<DIM>::StrandCompliantManager(
    TwoDScene<DIM>* scene, int max_newton, int max_iters, scalar criterion,
    bool compute_interhair, bool use_preconditioner)
    : m_max_num_newton(max_newton),
      m_max_num_iters(max_iters),
      m_criterion(criterion),
      m_scene(scene),
      m_compute_interhair(compute_interhair),
      m_use_preconditioner(use_preconditioner) {
...
template <int DIM>
void StrandCompliantManager<DIM>::localPreIterate(TwoDScene<DIM>& scene,
                                                  scalar dt) {
  int nhairs = m_integrators.size();

  thread_pool::ParallelFor(
      0, nhairs, [&](int hidx) { m_integrators[hidx]->preIterate(scene, dt); });
}

template <int DIM>
void StrandCompliantManager<DIM>::localStepScene(TwoDScene<DIM>& scene,
...
template <int DIM>
void StrandCompliantManager<DIM>::localPreconditionScene(TwoDScene<DIM>& scene,
                                                         scalar dt, VectorXs& r,
                                                         const VectorXs& b) {
  scalar t0 = timingutils::seconds();
  int nhairs = m_integrators.size();
...
template <int DIM>
void StrandCompliantManager<DIM>::localUpdateLambda(TwoDScene<DIM>& scene,
                                                    const VectorXs& dx,
                                                    const VectorXs& dv,
                                                    scalar dt) {
  int nhairs = m_integrators.size();
...
template <int DIM>
void StrandCompliantManager<DIM>::updateNextV(TwoDScene<DIM>& scene,
...
template <int DIM>
void StrandCompliantManager<DIM>::zeroFixedDoFs(const TwoDScene<DIM>& scene,
                                                VectorXs& vec) {
...
template <int DIM>
void StrandCompliantManager<DIM>::computeRHSIncrementalLocal(
    TwoDScene<DIM>& scene, scalar dt, VectorXs& b, const VectorXs& vplus) {
  m_gradU.setZero();
...
template <int DIM>
void StrandCompliantManager<DIM>::computeRHSIncrementalInterhair(
    TwoDScene<DIM>& scene, scalar dt, VectorXs& b, const VectorXs& vplus) {
  if (m_interhair_num(0) > 0) {
    mathutils::compute_cwiseProduct(m_c_buffer, m_invC_interhair,
                                    m_Phi_interhair);
    mathutils::accumulateJTPhi_coeff(b, -dt, m_c_buffer, m_J_interhair);
  }

  if (m_interhair_num(1) > 0) {
    mathutils::compute_cwiseProduct(m_cv_buffer, m_invCv_interhair,
                                    m_Phiv_interhair);
...
template <int DIM>
void StrandCompliantManager<DIM>::computeRHSLocal(TwoDScene<DIM>& scene,
...
template <int DIM>
void StrandCompliantManager<DIM>::computeAp(TwoDScene<DIM>& scene,
                                            const VectorXs& p, VectorXs& b,
                                            scalar dt) {
  scalar t0 = timingutils::seconds();
  ...
  int nhairs = m_integrators.size();
  thread_pool::ParallelFor(
      0, nhairs, [&](int hidx) { m_integrators[hidx]->computeAp(p, b); });

  if (m_interhair_num(2) > 0) {
    mathutils::computeJTPhi_coeff(m_c_buffer, m_invC_interhair, p,
                                  m_JT_interhair);
    mathutils::accumulateJTPhi_coeff(b, dt * dt, m_c_buffer, m_J_interhair);
  }

  if (m_interhair_num(3) > 0) {
    mathutils::computeJTPhi_coeff(m_cv_buffer, m_invCv_interhair, p,
                                  m_JvT_interhair);
    mathutils::accumulateJTPhi_coeff(b, dt, m_cv_buffer, m_Jv_interhair);
  }
...
template <int DIM>
void StrandCompliantManager<DIM>::localUpdateNumConstraints(const VectorXs& dx,
                                                            const VectorXs& dv,
                                                            scalar dt) {
  int num_pos, num_vel, num_J, num_Jv, num_Jxv, num_tildeK;
  m_scene->updateNumConstraintsLocal(num_pos, num_vel, num_J, num_Jv, num_Jxv,
                                     num_tildeK);
  ...
  m_scene->localPostPreprocess(m_lambda, m_lambda_v, m_J_nz, m_Jv_nz, m_Jxv_nz,
                               m_A_nz, m_invC_nz, m_invCv_nz, m_Phi, m_Phi_v,
                               dx, dv, dt);
}

template <int DIM>
void StrandCompliantManager<DIM>::interhairUpdateNumConstraints(
    const VectorXs& dx, const VectorXs& dv, scalar dt) {
  int num_pos, num_vel, num_J, num_Jv, num_Jxv, num_tildeK;
  m_scene->updateNumConstraintsInterHair(num_pos, num_vel, num_J, num_Jv,
                                         num_Jxv, num_tildeK, m_interhair_idx,
                                         m_interhair_num);
  ...
  thread_pool::ParallelFor(0, num_J_inter, [&](int i) {
    ...
    m_J_interhair_nz[i] =
        Triplets(t.row() - m_interhair_idx(0), t.col(), t.value());
  });
  m_J_interhair.resize(m_interhair_num(0), ndof);
  m_J_interhair.setFromTriplets(m_J_interhair_nz.begin(),
                                m_J_interhair_nz.end());
  ...
  thread_pool::ParallelFor(0, num_Jv_inter, [&](int i) {
    ...
    m_Jv_interhair_nz[i] =
        Triplets(t.row() - m_interhair_idx(1), t.col(), t.value());
  });
  m_Jv_interhair.resize(m_interhair_num(1), ndof);
  m_Jv_interhair.setFromTriplets(m_Jv_interhair_nz.begin(),
                                 m_Jv_interhair_nz.end());
...
template <int DIM>
bool StrandCompliantManager<DIM>::stepSceneNonlinear(TwoDScene<DIM>& scene,
                                                     scalar dt,
  ...
  for (; nkiters < m_max_num_newton; ++nkiters) {
    ...
    std::cout << "[Newton total iter: " << nkiters
              << ", res: " << newton_res_norm << "]" << std::endl;
#endif
...
#ifdef PCG_VERBOSE
      std::cout << "[PCG iter: " << iter << ", res: " << cg_res_norm << "]"
                << std::endl;
#endif
    }

    std::cout << "[PCG total iter: " << iter << ", res: " << cg_res_norm << "]"
              << std::endl;
  ...
  std::cout << "[Newton total iter: " << nkiters << ", res: " << newton_res_norm
            << "]" << std::endl;
...
template <int DIM>
bool StrandCompliantManager<DIM>::stepSceneLinear(TwoDScene<DIM>& scene,
                                                  scalar dt,
                                                  bool updatePreCompute) {
  ...
  for (int i = 0; i < np; ++i) {
    if (scene.isFixed(i)) {
      int numdofs = scene.isMassSpring() || scene.isTip(i) ? DIM : 4;
      m_dx_scripted.segment(scene.getDof(i), numdofs) =
  ...
  SceneStepper<DIM>::m_timing_statistics[0] +=
      (t1 - t0);  // local precomputation
  t0 = t1;
  ...
  t1 = timingutils::seconds();
  SceneStepper<DIM>::m_timing_statistics[2] +=
      (t1 - t0);  // local Matrix Composition
  t0 = t1;
  ...
  t1 = timingutils::seconds();
  SceneStepper<DIM>::m_timing_statistics[3] +=
      (t1 - t0);  // compute Interhair Variables
  t0 = t1;
...
template <int DIM>
bool StrandCompliantManager<DIM>::stepScene(TwoDScene<DIM>& scene, scalar dt,
                                            bool updatePreCompute) {
  if (m_max_num_newton == 0)
    return stepSceneLinear(scene, dt, updatePreCompute);
  else
    return stepSceneNonlinear(scene, dt, updatePreCompute);
}
