...
void HairFlow<DIM>::adjustVolumeGlobal(const scalar& prop) {
  for (int i = 0; i < m_eta.size(); ++i) {
    m_eta(i) =
        sqrt(std::max(0.0, prop) * m_eta(i) * (m_eta(i) + m_rad_vec(i) * 2.0) +
...
template <int DIM>
HairFlow<DIM>::HairFlow(TwoDScene<DIM>* parent,
...
bool HairFlow<DIM>::isContained(int colidx) {
  if (m_parent->getComponent(colidx) == DIM)
    return false;  //[H] todo, check if twist matters here or not
  int ip = m_parent->getVertFromDof(colidx);
  return find(ip) != -1;
}
...
void HairFlow<DIM>::writeReadable(std::ostream& o) const {
  ...
  for (int pidx : m_particle_indices) {
    o << x.segment<DIM>(m_parent->getDof(pidx)).transpose() << " "
      << m_parent->getScriptedGroup(pidx) << "\n";
  }
}
...
Vectors<DIM> HairFlow<DIM>::computeHairLiquidMomentum(const VectorXs& v) const {
  ...
  for (int i = 0; i < ne; ++i) {
    ...
    Vectors<DIM> ve =
        (v0 + v1) * 0.5 + m_dir_f.row(i).transpose().normalized() * m_u(i);
...
template <int DIM>
void HairFlow<DIM>::setConstraintParameters(const Vector6i& start,
                                            const Vector6i& num) {
