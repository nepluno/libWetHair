...
static void sorter_callback(int pidx, int& i, int& j, int& k) {
  ...
  int pi = (int)((p.x(0) - g_fluid2d_iptr->get_origin()(0)) /
                 g_fluid2d_iptr->cellsize());
  int pj = (int)((p.x(1) - g_fluid2d_iptr->get_origin()(1)) /
                 g_fluid2d_iptr->cellsize());
...
FluidSim2D::FluidSim2D(const Vector2s& origin_, scalar width, int ni_, int nj_,
  ...
  u_weight_hair.resize(ni + 1, nj);
  u_valid.resize(ni + 1, nj);
  u_hair.resize(ni + 1, nj);
  u_particle.resize(ni + 1, nj);
  u_weight_particle.resize(ni + 1, nj);
  u_drag.resize(ni + 1, nj);
  u_weight_total.resize(ni + 1, nj);
  u_pressure_grad.resize(ni + 1, nj);
  u_solid.resize(ni + 1, nj);
  v.resize(ni, nj + 1);
  temp_v.resize(ni, nj + 1);
  v_weights.resize(ni, nj + 1);
  v_weight_hair.resize(ni, nj + 1);
  v_valid.resize(ni, nj + 1);
  v_hair.resize(ni, nj + 1);
  v_particle.resize(ni, nj + 1);
  v_weight_particle.resize(ni, nj + 1);
  v_drag.resize(ni, nj + 1);
  v_weight_total.resize(ni, nj + 1);
  v_pressure_grad.resize(ni + 1, nj);
  v_solid.resize(ni, nj + 1);
...
void FluidSim2D::transferLiquidToGridParticle(const scalar& dt) {
  ...
  threadutils::thread_pool::ParallelFor(0, nj, [&](int j) {
    ...
    for (int i = 0; i < ni; ++i) {
      ...
      m_sorter->getCellAt(i, j, 0, [&](int pidx) {
        ...
        if (local_idx == 0) {
          ...
          if (m_pool_liquid_vol_cache(hidx * 2 + 0) > 0.0) {
            avg_pos += x.segment<2>(m_parent->getDof(pidx)) *
                       m_pool_liquid_vol_cache(hidx * 2 + 0);
            avg_vel += (v.segment<2>(m_parent->getDof(pidx)) +
          ...
          if (m_pool_liquid_vol_cache(hidx * 2 + 1) > 0.0) {
            ...
            Matrix2s proj =
                Matrix2s::Identity() - tan_dir * tan_dir.transpose();
      ...
      if (total_pool_liquid > release_vol) {
        ...
        m_pool_liquid_particle_cache[j].push_back(
            Particle<2>(avg_pos, avg_vel, radii_release, PT_LIQUID));
        ...
        // We follow Equ.(3) of [Lorenceau 2004] to determine the maximum radius
        // of liquid attached to hair Lorenceau, Élise, Christophe Clanet, and
        // David Quéré. "Capturing drops with a thin fiber." Journal of colloid
        // and interface science 279.1 (2004): 192-197.
        scalar V_max =
        ...
        if (total_regular_liquid > V_max + release_vol) {
          ...
          for (int p = 0; p < N_release; ++p) {
            ...
            m_regular_liquid_particle_cache[j].push_back(
                Particle<2>(avg_pos + rand_vec_projected, avg_vel,
  ...
  for (int j = 0; j < nj; ++j) {
    ...
    particles.insert(particles.end(), m_pool_liquid_particle_cache[j].begin(),
                     m_pool_liquid_particle_cache[j].end());
      ...
      for (int s = imin_y; s <= imax_y; ++s) {
        for (int r = imin_x; r <= imax_x; ++r) {
          m_sorter->getCellAt(r, s, 0, [&](int pidx) {
            ...
            const scalar rupture_dist = std::max(
                p.radii + eta(j),
                (1.0 + 0.5 * theta) * pow(vol_particle + vol_hair, 1.0 / 3.0));
    ...
    for (int j = 1; j < np - 1; ++j) {
      ...
      if ((mum == MUM_DIRECT_DIV || mum == MUM_MOMENTUM) &&
        ...
        if (new_modified_mass > 0.0)
          v.segment<2>(idof) *= old_modified_mass / new_modified_mass;
        else
...
void FluidSim2D::correct(scalar dt) {
  ...
  threadutils::thread_pool::ParallelFor(0, nj, [&](int j) {
    for (int i = 0; i < ni; ++i) {
      m_sorter->getCellAt(i, j, 0, [&](int pidx_i) {
        if (particles[pidx_i].type != PT_LIQUID || particles[pidx_i].deceased ||
        ...
        m_sorter->getCellAt(i, j, 0, [&](int pidx_j) {
          ...
          particles[pidx_i].x =
              mathutils::lerp(particles[pidx_i].x, particles[pidx_j].x, alpha);
          particles[pidx_i].v =
              mathutils::lerp(particles[pidx_i].v, particles[pidx_j].v, alpha);
          particles[pidx_i].c =
              mathutils::lerp(particles[pidx_i].c, particles[pidx_j].c, alpha);
  ...
  particles.erase(
      std::remove_if(particles.begin(), particles.end(),
                     [](const Particle<2>& p) { return p.deceased; }),
      particles.end());
...
void FluidSim2D::add_drag(scalar dt) {  // drag
...
void FluidSim2D::advect_particles(scalar dt) {
  ...
  for (SourceBoundary<2>* s : sources) {
    ...
    for (const Vector2s& p : s->detectors) {
      ...
      for (int k = 0; k < num_p_need; ++k) {
        scalar x = ((scalar)ix + 0.5 +
                    (((scalar)rand() / (scalar)RAND_MAX) * 0.5 - 0.5)) *
                   dx;
        scalar y = ((scalar)iy + 0.5 +
                    (((scalar)rand() / (scalar)RAND_MAX) * 0.5 - 0.5)) *
                   dx;
  ...
  particles.erase(
      std::remove_if(particles.begin(), particles.end(),
                     [&](const Particle<2>& p) {
                       return p.x(0) < origin(0) ||
                              p.x(0) > origin(0) + ((scalar)ni + 1.) * dx ||
                              p.x(1) < origin(1) ||
                              p.x(1) > origin(1) + ((scalar)nj + 1.) * dx;
                     }),
      particles.end());
...
void FluidSim2D::compute_liquid_phi() {
  ...
  threadutils::thread_pool::ParallelFor(0, nj, [&](int j) {
    for (int i = 0; i < ni; ++i) {
      ...
      m_sorter->getNeigboringParticles_cell(
          i, j, 0, -2, 2, -2, 2, 0, 0, [&](int pidx) {
            const Particle<2>& p = particles[pidx];
            phi_min =
                std::min(phi_min, (pos - p.x).norm() -
                                      1.02 * std::max(dx / sqrt(3.0), p.radii));
          });
...
void FluidSim2D::combine_velocity_field() {
  threadutils::thread_pool::ParallelFor(0, u.nj, [&](int j) {
    for (int i = 0; i < u.ni; ++i) {
      ...
      if (weight > 0) {
        u(i, j) = (u_weight_particle(i, j) * u_particle(i, j) +
                   u_weight_hair(i, j) * u_hair(i, j)) /
                  weight;
      } else {
        u(i, j) = 0.0;
      }
  ...
  threadutils::thread_pool::ParallelFor(0, v.nj, [&](int j) {
    for (int i = 0; i < v.ni; ++i) {
      ...
      if (weight > 0) {
        v(i, j) = (v_weight_particle(i, j) * v_particle(i, j) +
                   v_weight_hair(i, j) * v_hair(i, j)) /
                  weight;
      } else {
        v(i, j) = 0.0;
      }
...
Vector2s FluidSim2D::get_nodal_solid_phi_gradient(
    const Vector2s& position) const {
  Vector2s p = (position - origin) / dx;
  Vector2s normal;
...
Vector2s FluidSim2D::get_velocity(const Vector2s& position, const Array2s& u_,
                                  const Array2s& v_) const {
...
void FluidSim2D::compute_weights() {
  threadutils::thread_pool::ParallelFor(0, u_weights.nj, [&](int j) {
    for (int i = 0; i < u_weights.ni; ++i) {
      u_weights(i, j) =
          1 - mathutils::fraction_inside(nodal_solid_phi(i, j + 1),
                                         nodal_solid_phi(i, j));
  ...
  threadutils::thread_pool::ParallelFor(0, v_weights.nj, [&](int j) {
    for (int i = 0; i < v_weights.ni; ++i) {
      v_weights(i, j) =
          1 - mathutils::fraction_inside(nodal_solid_phi(i + 1, j),
                                         nodal_solid_phi(i, j));
...
void FluidSim2D::init_hair_particles() {
  ...
  for (const HairFlow<2>* flow : flows) {
    ...
    for (int i = 0; i < ne; ++i) {
      ...
      accu_next += (x.segment<2>(m_parent->getDof(e.first)) -
                    x.segment<2>(m_parent->getDof(e.second)))
                       .norm();
      if (accu_next - accu_length > default_distance) {
        scalar alpha = (default_distance + accu_length - prev_accu_next) /
                       (accu_next - prev_accu_next);
        const Vector2s& x0 = x.segment<2>(m_parent->getDof(e.first));
        const Vector2s& x1 = x.segment<2>(m_parent->getDof(e.second));
        ...
        particles.push_back(
            Particle<2>(pos, vel, radius, PT_HAIR, eidx, alpha));
  ...
  for (int i = 0; i < ni; ++i) {
    for (int j = 0; j < nj; ++j) {
      ...
      if (compute_phi_vel(pt, vel) > 0 && x >= srl && x <= srr && y >= srb &&
          y <= srt)
        add_particle(
            Particle<2>(pt, Vector2s::Zero(), dx / sqrt(2.0), PT_LIQUID));
    }
  }
}
...
void FluidSim2D::done_update_from_hair() {
  ...
  threadutils::thread_pool::ParallelFor(0, nflows, [&](int i) {
    ...
    memcpy(&u_vel_drag[u_base_idx], &u_edge_vel_drag[i][0],
           u_edge_vel_drag[i].size() * sizeof(EdgeVelDragIntersection<2>));

    int v_base_idx = (i == 0) ? 0 : v_num_edge_voxel_intersections[i - 1];
    memcpy(&v_vel_drag[v_base_idx], &v_edge_vel_drag[i][0],
           v_edge_vel_drag[i].size() * sizeof(EdgeVelDragIntersection<2>));
  });
      ...
      if (sum_weight > 0) {
        ...
        scalar multiplier2 =
            mathutils::clamp(m_parent->getDragRadiusMultiplier() *
                             ...
                             0.0, sum_linear_weight * dx * dx * dz / sum_vol);
        u_drag(i, j) =
            sum_drag / (sum_linear_weight * dx * dx * dx * rho_L) * multiplier2;
      } else {
        u_hair(i, j) = 0.0;
        u_drag(i, j) = 0.0;
      }
...
const std::vector<FluidSim2D::Boundary<2>*>& FluidSim2D::get_boundaries()
    const {
  return boundaries;
}

const std::vector<FluidSim2D::SourceBoundary<2>*>& FluidSim2D::get_sources()
    const {
  return sources;
}
...
scalar FluidSim2D::computeParticleAngularMomentum() {
  ...
  for (int i = 0; i < np; ++i) {
    ...
    Vector2s pu00 = m * (1.0 - iu_frac) * (1.0 - ju_frac) *
                    Vector2s(p.v(0) + p.c.col(0).dot(xu00 - p.x), 0.0);
    Vector2s pu10 = m * iu_frac * (1.0 - ju_frac) *
                    Vector2s(p.v(0) + p.c.col(0).dot(xu10 - p.x), 0.0);
    Vector2s pu01 = m * (1.0 - iu_frac) * ju_frac *
                    Vector2s(p.v(0) + p.c.col(0).dot(xu01 - p.x), 0.0);
    Vector2s pu11 = m * iu_frac * ju_frac *
                    Vector2s(p.v(0) + p.c.col(0).dot(xu11 - p.x), 0.0);

    sum += mathutils::cross2(xu00, pu00) + mathutils::cross2(xu01, pu01) +
           mathutils::cross2(xu10, pu10) + mathutils::cross2(xu11, pu11);
    ...
    Vector2s pv00 = m * (1.0 - iv_frac) * (1.0 - jv_frac) *
                    Vector2s(0.0, p.v(1) + p.c.col(1).dot(xv00 - p.x));
    Vector2s pv10 = m * iv_frac * (1.0 - jv_frac) *
                    Vector2s(0.0, p.v(1) + p.c.col(1).dot(xv10 - p.x));
    Vector2s pv01 = m * (1.0 - iv_frac) * jv_frac *
                    Vector2s(0.0, p.v(1) + p.c.col(1).dot(xv01 - p.x));
    Vector2s pv11 = m * iv_frac * jv_frac *
                    Vector2s(0.0, p.v(1) + p.c.col(1).dot(xv11 - p.x));

    sum += mathutils::cross2(xv00, pv00) + mathutils::cross2(xv01, pv01) +
           mathutils::cross2(xv10, pv10) + mathutils::cross2(xv11, pv11);
  }
...
scalar FluidSim2D::computeParticleGridAngularMomentum() {
  ...
  for (int j = 0; j < u_particle.nj; ++j)
    for (int i = 0; i < u_particle.ni; ++i) {
      Vector2s pos = Vector2s(i * dx, (j + 0.5) * dx) + origin;
      Vector2s u_vec =
          Vector2s(u_weight_particle(i, j) * u_particle(i, j), 0.0);
  ...
  for (int j = 0; j < v_particle.nj; ++j)
    for (int i = 0; i < v_particle.ni; ++i) {
      Vector2s pos = Vector2s((i + 0.5) * dx, j * dx) + origin;
      Vector2s v_vec =
          Vector2s(0.0, v_weight_particle(i, j) * v_particle(i, j));
...
scalar FluidSim2D::computeHairGridAngularMomentum() {
  ...
  for (int j = 0; j < u_hair.nj; ++j)
    for (int i = 0; i < u_hair.ni; ++i) {
      Vector2s pos = Vector2s(i * dx, (j + 0.5) * dx) + origin;
      Vector2s u_vec = Vector2s(u_weight_hair(i, j) * u_hair(i, j), 0.0);
      sum += mathutils::cross2(pos, u_vec);
    }

  for (int j = 0; j < v_hair.nj; ++j)
    for (int i = 0; i < v_hair.ni; ++i) {
      Vector2s pos = Vector2s((i + 0.5) * dx, j * dx) + origin;
...
scalar FluidSim2D::computeReweightedHairGridAngularMomentum() {
  ...
  for (int j = 0; j < u_hair.nj; ++j)
    for (int i = 1; i < u_hair.ni - 1; ++i) {
      scalar theta = 1.0;
      if (liquid_phi(i, j) >= 0 || liquid_phi(i - 1, j) >= 0)
        theta =
            mathutils::fraction_inside(liquid_phi(i - 1, j), liquid_phi(i, j));

      Vector2s pos = Vector2s(i * dx, (j + 0.5) * dx) + origin;
      Vector2s u_vec =
          Vector2s(u_weight_hair(i, j) * u_hair(i, j) * (1.0 - theta), 0.0);
      sum += mathutils::cross2(pos, u_vec);
    }

  for (int j = 1; j < v_hair.nj - 1; ++j)
    for (int i = 0; i < v_hair.ni; ++i) {
      scalar theta = 1.0;
      if (liquid_phi(i, j) >= 0 || liquid_phi(i, j - 1) >= 0)
        theta =
            mathutils::fraction_inside(liquid_phi(i, j - 1), liquid_phi(i, j));

      Vector2s pos = Vector2s((i + 0.5) * dx, j * dx) + origin;
      Vector2s v_vec =
          Vector2s(0.0, v_weight_hair(i, j) * v_hair(i, j) * (1.0 - theta));
      sum += mathutils::cross2(pos, v_vec);
    }
...
scalar FluidSim2D::computeReweightedParticleGridAngularMomentum() {
  ...
  for (int j = 0; j < u_particle.nj; ++j)
    for (int i = 1; i < u_particle.ni - 1; ++i) {
      ...
      Vector2s pos = Vector2s(i * dx, (j + 0.5) * dx) + origin;
      Vector2s u_vec =
          Vector2s(u_weight_particle(i, j) * u_particle(i, j) * theta, 0.0);
      sum += mathutils::cross2(pos, u_vec);
    }
  ...
  for (int j = 1; j < v_particle.nj - 1; ++j)
    for (int i = 0; i < v_particle.ni; ++i) {
      ...
      Vector2s pos = Vector2s((i + 0.5) * dx, j * dx) + origin;
      Vector2s v_vec =
          Vector2s(0.0, v_weight_particle(i, j) * v_particle(i, j) * theta);
      sum += mathutils::cross2(pos, v_vec);
    }
...
scalar FluidSim2D::computeParticleGridKineticEnergy() {
  ...
  for (int j = 0; j < nj; ++j)
    for (int i = 0; i < ni; ++i) {
      ...
      scalar mass =
          get_velocity(pos, u_weight_particle, v_weight_particle).norm();
...
void FluidSim2D::read(const scalar* data, size_t size_particles,
...
size_t FluidSim2D::crucial_grid_size() const {
  return (u.a.size() + v.a.size() + u_pressure_grad.a.size() +
          v_pressure_grad.a.size() + liquid_phi.a.size()) *
         sizeof(scalar);
}

void FluidSim2D::add_particle(const VectorXs& pos, const VectorXs& vel,
                              const scalar& radii, ParticleType type) {
  add_particle(Particle<2>(pos.segment<2>(0), vel.segment<2>(0), radii, type));
}

void FluidSim2D::preCompute(const VectorXs& x, const VectorXs& v,
...
void FluidSim2D::addGradEToTotal(const VectorXs& x, const VectorXs& v,
...
void extrapolate(Array2s& grid, Array2s& old_grid, const Array2s& grid_weight,
  ...
  for (int j = 1; j < grid.nj - 1; ++j)
    for (int i = 1; i < grid.ni - 1; ++i)
      valid(i, j) =
          grid_weight(i, j) >
          0;  // && (grid_liquid_weight(i, j) < 0 || grid_liquid_weight(i +
              // offset(0), j + offset(1)) < 0 );
