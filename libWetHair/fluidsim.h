...
#ifndef LIBWETHAIR_CORE_FLUIDSIM_H_
#define LIBWETHAIR_CORE_FLUIDSIM_H_
...
template <int DIM>
struct Particle {
  Particle(const Vectors<DIM>& x_, const Vectors<DIM>& v_, const scalar& radii_,
           ParticleType type_);
  Particle(const Vectors<DIM>& x_, const Vectors<DIM>& v_, const scalar& radii_,
           ParticleType type_, int edge_idx_, const scalar& edge_alpha_);
  Particle();
...
class FluidSim {
 public:
  ...
  struct Boundary {
    ...
    virtual scalar compute_phi_vel(const Vectors<DIM>& pos,
                                   Vectors<DIM>& vel) const = 0;
    virtual void write(std::vector<scalar>&) const = 0;
    virtual void read(const scalar* data) = 0;
    virtual size_t size() const = 0;

    bool is_root() const { return (parent == -1); }
  ...
  struct SolidBoundary : public Boundary<DIM> {
    SolidBoundary(const Vectors<DIM>& center_, const VectorXs& parameter_,
  ...
  struct SourceBoundary : public SolidBoundary<DIM> {
    SourceBoundary(const Vectors<DIM>& center_, const VectorXs& parameter_,
                   BOUNDARY_TYPE type_, bool inside, const Vector3s& raxis,
                   const scalar& rangle, const Vectors<DIM>& eject_vel_,
                   const scalar& start_, const scalar& end_,
                   const scalar& spray_angle_, const scalar& drop_radius_prop_,
                   const scalar& sub_activate_length,
                   const scalar& sub_inactivate_length);
  ...
  virtual void add_particle(const VectorXs& pos, const VectorXs& vel,
                            const scalar& radii, ParticleType type) = 0;
  ...
  virtual void read(const scalar* data, size_t size_particles,
                    size_t size_boundaries) = 0;
