...
inline scalar box_phi(const Vector2s& position, const Vector2s& centre,
                      const Vector2s& expand, const scalar& radius) {
...
inline scalar circle_phi(const Vector3s& position, const Vector3s& centre,
                         scalar radius) {
  return ((position - centre).norm() - radius);
}

inline scalar capsule_phi(const Vector2s& position, const Vector2s& centre,
...
inline scalar box_phi(const Vector3s& position, const Vector3s& centre,
  ...
  return min(max(max(dx, dy), dz), 0.0) +
...
template <int DIM>
FluidSim::SolidBoundary<DIM>::SolidBoundary(const Vectors<DIM>& center_,
                                            const VectorXs& parameter_,
                                            BOUNDARY_TYPE type_, bool inside,
                                            const Vector3s& raxis,
                                            const scalar& rangle)
    : Boundary<DIM>(type_),
      center(center_),
...
FluidSim::SourceBoundary<DIM>::SourceBoundary(
    const Vectors<DIM>& center_, const VectorXs& parameter_,
    BOUNDARY_TYPE type_, bool inside, const Vector3s& raxis,
    const scalar& rangle, const Vectors<DIM>& eject_vel_, const scalar& start_,
    const scalar& end_, const scalar& spray_angle_,
    const scalar& drop_radius_prop_, const scalar& sub_activate_length_,
    const scalar& sub_inactivate_length_)
    : SolidBoundary<DIM>(center_, parameter_, type_, inside, raxis, rangle),
      eject_vel(eject_vel_),
      last_parent(NULL),
      start(start_),
      end(end_),
      activated(false),
      spray_angle(spray_angle_),
      drop_radius_prop(drop_radius_prop_),
      sub_activate_length(sub_activate_length_),
      sub_inactivate_length(sub_inactivate_length_) {}

template <>
scalar FluidSim::SolidBoundary<3>::compute_phi_vel(const Vectors<3>& pos,
                                                   Vectors<3>& vel) const {
  scalar phi = 0.0;
...
template <>
scalar FluidSim::SolidBoundary<2>::compute_phi_vel(const Vectors<2>& pos,
                                                   Vectors<2>& vel) const {
  scalar phi = 0.0;
  ...
  switch (type) {
    case BT_BOX: {
      ...
      phi = sign * box_phi(rotp2, center, Vector2s(parameter(0), parameter(1)),
                           parameter(3));
      break;
    }
    case BT_CIRCLE: {
...
void FluidSim::SourceBoundary<2>::sample(FluidSim* parent) {
  ...
  for (int j = 0; j < nj; ++j)
    for (int i = 0; i < ni; ++i) {
      Vector2s pos = Vector2s((i + 0.5) * cellsize, (j + 0.5) * cellsize) +
                     fluid2d->get_origin();
      Vector2s v;
...
void FluidSim::SourceBoundary<2>::advance(const scalar& dt) {
  Eigen::Quaternion<scalar> p =
      Eigen::Quaternion<scalar>(0.0, eject_vel(0), eject_vel(1), 0);
  Vector3s ev = (FluidSim::SolidBoundary<2>::future_rot *
                 FluidSim::SolidBoundary<2>::rot.inverse() * p *
                 FluidSim::SolidBoundary<2>::rot *
                 FluidSim::SolidBoundary<2>::future_rot.inverse())
                    .vec();
  eject_vel(0) = ev(0);
  eject_vel(1) = ev(1);
...
void FluidSim::SourceBoundary<3>::advance(const scalar& dt) {
  Eigen::Quaternion<scalar> p =
      Eigen::Quaternion<scalar>(0.0, eject_vel(0), eject_vel(1), eject_vel(2));
  eject_vel = (FluidSim::SolidBoundary<3>::future_rot *
               FluidSim::SolidBoundary<3>::rot.inverse() * p *
               FluidSim::SolidBoundary<3>::rot *
               FluidSim::SolidBoundary<3>::future_rot.inverse())
                  .vec();
...
void FluidSim::OperatorBoundary<DIM>::advance(const scalar& dt) {
  ...
  threadutils::thread_pool::ParallelFor(
      0, nb, [&](int i) { children[i]->advance(dt); });
}

template <int DIM>
scalar FluidSim::OperatorBoundary<DIM>::compute_phi_vel(
    const Vectors<DIM>& pos, Vectors<DIM>& vel) const {
...
template <int DIM>
Particle<DIM>::Particle(const Vectors<DIM>& x_, const Vectors<DIM>& v_,
                        const scalar& radii_, ParticleType type_)
    : x(x_),
      v(v_),
      radii(radii_),
      type(type_),
...
template <int DIM>
Particle<DIM>::Particle(const Vectors<DIM>& x_, const Vectors<DIM>& v_,
                        const scalar& radii_, ParticleType type_, int edge_idx_,
                        const scalar& edge_alpha_)
    : x(x_),
      v(v_),
      radii(radii_),
      type(type_),
