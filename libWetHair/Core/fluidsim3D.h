...
#ifndef LIBWETHAIR_CORE_FLUIDSIM3D_H_
#define LIBWETHAIR_CORE_FLUIDSIM3D_H_
...
class FluidSim3D : public FluidSim {
 public:
  ...
  FluidSim3D(const Vector3s& origin_, scalar width, int ni_, int nj_, int nk_,
             const std::vector<Boundary<3>*>& boundaries_,
  ...
  Vector3s get_velocity(const Vector3s& position, const Array3s& u,
                        const Array3s& v, const Array3s& w) const;
 ...
 protected:
  ...
  Array3s u_vol_liquid, v_vol_liquid, w_vol_liquid, ex_vol_liquid,
      ey_vol_liquid, ez_vol_liquid, c_vol_liquid;
