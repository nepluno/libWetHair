...
#ifndef LIBWETHAIR_CORE_MATH_UTILITIES_H_
#define LIBWETHAIR_CORE_MATH_UTILITIES_H_
...
template <class T>
void write_matlab(std::ostream& output, const Eigen::SparseMatrix<T>& mat,
                  const char* variable_name, int significant_digits = 18) {
  ...
  for (int k = 0; k < mat.outerSize(); ++k) {
    for (typename Eigen::SparseMatrix<T>::InnerIterator it(mat, k); it; ++it) {
      if (it.value() != 0.0)
        output << (it.row() + 1) << ", " << (it.col() + 1) << ", " << it.value()
               << std::endl;
    }
  }
  output << "]);" << std::endl;
...
const static scalar bdf_weights[6][13] = {
    ...
    {121.0 / 36.0, -11.0, 29.0 / 2.0, -92.0 / 9.0, 51.0 / 12.0, -1.0, 1.0 / 9.0,
     0, 0, 0, 0, 0, 0},
    {625.0 / 144.0, -50.0 / 3.0, 57.0 / 2.0, -266.0 / 9.0, 497.0 / 24.0, -10.0,
     59.0 / 18.0, -2.0 / 3.0, 1.0 / 16.0, 0, 0, 0, 0},
    {18769.0 / 3600.0, -137.0 / 6.0, 287.0 / 6.0, -587.0 / 9.0, 1537.0 / 24.0,
     -3506.0 / 75.0, 461.0 / 18.0, -31.0 / 3.0, 139.0 / 48.0, -1.0 / 2.0,
     1.0 / 25.0, 0, 0},
    {2401.0 / 400.0, -147.0 / 5.0, 291.0 / 4.0, -368.0 / 3.0, 1237.0 / 8.0,
     -3772.0 / 25.0, 5216.0 / 45.0, -70.0, 521.0 / 16.0, -101.0 / 9.0,
     269.0 / 100.0, -2.0 / 5.0, 1.0 / 36.0}};
...
const static scalar gauss_legendre_point[5][5] = {
    ...
    {0.2113248654051871344705659794271923601627349853515625,
     0.7886751345948128655294340205728076398372650146484375, 0, 0, 0},
    {0.112701665379258297861042592558078467845916748046875, 0.5,
     0.887298334620741702138957407441921532154083251953125, 0, 0},
    {0.069431844202973713731097404888714663684368133544921875,
     0.33000947820757187134432797392946667969226837158203125,
     0.66999052179242812865567202607053332030773162841796875,
     0.9305681557970262307577513638534583151340484619140625, 0},
    {0.046910077030668018149839326724759303033351898193359375,
     0.2307653449471585016539165735593996942043304443359375, 0.5,
     0.7692346550528414983460834264406003057956695556640625,
     0.953089922969331926339009442017413675785064697265625}};

const static scalar gauss_legendre_weight[5][5] = {
    {1, 0, 0, 0, 0},
    {0.5, 0.5, 0, 0, 0},
    {5.0 / 18.0, 4.0 / 9.0, 5.0 / 18.0, 0, 0},
    {0.1739274225687269248563637802362791262567043304443359375,
     0.326072577431273102899211835392634384334087371826171875,
     0.326072577431273102899211835392634384334087371826171875,
     0.1739274225687269248563637802362791262567043304443359375},
    {0.118463442528094542449679238416138105094432830810546875,
     0.2393143352496832354514566532088792882859706878662109375, 64. / 255.,
     0.2393143352496832354514566532088792882859706878662109375,
     0.118463442528094542449679238416138105094432830810546875}};
...
inline T linear_kernel(const Eigen::Matrix<T, 2, 1>& d, const T& h) {
  return std::max(((T)1.0 - (T)fabs(d(0) / h)) * ((T)1.0 - (T)fabs(d(1) / h)),
                  (T)0.0);
}
...
inline T akinci_cohesion_kernel(const T& r, const T& h) {
  ...
  if ((T)2. * r > h && r <= h) {
    ...
    return coeff * ((T)2. * (h - r) * (h - r) * (h - r) * r * r * r -
                    pow(h, (T)6.) / (T)64.);
  } else {
    return (T)0.0;
  }
...
inline T poly6_kernel_2d(const T& r2, const T& h) {
  if (r2 <= h * h)
    return (T)4.0 / (M_PI * pow(h, (T)8.0)) * pow(h * h - r2, (T)3.0);
  else
    return 0.0;
}

template <typename T>
inline Eigen::Matrix<T, 2, 1> grad_poly6_kernel_2d(
    const Eigen::Matrix<T, 2, 1>& d, const T& h) {
  T r2 = d.squaredNorm();
  if (r2 <= h * h)
    return (T)-24.0 / (M_PI * pow(h, (T)8.0)) * pow(h * h - r2, (T)2.0) * d;
  else
    return Eigen::Matrix<T, 2, 1>::Zero();
}

template <typename T>
inline void orientationMatrix(const Eigen::Matrix<T, 3, 1>& t, const T& r,
...
inline T softsqrt(const T& x, const T& h) {
  if (x < h) {
    return x * (15.0 * h * h - 10.0 * h * x + 3.0 * x * x) /
           (8.0 * sqrt(h * h * h * h * h));
  } else {
    return sqrt(x);
  }
...
inline T gradsoftsqrt(const T& x, const T& h) {
  if (x < h) {
    return (15.0 * h * h - 20.0 * h * x + 9.0 * x * x) /
           (8.0 * sqrt(h * h * h * h * h));
  } else {
    return 0.5 / sqrt(x);
  }
...
inline T softmax(const T& x, const T& xa, const T& h) {
  if (x < xa - h) {
    ...
    return (h * h + (T)2.0 * h * x + (T)2.0 * h * xa + x * x - (T)2.0 * x * xa +
            xa * xa) /
           ((T)4.0 * h);
  } else {
    return x;
...
inline T softclamp(const T& x, const T& xa, const T& xb, const T& h) {
  if (x < xa - h) {
    ...
    const T t5 = xa * xa;
    return t3 *
               (t4 * t5 * 6.0 + (t4 * t4) * 3.0 - t5 * t5 + h * t4 * xa * 8.0) *
               (1.0 / 1.6E1) -
           (t2 * t2) * t3 * (1.0 / 1.6E1) +
           t2 * t3 * (t4 * 6.0 - t5 * 6.0) * (1.0 / 1.6E1) +
           t3 * x * (h * t4 * 8.0 - t4 * xa * 1.2E1 + t5 * xa * 4.0) *
               (1.0 / 1.6E1) +
           t2 * t3 * x * xa * (1.0 / 4.0);
  } else if (x < xb - h) {
    return x;
    ...
    const T t5 = xb * xb;
    return t3 *
               (t4 * t5 * 6.0 + (t4 * t4) * 3.0 - t5 * t5 - h * t4 * xb * 8.0) *
               (-1.0 / 1.6E1) +
           (t2 * t2) * t3 * (1.0 / 1.6E1) -
           t2 * t3 * (t4 * 3.0 - t5 * 3.0) * (1.0 / 8.0) +
           t3 * x * (h * t4 * 2.0 + t4 * xb * 3.0 - t5 * xb) * (1.0 / 4.0) -
           t2 * t3 * x * xb * (1.0 / 4.0);
  } else {
    return xb;
  }
...
inline T gradsoftclamp(const T& x, const T& xa, const T& xb, const T& h) {
  if (x < xa - h) {
    ...
    return (h + x - xa) * (h + x - xa) * (2.0 * h - x + xa) /
           ((T)4.0 * h * h * h);
  } else if (x < xb - h) {
    return (T)1.0;
  } else if (x < xb + h) {
    return (h - x + xb) * (h - x + xb) * (2.0 * h + x - xb) /
           ((T)4.0 * h * h * h);
  } else {
    return (T)0;
  }
...
template <typename T>
inline T cross2(const Eigen::Matrix<T, 2, 1>& a,
                const Eigen::Matrix<T, 2, 1>& b) {
  return a(0) * b(1) - a(1) * b(0);
}

template <typename T>
inline T unicross(const Eigen::Matrix<T, 2, 1>& a,
                  const Eigen::Matrix<T, 2, 1>& b) {
...
template <typename T>
inline T unicross(const Eigen::Matrix<T, 3, 1>& a,
                  const Eigen::Matrix<T, 3, 1>& b) {
  return a.cross(b).norm();
}

template <typename T>
inline Eigen::Matrix<T, 3, 1> unicross_vec(const Eigen::Matrix<T, 2, 1>& a,
                                           const Eigen::Matrix<T, 2, 1>& b) {
  return Eigen::Matrix<T, 3, 1>((T)0.0, (T)0.0, cross2(a, b));
}

template <typename T>
inline Eigen::Matrix<T, 3, 1> unicross_vec(const Eigen::Matrix<T, 3, 1>& a,
                                           const Eigen::Matrix<T, 3, 1>& b) {
  return a.cross(b);
}

template <typename T, int DIM>
inline T pointedgedist(const Eigen::Matrix<T, DIM, 1>& x1,
                       const Eigen::Matrix<T, DIM, 1>& x2,
                       const Eigen::Matrix<T, DIM, 1>& x3) {
...
template <typename T, int DIM>
inline void gradsoftcapsule_phi(Eigen::Matrix<T, DIM * 3, 1>& grad,
                                const Eigen::Matrix<T, DIM, 1>& p,
                                const Eigen::Matrix<T, DIM, 1>& a,
                                const Eigen::Matrix<T, DIM, 1>& b,
                                const T& radius, const T& h) {
  ...
  Eigen::Matrix<T, DIM, DIM> dudp =
      Eigen::Matrix<T, DIM, DIM>::Identity() -
      gradsoftclamp(d, 0.0, 1.0, h) * (ba * ba.transpose()) / sigma;
  ...
  Eigen::Matrix<T, DIM, DIM> duda =
      (alpha - 1.0) * Eigen::Matrix<T, DIM, DIM>::Identity() -
      ba * (dscdd * ddda.transpose());
  Eigen::Matrix<T, DIM, DIM> dudb =
      -alpha * Eigen::Matrix<T, DIM, DIM>::Identity() -
      ba * (dscdd * dddb.transpose());

  grad.segment(0, DIM) =
      gradsoftsqrt(u.squaredNorm(), h) * 2.0 * dudp.transpose() * u;
  grad.segment(DIM, DIM) =
      gradsoftsqrt(u.squaredNorm(), h) * 2.0 * duda.transpose() * u;
  grad.segment(DIM * 2, DIM) =
      gradsoftsqrt(u.squaredNorm(), h) * 2.0 * dudb.transpose() * u;
}

template <typename T>
inline void hesssoftcapsule_phi(Eigen::Matrix<T, 6, 6>& hess,
  ...
  hess(0, 0) = t28 * (t72 + (t17 * t17) * 2.0 + t2 * t4 * t5 * t20 * t21 * 2.0 +
                      t3 * t5 * t16 * t20 * t21 * 2.0) +
               (t27 * t27) * t29;
  hess(0, 1) = t71;
  hess(0, 2) =
      t95 +
      t28 *
          (t54 + t93 + t94 +
           t17 *
               (-t15 + t2 * t19 * (t37 + t8 * (qa0 * -2.0 + qb0 + qx0)) + 1.0) *
               2.0 +
           t4 * (t51 + t91 - t8 * t19 * t36) * 2.0 - t3 * t8 * t16 * t19 * 2.0);
  hess(0, 3) =
      t119 +
      t28 * (t59 + t117 + t118 + t4 * (t56 + t5 * t8 * t21 * t48) * 2.0 -
             t2 * t8 * t16 * t19 * 2.0 + t2 * t3 * t8 * t16 * t21 * t48 * 2.0);
  hess(0, 4) = t144 - t28 * (t54 + t142 + t143 - t17 * t55 * 2.0 +
                             t4 * (t51 + t141 - t8 * t19 * t36) * 2.0 -
                             t3 * t8 * t16 * t19 * 2.0);
  hess(0, 5) =
      t173 - t28 * (t59 + t171 + t4 * (t56 + t5 * t8 * t21 * t57) * 2.0 -
                    t2 * t8 * t16 * t19 * 2.0 - t2 * t3 * t8 * t19 * t60 * 2.0 +
                    t2 * t3 * t8 * t16 * t21 * t57 * 2.0);
  hess(1, 0) = t71;
  hess(1, 1) = t28 * (t72 + (t33 * t33) * 2.0 + t2 * t4 * t6 * t20 * t21 * 2.0 +
                      t3 * t6 * t16 * t20 * t21 * 2.0) +
               t29 * (t69 * t69);
  hess(1, 2) =
      t98 +
      t28 * (t83 + t96 + t97 + t16 * (t81 + t6 * t8 * t21 * t41) * 2.0 -
             t3 * t4 * t8 * t19 * 2.0 + t2 * t3 * t4 * t8 * t21 * t41 * 2.0);
  hess(1, 3) = t125 + t28 * (t88 + t121 + t123 + t124 +
                             t16 * (t86 + t120 - t8 * t19 * t44) * 2.0 -
                             t2 * t4 * t8 * t19 * 2.0);
  hess(1, 4) =
      t149 - t28 * (t83 + t147 + t16 * (t81 + t6 * t8 * t21 * t52) * 2.0 -
                    t3 * t4 * t8 * t19 * 2.0 - t2 * t3 * t8 * t19 * t55 * 2.0 +
                    t2 * t3 * t4 * t8 * t21 * t52 * 2.0);
  hess(1, 5) = t179 - t28 * (t88 + t177 + t178 - t33 * t60 * 2.0 +
                             t16 * (t86 + t176 - t8 * t19 * t44) * 2.0 -
                             t2 * t4 * t8 * t19 * 2.0);
  hess(2, 0) = t95 + t28 * (t93 + t94 + t17 * t74 * 2.0 -
                            t4 * (-t91 + t116 + t140) * 2.0 -
                            t3 * t16 * t19 * t92 * 2.0);
  hess(2, 1) =
      t98 +
      t28 * (t96 + t97 + t148 +
             t4 * (t146 - t3 * t8 * t19 + t2 * t3 * t8 * t21 * t41) * 2.0 +
             t6 * t8 * t16 * t21 * t41 * 2.0);
  hess(2, 2) =
      t28 * ((t74 * t74) * 2.0 +
             t4 * (t19 * t41 * -2.0 + t2 * t21 * t99 + t2 * t19 * t105) * 2.0 +
             t6 * t38 * t99 * 2.0 + t3 * t16 * t21 * t99 * 2.0 +
             t3 * t16 * t19 * t105 * 2.0) +
      t29 * (t77 * t77);
  hess(2, 3) =
      t129 +
      t28 * (t127 + t128 +
             t4 * (-t19 * t48 + t2 * t19 * t109 + t2 * t21 * t41 * t48) * 2.0 -
             t16 * t19 * t41 * 2.0 + t3 * t16 * t19 * t109 * 2.0 +
             t3 * t16 * t21 * t41 * t48 * 2.0);
  hess(2, 4) = t157;
  hess(2, 5) =
      t181 -
      t28 * (t180 +
             t4 * (-t19 * t57 + t2 * t19 * t114 + t2 * t21 * t41 * t57) * 2.0 -
             t16 * t19 * t41 * 2.0 - t3 * t19 * t41 * t60 * 2.0 +
             t3 * t16 * t19 * t114 * 2.0 + t3 * t16 * t21 * t41 * t57 * 2.0);
  hess(3, 0) =
      t119 + t28 * (t117 + t118 + t172 +
                    t16 * (-t116 + t170 + t2 * t3 * t8 * t21 * t48) * 2.0 +
                    t4 * t5 * t8 * t21 * t48 * 2.0);
  hess(3, 1) =
      t125 + t28 * (t121 + t123 + t124 - t16 * (-t120 + t145 + t175) * 2.0 -
                    t2 * t4 * t19 * t122 * 2.0);
  hess(3, 2) =
      t129 +
      t28 * (t127 + t128 +
             t16 * (-t126 + t3 * t19 * t109 + t3 * t21 * t41 * t48) * 2.0 -
             t4 * t19 * t48 * 2.0 + t2 * t4 * t19 * t109 * 2.0 +
             t2 * t4 * t21 * t41 * t48 * 2.0);
  hess(3, 3) =
      t28 *
          ((t50 * t50) * 2.0 +
           t16 * (t19 * t48 * -2.0 + t3 * t21 * t130 + t3 * t19 * t134) * 2.0 +
           t5 * t38 * t130 * 2.0 + t2 * t4 * t21 * t130 * 2.0 +
           t2 * t4 * t19 * t134 * 2.0) +
      t29 * (t80 * t80);
  hess(3, 4) =
      t161 -
      t28 *
          (t160 + t16 * (-t135 + t3 * t19 * t136 + t3 * t21 * t48 * t52) * 2.0 -
           t4 * t19 * t48 * 2.0 - t2 * t19 * t48 * t55 * 2.0 +
           t2 * t4 * t19 * t136 * 2.0 + t2 * t4 * t21 * t48 * t52 * 2.0);
  hess(3, 5) = t189;
  hess(4, 0) = t144 + t28 * (-t142 - t143 + t17 * t55 * 2.0 +
                             t4 * (t116 + t140 - t141) * 2.0 +
                             t3 * t16 * t19 * t92 * 2.0);
  hess(4, 1) =
      t149 -
      t28 *
          (t147 + t148 + t4 * (-t145 + t146 + t2 * t3 * t8 * t21 * t52) * 2.0 -
           t2 * t3 * t8 * t19 * t55 * 2.0 + t6 * t8 * t16 * t21 * t52 * 2.0);
  hess(4, 2) = t157;
  hess(4, 3) =
      t161 -
      t28 *
          (t160 + t4 * (-t158 + t2 * t19 * t136 + t2 * t21 * t48 * t52) * 2.0 -
           t16 * t19 * t52 * 2.0 - t2 * t19 * t48 * t55 * 2.0 +
           t3 * t16 * t19 * t136 * 2.0 + t3 * t16 * t21 * t48 * t52 * 2.0);
  hess(4, 4) =
      t28 *
          (t4 * (t19 * t52 * 2.0 - t2 * t21 * (t162 * t162) + t2 * t19 * t165) *
               -2.0 +
           (t55 * t55) * 2.0 + t6 * t38 * (t163 * t163) * 2.0 -
           t3 * t16 * t19 * t165 * 2.0 + t3 * t16 * t21 * (t166 * t166) * 2.0) +
      t29 * (t85 * t85);
  hess(4, 5) =
      t192 - t28 * (t190 + t191 +
                    t4 * (t167 + t2 * t19 * t169 - t2 * t21 * t52 * t57) * 2.0 +
                    t16 * t19 * t52 * 2.0 + t3 * t16 * t19 * t169 * 2.0 -
                    t3 * t16 * t21 * t52 * t57 * 2.0);
  hess(5, 0) =
      t173 -
      t28 *
          (t171 + t172 + t16 * (-t116 + t170 + t2 * t3 * t8 * t21 * t57) * 2.0 -
           t2 * t3 * t8 * t19 * t60 * 2.0 + t4 * t5 * t8 * t21 * t57 * 2.0);
  hess(5, 1) = t179 + t28 * (-t177 - t178 + t33 * t60 * 2.0 +
                             t16 * (t145 + t175 - t176) * 2.0 +
                             t2 * t4 * t19 * t122 * 2.0);
  hess(5, 2) =
      t181 -
      t28 *
          (t180 + t16 * (-t126 + t3 * t19 * t114 + t3 * t21 * t41 * t57) * 2.0 -
           t4 * t19 * t57 * 2.0 - t3 * t19 * t41 * t60 * 2.0 +
           t2 * t4 * t19 * t114 * 2.0 + t2 * t4 * t21 * t41 * t57 * 2.0);
  hess(5, 3) = t189;
  hess(5, 4) =
      t192 -
      t28 * (t190 + t191 +
             t16 * (t135 + t3 * t19 * t169 - t3 * t21 * t52 * t57) * 2.0 +
             t4 * t19 * t57 * 2.0 + t2 * t4 * t19 * t169 * 2.0 -
             t2 * t4 * t21 * t52 * t57 * 2.0);
  hess(5, 5) =
      t28 *
          (t16 *
               (t19 * t57 * 2.0 - t3 * t21 * (t193 * t193) + t3 * t19 * t196) *
               -2.0 +
           (t60 * t60) * 2.0 + t5 * t38 * (t194 * t194) * 2.0 -
           t2 * t4 * t19 * t196 * 2.0 + t2 * t4 * t21 * (t197 * t197) * 2.0) +
      t29 * (t90 * t90);
}

template <typename T>
inline void hesssoftcapsule_phi(Eigen::Matrix<T, 9, 9>& hess,
  ...
  hess(0, 0) = t133 + t154 + t31 * t34 * t35 * 2.0 +
               t5 * t31 * t38 * t42 * 2.0 +
               t2 * t5 * t6 * t35 * t36 * t37 * 2.0 +
               t3 * t6 * t20 * t35 * t36 * t37 * 2.0 +
               t4 * t6 * t21 * t35 * t36 * t37 * 2.0 +
               t2 * t3 * t10 * t20 * t29 * t38 * t42 * 2.0 +
               t2 * t4 * t10 * t21 * t29 * t38 * t42 * 2.0;
  hess(0, 1) = t127 + t128 + t131 + t132 + t5 * t31 * t38 * t48 * 2.0 +
               t2 * t3 * t10 * t31 * t32 * t35 * 2.0 +
               t2 * t3 * t10 * t29 * t35 * t44 * 2.0 +
               t2 * t3 * t10 * t20 * t29 * t38 * t48 * 2.0 +
               t2 * t4 * t10 * t21 * t29 * t38 * t48 * 2.0;
  hess(0, 2) = t144 + t145 + t148 + t149 + t5 * t31 * t38 * t54 * 2.0 +
               t2 * t4 * t10 * t31 * t32 * t35 * 2.0 +
               t2 * t4 * t10 * t29 * t35 * t50 * 2.0 +
               t2 * t3 * t10 * t20 * t29 * t38 * t54 * 2.0 +
               t2 * t4 * t10 * t21 * t29 * t38 * t54 * 2.0;
  hess(0, 3) = t103 + t104 +
               t5 * t35 * (t94 - t10 * t29 * t57 + t6 * t10 * t37 * t61) * 2.0 +
               t31 * t35 * t63 * 2.0 + t5 * t31 * t38 * t67 * 2.0 -
               t3 * t10 * t20 * t29 * t35 * 2.0 -
               t4 * t10 * t21 * t29 * t35 * 2.0 +
               t2 * t3 * t10 * t20 * t35 * t37 * t61 * 2.0 +
               t2 * t3 * t10 * t20 * t29 * t38 * t67 * 2.0 +
               t2 * t4 * t10 * t21 * t35 * t37 * t61 * 2.0 +
               t2 * t4 * t10 * t21 * t29 * t38 * t67 * 2.0 +
               t2 * t7 * t10 * t29 * t32 * t35 * t61 * 2.0 +
               t2 * t8 * t10 * t29 * t32 * t35 * t61 * 2.0;
  hess(0, 4) =
      t114 + t115 +
      t5 * t35 *
          (t105 + t6 * t10 * t37 * (t71 + t10 * (a(1) * -2.0 + b(1) + p(1)))) *
          2.0 +
      t5 * t31 * t38 * t80 * 2.0 - t2 * t10 * t20 * t29 * t35 * 2.0 +
      t2 * t31 * t32 * t35 * t74 * 2.0 + t2 * t3 * t10 * t29 * t35 * t76 * 2.0 +
      t2 * t3 * t10 * t20 * t35 * t37 * t74 * 2.0 +
      t2 * t3 * t10 * t20 * t29 * t38 * t80 * 2.0 +
      t2 * t4 * t10 * t21 * t35 * t37 * t74 * 2.0 +
      t2 * t4 * t10 * t21 * t29 * t38 * t80 * 2.0 +
      t2 * t8 * t10 * t29 * t32 * t35 * t74 * 2.0;
  hess(0, 5) =
      t125 + t126 +
      t5 * t35 *
          (t116 + t6 * t10 * t37 * (t84 + t10 * (a(2) * -2.0 + b(2) + p(2)))) *
          2.0 +
      t5 * t31 * t38 * t93 * 2.0 - t2 * t10 * t21 * t29 * t35 * 2.0 +
      t2 * t31 * t32 * t35 * t87 * 2.0 + t2 * t4 * t10 * t29 * t35 * t89 * 2.0 +
      t2 * t3 * t10 * t20 * t35 * t37 * t87 * 2.0 +
      t2 * t3 * t10 * t20 * t29 * t38 * t93 * 2.0 +
      t2 * t4 * t10 * t21 * t35 * t37 * t87 * 2.0 +
      t2 * t4 * t10 * t21 * t29 * t38 * t93 * 2.0 +
      t2 * t7 * t10 * t29 * t32 * t35 * t87 * 2.0;
  hess(0, 6) = -t103 - t104 + t134 + t157 -
               t5 * t35 * (t94 - t10 * t29 * t57 + t6 * t10 * t37 * t95) * 2.0 +
               t31 * t35 * t97 * 2.0 - t5 * t31 * t38 * t101 * 2.0 -
               t2 * t3 * t10 * t20 * t35 * t37 * t95 * 2.0 -
               t2 * t3 * t10 * t20 * t29 * t38 * t101 * 2.0 -
               t2 * t4 * t10 * t21 * t35 * t37 * t95 * 2.0 -
               t2 * t4 * t10 * t21 * t29 * t38 * t101 * 2.0 -
               t2 * t7 * t10 * t29 * t32 * t35 * t95 * 2.0 -
               t2 * t8 * t10 * t29 * t32 * t35 * t95 * 2.0;
  hess(0, 7) = -t114 - t115 - t5 * t35 * (t105 + t6 * t10 * t37 * t106) * 2.0 -
               t5 * t31 * t38 * t112 * 2.0 + t2 * t10 * t20 * t29 * t35 * 2.0 -
               t2 * t31 * t32 * t35 * t106 * 2.0 +
               t2 * t3 * t10 * t29 * t35 * t108 * 2.0 -
               t2 * t3 * t10 * t20 * t35 * t37 * t106 * 2.0 -
               t2 * t3 * t10 * t20 * t29 * t38 * t112 * 2.0 -
               t2 * t4 * t10 * t21 * t35 * t37 * t106 * 2.0 -
               t2 * t4 * t10 * t21 * t29 * t38 * t112 * 2.0 -
               t2 * t8 * t10 * t29 * t32 * t35 * t106 * 2.0;
  hess(0, 8) = -t125 - t126 - t5 * t35 * (t116 + t6 * t10 * t37 * t117) * 2.0 -
               t5 * t31 * t38 * t123 * 2.0 + t2 * t10 * t21 * t29 * t35 * 2.0 -
               t2 * t31 * t32 * t35 * t117 * 2.0 +
               t2 * t4 * t10 * t29 * t35 * t119 * 2.0 -
               t2 * t3 * t10 * t20 * t35 * t37 * t117 * 2.0 -
               t2 * t3 * t10 * t20 * t29 * t38 * t123 * 2.0 -
               t2 * t4 * t10 * t21 * t35 * t37 * t117 * 2.0 -
               t2 * t4 * t10 * t21 * t29 * t38 * t123 * 2.0 -
               t2 * t7 * t10 * t29 * t32 * t35 * t117 * 2.0;
  hess(1, 0) = t127 + t128 + t131 + t132 + t20 * t38 * t42 * t130 * 2.0 +
               t2 * t3 * t10 * t29 * t34 * t35 * 2.0 +
               t2 * t3 * t10 * t32 * t35 * t130 * 2.0 +
               t2 * t3 * t5 * t10 * t29 * t38 * t42 * 2.0 +
               t3 * t4 * t10 * t21 * t29 * t38 * t42 * 2.0;
  hess(1, 1) = t133 + t155 + t35 * t44 * t130 * 2.0 +
               t20 * t38 * t48 * t130 * 2.0 +
               t2 * t5 * t7 * t35 * t36 * t37 * 2.0 +
               t3 * t7 * t20 * t35 * t36 * t37 * 2.0 +
               t4 * t7 * t21 * t35 * t36 * t37 * 2.0 +
               t2 * t3 * t5 * t10 * t29 * t38 * t48 * 2.0 +
               t3 * t4 * t10 * t21 * t29 * t38 * t48 * 2.0;
  hess(1, 2) = t150 + t151 + t152 + t153 + t20 * t38 * t54 * t130 * 2.0 +
               t3 * t4 * t10 * t29 * t35 * t50 * 2.0 +
               t3 * t4 * t10 * t32 * t35 * t130 * 2.0 +
               t2 * t3 * t5 * t10 * t29 * t38 * t54 * 2.0 +
               t3 * t4 * t10 * t21 * t29 * t38 * t54 * 2.0;
  hess(1, 3) = t136 + t137 + t20 * t35 * (t135 + t7 * t10 * t37 * t61) * 2.0 +
               t20 * t38 * t67 * t130 * 2.0 - t3 * t5 * t10 * t29 * t35 * 2.0 +
               t3 * t32 * t35 * t61 * t130 * 2.0 +
               t2 * t3 * t10 * t29 * t35 * t63 * 2.0 +
               t2 * t3 * t5 * t10 * t35 * t37 * t61 * 2.0 +
               t2 * t3 * t5 * t10 * t29 * t38 * t67 * 2.0 +
               t3 * t4 * t10 * t21 * t35 * t37 * t61 * 2.0 +
               t3 * t4 * t10 * t21 * t29 * t38 * t67 * 2.0 +
               t3 * t8 * t10 * t29 * t32 * t35 * t61 * 2.0;
  hess(1, 4) =
      -t134 + t139 + t140 +
      t20 * t35 * (t138 - t10 * t29 * t70 + t7 * t10 * t37 * t74) * 2.0 +
      t35 * t76 * t130 * 2.0 + t20 * t38 * t80 * t130 * 2.0 -
      t2 * t5 * t10 * t29 * t35 * 2.0 +
      t2 * t3 * t5 * t10 * t35 * t37 * t74 * 2.0 +
      t2 * t3 * t5 * t10 * t29 * t38 * t80 * 2.0 +
      t3 * t4 * t10 * t21 * t35 * t37 * t74 * 2.0 +
      t3 * t4 * t10 * t21 * t29 * t38 * t80 * 2.0 +
      t3 * t6 * t10 * t29 * t32 * t35 * t74 * 2.0 +
      t3 * t8 * t10 * t29 * t32 * t35 * t74 * 2.0;
  hess(1, 5) = t142 + t143 + t20 * t35 * (t141 + t7 * t10 * t37 * t87) * 2.0 +
               t20 * t38 * t93 * t130 * 2.0 - t3 * t10 * t21 * t29 * t35 * 2.0 +
               t3 * t32 * t35 * t87 * t130 * 2.0 +
               t3 * t4 * t10 * t29 * t35 * t89 * 2.0 +
               t2 * t3 * t5 * t10 * t35 * t37 * t87 * 2.0 +
               t2 * t3 * t5 * t10 * t29 * t38 * t93 * 2.0 +
               t3 * t4 * t10 * t21 * t35 * t37 * t87 * 2.0 +
               t3 * t4 * t10 * t21 * t29 * t38 * t93 * 2.0 +
               t3 * t6 * t10 * t29 * t32 * t35 * t87 * 2.0;
  hess(1, 6) = -t136 - t137 - t20 * t35 * (t135 + t7 * t10 * t37 * t95) * 2.0 -
               t20 * t38 * t101 * t130 * 2.0 + t3 * t5 * t10 * t29 * t35 * 2.0 -
               t3 * t32 * t35 * t95 * t130 * 2.0 +
               t2 * t3 * t10 * t29 * t35 * t97 * 2.0 -
               t2 * t3 * t5 * t10 * t35 * t37 * t95 * 2.0 -
               t2 * t3 * t5 * t10 * t29 * t38 * t101 * 2.0 -
               t3 * t4 * t10 * t21 * t35 * t37 * t95 * 2.0 -
               t3 * t4 * t10 * t21 * t29 * t38 * t101 * 2.0 -
               t3 * t8 * t10 * t29 * t32 * t35 * t95 * 2.0;
  hess(1, 7) =
      t134 - t139 - t140 + t156 -
      t20 * t35 * (t138 - t10 * t29 * t70 + t7 * t10 * t37 * t106) * 2.0 +
      t35 * t108 * t130 * 2.0 - t20 * t38 * t112 * t130 * 2.0 -
      t2 * t3 * t5 * t10 * t35 * t37 * t106 * 2.0 -
      t2 * t3 * t5 * t10 * t29 * t38 * t112 * 2.0 -
      t3 * t4 * t10 * t21 * t35 * t37 * t106 * 2.0 -
      t3 * t4 * t10 * t21 * t29 * t38 * t112 * 2.0 -
      t3 * t6 * t10 * t29 * t32 * t35 * t106 * 2.0 -
      t3 * t8 * t10 * t29 * t32 * t35 * t106 * 2.0;
  hess(1, 8) = -t142 - t143 - t20 * t35 * (t141 + t7 * t10 * t37 * t117) * 2.0 -
               t20 * t38 * t123 * t130 * 2.0 +
               t3 * t10 * t21 * t29 * t35 * 2.0 -
               t3 * t32 * t35 * t117 * t130 * 2.0 +
               t3 * t4 * t10 * t29 * t35 * t119 * 2.0 -
               t2 * t3 * t5 * t10 * t35 * t37 * t117 * 2.0 -
               t2 * t3 * t5 * t10 * t29 * t38 * t123 * 2.0 -
               t3 * t4 * t10 * t21 * t35 * t37 * t117 * 2.0 -
               t3 * t4 * t10 * t21 * t29 * t38 * t123 * 2.0 -
               t3 * t6 * t10 * t29 * t32 * t35 * t117 * 2.0;
  hess(2, 0) = t144 + t145 + t148 + t149 + t21 * t38 * t42 * t147 * 2.0 +
               t2 * t4 * t10 * t29 * t34 * t35 * 2.0 +
               t2 * t4 * t10 * t32 * t35 * t147 * 2.0 +
               t2 * t4 * t5 * t10 * t29 * t38 * t42 * 2.0 +
               t3 * t4 * t10 * t20 * t29 * t38 * t42 * 2.0;
  hess(2, 1) = t150 + t151 + t152 + t153 + t21 * t38 * t48 * t147 * 2.0 +
               t3 * t4 * t10 * t29 * t35 * t44 * 2.0 +
               t3 * t4 * t10 * t32 * t35 * t147 * 2.0 +
               t2 * t4 * t5 * t10 * t29 * t38 * t48 * 2.0 +
               t3 * t4 * t10 * t20 * t29 * t38 * t48 * 2.0;
  hess(2, 2) = t154 + t155 + t35 * t50 * t147 * 2.0 +
               t21 * t38 * t54 * t147 * 2.0 +
               t2 * t5 * t8 * t35 * t36 * t37 * 2.0 +
               t3 * t8 * t20 * t35 * t36 * t37 * 2.0 +
               t4 * t8 * t21 * t35 * t36 * t37 * 2.0 +
               t2 * t4 * t5 * t10 * t29 * t38 * t54 * 2.0 +
               t3 * t4 * t10 * t20 * t29 * t38 * t54 * 2.0;
  hess(2, 3) = t159 + t160 + t21 * t35 * (t158 + t8 * t10 * t37 * t61) * 2.0 +
               t21 * t38 * t67 * t147 * 2.0 - t4 * t5 * t10 * t29 * t35 * 2.0 +
               t4 * t32 * t35 * t61 * t147 * 2.0 +
               t2 * t4 * t10 * t29 * t35 * t63 * 2.0 +
               t2 * t4 * t5 * t10 * t35 * t37 * t61 * 2.0 +
               t2 * t4 * t5 * t10 * t29 * t38 * t67 * 2.0 +
               t3 * t4 * t10 * t20 * t35 * t37 * t61 * 2.0 +
               t3 * t4 * t10 * t20 * t29 * t38 * t67 * 2.0 +
               t4 * t7 * t10 * t29 * t32 * t35 * t61 * 2.0;
  hess(2, 4) = t162 + t163 + t21 * t35 * (t161 + t8 * t10 * t37 * t74) * 2.0 +
               t21 * t38 * t80 * t147 * 2.0 - t4 * t10 * t20 * t29 * t35 * 2.0 +
               t4 * t32 * t35 * t74 * t147 * 2.0 +
               t3 * t4 * t10 * t29 * t35 * t76 * 2.0 +
               t2 * t4 * t5 * t10 * t35 * t37 * t74 * 2.0 +
               t2 * t4 * t5 * t10 * t29 * t38 * t80 * 2.0 +
               t3 * t4 * t10 * t20 * t35 * t37 * t74 * 2.0 +
               t3 * t4 * t10 * t20 * t29 * t38 * t80 * 2.0 +
               t4 * t6 * t10 * t29 * t32 * t35 * t74 * 2.0;
  hess(2, 5) =
      -t156 - t157 + t165 + t166 +
      t21 * t35 * (t164 - t10 * t29 * t83 + t8 * t10 * t37 * t87) * 2.0 +
      t35 * t89 * t147 * 2.0 + t21 * t38 * t93 * t147 * 2.0 +
      t2 * t4 * t5 * t10 * t35 * t37 * t87 * 2.0 +
      t2 * t4 * t5 * t10 * t29 * t38 * t93 * 2.0 +
      t3 * t4 * t10 * t20 * t35 * t37 * t87 * 2.0 +
      t3 * t4 * t10 * t20 * t29 * t38 * t93 * 2.0 +
      t4 * t6 * t10 * t29 * t32 * t35 * t87 * 2.0 +
      t4 * t7 * t10 * t29 * t32 * t35 * t87 * 2.0;
  hess(2, 6) = -t159 - t160 - t21 * t35 * (t158 + t8 * t10 * t37 * t95) * 2.0 -
               t21 * t38 * t101 * t147 * 2.0 + t4 * t5 * t10 * t29 * t35 * 2.0 -
               t4 * t32 * t35 * t95 * t147 * 2.0 +
               t2 * t4 * t10 * t29 * t35 * t97 * 2.0 -
               t2 * t4 * t5 * t10 * t35 * t37 * t95 * 2.0 -
               t2 * t4 * t5 * t10 * t29 * t38 * t101 * 2.0 -
               t3 * t4 * t10 * t20 * t35 * t37 * t95 * 2.0 -
               t3 * t4 * t10 * t20 * t29 * t38 * t101 * 2.0 -
               t4 * t7 * t10 * t29 * t32 * t35 * t95 * 2.0;
  hess(2, 7) = -t162 - t163 - t21 * t35 * (t161 + t8 * t10 * t37 * t106) * 2.0 -
               t21 * t38 * t112 * t147 * 2.0 +
               t4 * t10 * t20 * t29 * t35 * 2.0 -
               t4 * t32 * t35 * t106 * t147 * 2.0 +
               t3 * t4 * t10 * t29 * t35 * t108 * 2.0 -
               t2 * t4 * t5 * t10 * t35 * t37 * t106 * 2.0 -
               t2 * t4 * t5 * t10 * t29 * t38 * t112 * 2.0 -
               t3 * t4 * t10 * t20 * t35 * t37 * t106 * 2.0 -
               t3 * t4 * t10 * t20 * t29 * t38 * t112 * 2.0 -
               t4 * t6 * t10 * t29 * t32 * t35 * t106 * 2.0;
  hess(2, 8) =
      t156 + t157 - t165 - t166 -
      t21 * t35 * (t164 - t10 * t29 * t83 + t8 * t10 * t37 * t117) * 2.0 +
      t35 * t119 * t147 * 2.0 - t21 * t38 * t123 * t147 * 2.0 -
      t2 * t4 * t5 * t10 * t35 * t37 * t117 * 2.0 -
      t2 * t4 * t5 * t10 * t29 * t38 * t123 * 2.0 -
      t3 * t4 * t10 * t20 * t35 * t37 * t117 * 2.0 -
      t3 * t4 * t10 * t20 * t29 * t38 * t123 * 2.0 -
      t4 * t6 * t10 * t29 * t32 * t35 * t117 * 2.0 -
      t4 * t7 * t10 * t29 * t32 * t35 * t117 * 2.0;
  hess(3, 0) =
      t248 + t249 +
      t5 * t35 * (t244 - t2 * t10 * t32 + t6 * t36 * t37 * t170) * 2.0 +
      t34 * t35 * t172 * 2.0 + t5 * t38 * t42 * t172 * 2.0 +
      t2 * t3 * t20 * t35 * t36 * t37 * t170 * 2.0 +
      t2 * t4 * t21 * t35 * t36 * t37 * t170 * 2.0 +
      t2 * t7 * t29 * t32 * t35 * t36 * t170 * 2.0 +
      t2 * t8 * t29 * t32 * t35 * t36 * t170 * 2.0 +
      t3 * t10 * t20 * t29 * t38 * t42 * t170 * 2.0 +
      t4 * t10 * t21 * t29 * t38 * t42 * t170 * 2.0;
  hess(3, 1) =
      t207 + t252 +
      t5 * t35 * (t250 - t3 * t10 * t32 + t2 * t3 * t36 * t37 * t170) * 2.0 +
      t5 * t38 * t48 * t172 * 2.0 + t2 * t3 * t10 * t32 * t35 * t172 * 2.0 +
      t3 * t10 * t29 * t35 * t44 * t170 * 2.0 +
      t7 * t20 * t35 * t36 * t37 * t170 * 2.0 +
      t3 * t4 * t21 * t35 * t36 * t37 * t170 * 2.0 +
      t3 * t8 * t29 * t32 * t35 * t36 * t170 * 2.0 +
      t3 * t10 * t20 * t29 * t38 * t48 * t170 * 2.0 +
      t4 * t10 * t21 * t29 * t38 * t48 * t170 * 2.0;
  hess(3, 2) =
      t227 + t254 +
      t5 * t35 * (t253 - t4 * t10 * t32 + t2 * t4 * t36 * t37 * t170) * 2.0 +
      t5 * t38 * t54 * t172 * 2.0 + t2 * t4 * t10 * t32 * t35 * t172 * 2.0 +
      t4 * t10 * t29 * t35 * t50 * t170 * 2.0 +
      t8 * t21 * t35 * t36 * t37 * t170 * 2.0 +
      t3 * t4 * t20 * t35 * t36 * t37 * t170 * 2.0 +
      t4 * t7 * t29 * t32 * t35 * t36 * t170 * 2.0 +
      t3 * t10 * t20 * t29 * t38 * t54 * t170 * 2.0 +
      t4 * t10 * t21 * t29 * t38 * t54 * t170 * 2.0;
  hess(3, 3) = t191 + t192 + t35 * t63 * t172 * 2.0 +
               t5 * t35 *
                   (t186 - t32 * t61 - t10 * t29 * t170 +
                    t2 * t10 * t29 * t179 + t2 * t10 * t37 * t61 * t170) *
                   2.0 +
               t5 * t38 * t67 * t172 * 2.0 +
               t3 * t10 * t20 * t29 * t35 * t179 * 2.0 +
               t4 * t10 * t21 * t29 * t35 * t179 * 2.0 +
               t3 * t10 * t20 * t35 * t37 * t61 * t170 * 2.0 +
               t3 * t10 * t20 * t29 * t38 * t67 * t170 * 2.0 +
               t4 * t10 * t21 * t35 * t37 * t61 * t170 * 2.0 +
               t4 * t10 * t21 * t29 * t38 * t67 * t170 * 2.0 +
               t7 * t10 * t29 * t32 * t35 * t61 * t170 * 2.0 +
               t8 * t10 * t29 * t32 * t35 * t61 * t170 * 2.0;
  hess(3, 4) = t194 + t195 +
               t5 * t35 *
                   (t193 - t32 * t74 + t6 * t10 * t29 * t182 +
                    t2 * t10 * t37 * t74 * t170) *
                   2.0 +
               t5 * t38 * t80 * t172 * 2.0 -
               t10 * t20 * t29 * t35 * t170 * 2.0 +
               t2 * t32 * t35 * t74 * t172 * 2.0 +
               t3 * t10 * t29 * t35 * t76 * t170 * 2.0 +
               t2 * t3 * t10 * t20 * t29 * t35 * t182 * 2.0 +
               t2 * t4 * t10 * t21 * t29 * t35 * t182 * 2.0 +
               t3 * t10 * t20 * t35 * t37 * t74 * t170 * 2.0 +
               t3 * t10 * t20 * t29 * t38 * t80 * t170 * 2.0 +
               t4 * t10 * t21 * t35 * t37 * t74 * t170 * 2.0 +
               t4 * t10 * t21 * t29 * t38 * t80 * t170 * 2.0 +
               t8 * t10 * t29 * t32 * t35 * t74 * t170 * 2.0;
  hess(3, 5) = t199 + t200 +
               t5 * t35 *
                   (t198 - t32 * t87 + t6 * t10 * t29 * t185 +
                    t2 * t10 * t37 * t87 * t170) *
                   2.0 +
               t5 * t38 * t93 * t172 * 2.0 -
               t10 * t21 * t29 * t35 * t170 * 2.0 +
               t2 * t32 * t35 * t87 * t172 * 2.0 +
               t4 * t10 * t29 * t35 * t89 * t170 * 2.0 +
               t2 * t3 * t10 * t20 * t29 * t35 * t185 * 2.0 +
               t2 * t4 * t10 * t21 * t29 * t35 * t185 * 2.0 +
               t3 * t10 * t20 * t35 * t37 * t87 * t170 * 2.0 +
               t3 * t10 * t20 * t29 * t38 * t93 * t170 * 2.0 +
               t4 * t10 * t21 * t35 * t37 * t87 * t170 * 2.0 +
               t4 * t10 * t21 * t29 * t38 * t93 * t170 * 2.0 +
               t7 * t10 * t29 * t32 * t35 * t87 * t170 * 2.0;
  hess(3, 6) = -t191 - t192 + t35 * t97 * t172 * 2.0 -
               t5 * t35 *
                   (t186 - t32 * t95 - t10 * t29 * t170 +
                    t2 * t10 * t29 * t189 + t2 * t10 * t37 * t95 * t170) *
                   2.0 -
               t5 * t38 * t101 * t172 * 2.0 -
               t3 * t10 * t20 * t29 * t35 * t189 * 2.0 -
               t4 * t10 * t21 * t29 * t35 * t189 * 2.0 -
               t3 * t10 * t20 * t35 * t37 * t95 * t170 * 2.0 -
               t3 * t10 * t20 * t29 * t38 * t101 * t170 * 2.0 -
               t4 * t10 * t21 * t35 * t37 * t95 * t170 * 2.0 -
               t4 * t10 * t21 * t29 * t38 * t101 * t170 * 2.0 -
               t7 * t10 * t29 * t32 * t35 * t95 * t170 * 2.0 -
               t8 * t10 * t29 * t32 * t35 * t95 * t170 * 2.0;
  hess(3, 7) = -t194 - t195 -
               t5 * t35 *
                   (t193 - t32 * t106 + t6 * t10 * t29 * t196 +
                    t2 * t10 * t37 * t106 * t170) *
                   2.0 -
               t5 * t38 * t112 * t172 * 2.0 +
               t10 * t20 * t29 * t35 * t170 * 2.0 -
               t2 * t32 * t35 * t106 * t172 * 2.0 +
               t3 * t10 * t29 * t35 * t108 * t170 * 2.0 -
               t2 * t3 * t10 * t20 * t29 * t35 * t196 * 2.0 -
               t2 * t4 * t10 * t21 * t29 * t35 * t196 * 2.0 -
               t3 * t10 * t20 * t35 * t37 * t106 * t170 * 2.0 -
               t3 * t10 * t20 * t29 * t38 * t112 * t170 * 2.0 -
               t4 * t10 * t21 * t35 * t37 * t106 * t170 * 2.0 -
               t4 * t10 * t21 * t29 * t38 * t112 * t170 * 2.0 -
               t8 * t10 * t29 * t32 * t35 * t106 * t170 * 2.0;
  hess(3, 8) = -t199 - t200 -
               t5 * t35 *
                   (t198 - t32 * t117 + t6 * t10 * t29 * t201 +
                    t2 * t10 * t37 * t117 * t170) *
                   2.0 -
               t5 * t38 * t123 * t172 * 2.0 +
               t10 * t21 * t29 * t35 * t170 * 2.0 -
               t2 * t32 * t35 * t117 * t172 * 2.0 +
               t4 * t10 * t29 * t35 * t119 * t170 * 2.0 -
               t2 * t3 * t10 * t20 * t29 * t35 * t201 * 2.0 -
               t2 * t4 * t10 * t21 * t29 * t35 * t201 * 2.0 -
               t3 * t10 * t20 * t35 * t37 * t117 * t170 * 2.0 -
               t3 * t10 * t20 * t29 * t38 * t123 * t170 * 2.0 -
               t4 * t10 * t21 * t35 * t37 * t117 * t170 * 2.0 -
               t4 * t10 * t21 * t29 * t38 * t123 * t170 * 2.0 -
               t7 * t10 * t29 * t32 * t35 * t117 * t170 * 2.0;
  hess(4, 0) =
      t207 + t274 +
      t20 * t35 * (t271 - t2 * t10 * t32 + t2 * t3 * t36 * t37 * t204) * 2.0 +
      t20 * t38 * t42 * t206 * 2.0 + t2 * t3 * t10 * t32 * t35 * t206 * 2.0 +
      t2 * t10 * t29 * t34 * t35 * t204 * 2.0 +
      t5 * t6 * t35 * t36 * t37 * t204 * 2.0 +
      t2 * t5 * t10 * t29 * t38 * t42 * t204 * 2.0 +
      t2 * t4 * t21 * t35 * t36 * t37 * t204 * 2.0 +
      t2 * t8 * t29 * t32 * t35 * t36 * t204 * 2.0 +
      t4 * t10 * t21 * t29 * t38 * t42 * t204 * 2.0;
  hess(4, 1) =
      t278 + t279 +
      t20 * t35 * (t277 - t3 * t10 * t32 + t7 * t36 * t37 * t204) * 2.0 +
      t35 * t44 * t206 * 2.0 + t20 * t38 * t48 * t206 * 2.0 +
      t2 * t3 * t5 * t35 * t36 * t37 * t204 * 2.0 +
      t2 * t5 * t10 * t29 * t38 * t48 * t204 * 2.0 +
      t3 * t4 * t21 * t35 * t36 * t37 * t204 * 2.0 +
      t3 * t6 * t29 * t32 * t35 * t36 * t204 * 2.0 +
      t3 * t8 * t29 * t32 * t35 * t36 * t204 * 2.0 +
      t4 * t10 * t21 * t29 * t38 * t48 * t204 * 2.0;
  hess(4, 2) =
      t228 + t281 +
      t20 * t35 * (t280 - t4 * t10 * t32 + t3 * t4 * t36 * t37 * t204) * 2.0 +
      t20 * t38 * t54 * t206 * 2.0 + t3 * t4 * t10 * t32 * t35 * t206 * 2.0 +
      t4 * t10 * t29 * t35 * t50 * t204 * 2.0 +
      t8 * t21 * t35 * t36 * t37 * t204 * 2.0 +
      t2 * t4 * t5 * t35 * t36 * t37 * t204 * 2.0 +
      t2 * t5 * t10 * t29 * t38 * t54 * t204 * 2.0 +
      t4 * t6 * t29 * t32 * t35 * t36 * t204 * 2.0 +
      t4 * t10 * t21 * t29 * t38 * t54 * t204 * 2.0;
  hess(4, 3) = t213 + t214 +
               t20 * t35 *
                   (t212 - t32 * t61 + t7 * t10 * t29 * t177 +
                    t3 * t10 * t37 * t61 * t204) *
                   2.0 +
               t20 * t38 * t67 * t206 * 2.0 -
               t5 * t10 * t29 * t35 * t204 * 2.0 +
               t3 * t32 * t35 * t61 * t206 * 2.0 +
               t2 * t10 * t29 * t35 * t63 * t204 * 2.0 +
               t2 * t3 * t5 * t10 * t29 * t35 * t177 * 2.0 +
               t3 * t4 * t10 * t21 * t29 * t35 * t177 * 2.0 +
               t2 * t5 * t10 * t35 * t37 * t61 * t204 * 2.0 +
               t2 * t5 * t10 * t29 * t38 * t67 * t204 * 2.0 +
               t4 * t10 * t21 * t35 * t37 * t61 * t204 * 2.0 +
               t4 * t10 * t21 * t29 * t38 * t67 * t204 * 2.0 +
               t8 * t10 * t29 * t32 * t35 * t61 * t204 * 2.0;
  hess(4, 4) = t218 + t219 + t35 * t76 * t206 * 2.0 +
               t20 * t35 *
                   (t215 - t32 * t74 - t10 * t29 * t204 +
                    t3 * t10 * t29 * t211 + t3 * t10 * t37 * t74 * t204) *
                   2.0 +
               t20 * t38 * t80 * t206 * 2.0 +
               t2 * t5 * t10 * t29 * t35 * t211 * 2.0 +
               t4 * t10 * t21 * t29 * t35 * t211 * 2.0 +
               t2 * t5 * t10 * t35 * t37 * t74 * t204 * 2.0 +
               t2 * t5 * t10 * t29 * t38 * t80 * t204 * 2.0 +
               t4 * t10 * t21 * t35 * t37 * t74 * t204 * 2.0 +
               t4 * t10 * t21 * t29 * t38 * t80 * t204 * 2.0 +
               t6 * t10 * t29 * t32 * t35 * t74 * t204 * 2.0 +
               t8 * t10 * t29 * t32 * t35 * t74 * t204 * 2.0;
  hess(4, 5) = t221 + t222 +
               t20 * t35 *
                   (t220 - t32 * t87 + t7 * t10 * t29 * t185 +
                    t3 * t10 * t37 * t87 * t204) *
                   2.0 +
               t20 * t38 * t93 * t206 * 2.0 -
               t10 * t21 * t29 * t35 * t204 * 2.0 +
               t3 * t32 * t35 * t87 * t206 * 2.0 +
               t4 * t10 * t29 * t35 * t89 * t204 * 2.0 +
               t2 * t3 * t5 * t10 * t29 * t35 * t185 * 2.0 +
               t3 * t4 * t10 * t21 * t29 * t35 * t185 * 2.0 +
               t2 * t5 * t10 * t35 * t37 * t87 * t204 * 2.0 +
               t2 * t5 * t10 * t29 * t38 * t93 * t204 * 2.0 +
               t4 * t10 * t21 * t35 * t37 * t87 * t204 * 2.0 +
               t4 * t10 * t21 * t29 * t38 * t93 * t204 * 2.0 +
               t6 * t10 * t29 * t32 * t35 * t87 * t204 * 2.0;
  hess(4, 6) = -t213 - t214 -
               t20 * t35 *
                   (t212 - t32 * t95 + t7 * t10 * t29 * t187 +
                    t3 * t10 * t37 * t95 * t204) *
                   2.0 -
               t20 * t38 * t101 * t206 * 2.0 +
               t5 * t10 * t29 * t35 * t204 * 2.0 -
               t3 * t32 * t35 * t95 * t206 * 2.0 +
               t2 * t10 * t29 * t35 * t97 * t204 * 2.0 -
               t2 * t3 * t5 * t10 * t29 * t35 * t187 * 2.0 -
               t3 * t4 * t10 * t21 * t29 * t35 * t187 * 2.0 -
               t2 * t5 * t10 * t35 * t37 * t95 * t204 * 2.0 -
               t2 * t5 * t10 * t29 * t38 * t101 * t204 * 2.0 -
               t4 * t10 * t21 * t35 * t37 * t95 * t204 * 2.0 -
               t4 * t10 * t21 * t29 * t38 * t101 * t204 * 2.0 -
               t8 * t10 * t29 * t32 * t35 * t95 * t204 * 2.0;
  hess(4, 7) = -t218 - t219 + t35 * t108 * t206 * 2.0 -
               t20 * t35 *
                   (t215 - t32 * t106 - t10 * t29 * t204 +
                    t3 * t10 * t29 * t217 + t3 * t10 * t37 * t106 * t204) *
                   2.0 -
               t20 * t38 * t112 * t206 * 2.0 -
               t2 * t5 * t10 * t29 * t35 * t217 * 2.0 -
               t4 * t10 * t21 * t29 * t35 * t217 * 2.0 -
               t2 * t5 * t10 * t35 * t37 * t106 * t204 * 2.0 -
               t2 * t5 * t10 * t29 * t38 * t112 * t204 * 2.0 -
               t4 * t10 * t21 * t35 * t37 * t106 * t204 * 2.0 -
               t4 * t10 * t21 * t29 * t38 * t112 * t204 * 2.0 -
               t6 * t10 * t29 * t32 * t35 * t106 * t204 * 2.0 -
               t8 * t10 * t29 * t32 * t35 * t106 * t204 * 2.0;
  hess(4, 8) = -t221 - t222 -
               t20 * t35 *
                   (t220 - t32 * t117 + t7 * t10 * t29 * t201 +
                    t3 * t10 * t37 * t117 * t204) *
                   2.0 -
               t20 * t38 * t123 * t206 * 2.0 +
               t10 * t21 * t29 * t35 * t204 * 2.0 -
               t3 * t32 * t35 * t117 * t206 * 2.0 +
               t4 * t10 * t29 * t35 * t119 * t204 * 2.0 -
               t2 * t3 * t5 * t10 * t29 * t35 * t201 * 2.0 -
               t3 * t4 * t10 * t21 * t29 * t35 * t201 * 2.0 -
               t2 * t5 * t10 * t35 * t37 * t117 * t204 * 2.0 -
               t2 * t5 * t10 * t29 * t38 * t123 * t204 * 2.0 -
               t4 * t10 * t21 * t35 * t37 * t117 * t204 * 2.0 -
               t4 * t10 * t21 * t29 * t38 * t123 * t204 * 2.0 -
               t6 * t10 * t29 * t32 * t35 * t117 * t204 * 2.0;
  hess(5, 0) =
      t227 + t302 +
      t21 * t35 * (t299 - t2 * t10 * t32 + t2 * t4 * t36 * t37 * t224) * 2.0 +
      t21 * t38 * t42 * t226 * 2.0 + t2 * t4 * t10 * t32 * t35 * t226 * 2.0 +
      t2 * t10 * t29 * t34 * t35 * t224 * 2.0 +
      t5 * t6 * t35 * t36 * t37 * t224 * 2.0 +
      t2 * t5 * t10 * t29 * t38 * t42 * t224 * 2.0 +
      t2 * t3 * t20 * t35 * t36 * t37 * t224 * 2.0 +
      t2 * t7 * t29 * t32 * t35 * t36 * t224 * 2.0 +
      t3 * t10 * t20 * t29 * t38 * t42 * t224 * 2.0;
  hess(5, 1) =
      t228 + t306 +
      t21 * t35 * (t304 - t3 * t10 * t32 + t3 * t4 * t36 * t37 * t224) * 2.0 +
      t21 * t38 * t48 * t226 * 2.0 + t3 * t4 * t10 * t32 * t35 * t226 * 2.0 +
      t3 * t10 * t29 * t35 * t44 * t224 * 2.0 +
      t7 * t20 * t35 * t36 * t37 * t224 * 2.0 +
      t2 * t3 * t5 * t35 * t36 * t37 * t224 * 2.0 +
      t2 * t5 * t10 * t29 * t38 * t48 * t224 * 2.0 +
      t3 * t6 * t29 * t32 * t35 * t36 * t224 * 2.0 +
      t3 * t10 * t20 * t29 * t38 * t48 * t224 * 2.0;
  hess(5, 2) =
      t308 + t309 +
      t21 * t35 * (t307 - t4 * t10 * t32 + t8 * t36 * t37 * t224) * 2.0 +
      t35 * t50 * t226 * 2.0 + t21 * t38 * t54 * t226 * 2.0 +
      t2 * t4 * t5 * t35 * t36 * t37 * t224 * 2.0 +
      t3 * t4 * t20 * t35 * t36 * t37 * t224 * 2.0 +
      t2 * t5 * t10 * t29 * t38 * t54 * t224 * 2.0 +
      t4 * t6 * t29 * t32 * t35 * t36 * t224 * 2.0 +
      t4 * t7 * t29 * t32 * t35 * t36 * t224 * 2.0 +
      t3 * t10 * t20 * t29 * t38 * t54 * t224 * 2.0;
  hess(5, 3) = t234 + t235 +
               t21 * t35 *
                   (t233 - t32 * t61 + t8 * t10 * t29 * t177 +
                    t4 * t10 * t37 * t61 * t224) *
                   2.0 +
               t21 * t38 * t67 * t226 * 2.0 -
               t5 * t10 * t29 * t35 * t224 * 2.0 +
               t4 * t32 * t35 * t61 * t226 * 2.0 +
               t2 * t10 * t29 * t35 * t63 * t224 * 2.0 +
               t2 * t4 * t5 * t10 * t29 * t35 * t177 * 2.0 +
               t3 * t4 * t10 * t20 * t29 * t35 * t177 * 2.0 +
               t2 * t5 * t10 * t35 * t37 * t61 * t224 * 2.0 +
               t2 * t5 * t10 * t29 * t38 * t67 * t224 * 2.0 +
               t3 * t10 * t20 * t35 * t37 * t61 * t224 * 2.0 +
               t3 * t10 * t20 * t29 * t38 * t67 * t224 * 2.0 +
               t7 * t10 * t29 * t32 * t35 * t61 * t224 * 2.0;
  hess(5, 4) = t237 + t238 +
               t21 * t35 *
                   (t236 - t32 * t74 + t8 * t10 * t29 * t182 +
                    t4 * t10 * t37 * t74 * t224) *
                   2.0 +
               t21 * t38 * t80 * t226 * 2.0 -
               t10 * t20 * t29 * t35 * t224 * 2.0 +
               t4 * t32 * t35 * t74 * t226 * 2.0 +
               t3 * t10 * t29 * t35 * t76 * t224 * 2.0 +
               t2 * t4 * t5 * t10 * t29 * t35 * t182 * 2.0 +
               t3 * t4 * t10 * t20 * t29 * t35 * t182 * 2.0 +
               t2 * t5 * t10 * t35 * t37 * t74 * t224 * 2.0 +
               t2 * t5 * t10 * t29 * t38 * t80 * t224 * 2.0 +
               t3 * t10 * t20 * t35 * t37 * t74 * t224 * 2.0 +
               t3 * t10 * t20 * t29 * t38 * t80 * t224 * 2.0 +
               t6 * t10 * t29 * t32 * t35 * t74 * t224 * 2.0;
  hess(5, 5) = t242 + t243 + t35 * t89 * t226 * 2.0 +
               t21 * t35 *
                   (t239 - t32 * t87 - t10 * t29 * t224 +
                    t4 * t10 * t29 * t232 + t4 * t10 * t37 * t87 * t224) *
                   2.0 +
               t21 * t38 * t93 * t226 * 2.0 +
               t2 * t5 * t10 * t29 * t35 * t232 * 2.0 +
               t3 * t10 * t20 * t29 * t35 * t232 * 2.0 +
               t2 * t5 * t10 * t35 * t37 * t87 * t224 * 2.0 +
               t2 * t5 * t10 * t29 * t38 * t93 * t224 * 2.0 +
               t3 * t10 * t20 * t35 * t37 * t87 * t224 * 2.0 +
               t3 * t10 * t20 * t29 * t38 * t93 * t224 * 2.0 +
               t6 * t10 * t29 * t32 * t35 * t87 * t224 * 2.0 +
               t7 * t10 * t29 * t32 * t35 * t87 * t224 * 2.0;
  hess(5, 6) = -t234 - t235 -
               t21 * t35 *
                   (t233 - t32 * t95 + t8 * t10 * t29 * t187 +
                    t4 * t10 * t37 * t95 * t224) *
                   2.0 -
               t21 * t38 * t101 * t226 * 2.0 +
               t5 * t10 * t29 * t35 * t224 * 2.0 -
               t4 * t32 * t35 * t95 * t226 * 2.0 +
               t2 * t10 * t29 * t35 * t97 * t224 * 2.0 -
               t2 * t4 * t5 * t10 * t29 * t35 * t187 * 2.0 -
               t3 * t4 * t10 * t20 * t29 * t35 * t187 * 2.0 -
               t2 * t5 * t10 * t35 * t37 * t95 * t224 * 2.0 -
               t2 * t5 * t10 * t29 * t38 * t101 * t224 * 2.0 -
               t3 * t10 * t20 * t35 * t37 * t95 * t224 * 2.0 -
               t3 * t10 * t20 * t29 * t38 * t101 * t224 * 2.0 -
               t7 * t10 * t29 * t32 * t35 * t95 * t224 * 2.0;
  hess(5, 7) = -t237 - t238 -
               t21 * t35 *
                   (t236 - t32 * t106 + t8 * t10 * t29 * t196 +
                    t4 * t10 * t37 * t106 * t224) *
                   2.0 -
               t21 * t38 * t112 * t226 * 2.0 +
               t10 * t20 * t29 * t35 * t224 * 2.0 -
               t4 * t32 * t35 * t106 * t226 * 2.0 +
               t3 * t10 * t29 * t35 * t108 * t224 * 2.0 -
               t2 * t4 * t5 * t10 * t29 * t35 * t196 * 2.0 -
               t3 * t4 * t10 * t20 * t29 * t35 * t196 * 2.0 -
               t2 * t5 * t10 * t35 * t37 * t106 * t224 * 2.0 -
               t2 * t5 * t10 * t29 * t38 * t112 * t224 * 2.0 -
               t3 * t10 * t20 * t35 * t37 * t106 * t224 * 2.0 -
               t3 * t10 * t20 * t29 * t38 * t112 * t224 * 2.0 -
               t6 * t10 * t29 * t32 * t35 * t106 * t224 * 2.0;
  hess(5, 8) = -t242 - t243 + t35 * t119 * t226 * 2.0 -
               t21 * t35 *
                   (t239 - t32 * t117 - t10 * t29 * t224 +
                    t4 * t10 * t29 * t241 + t4 * t10 * t37 * t117 * t224) *
                   2.0 -
               t21 * t38 * t123 * t226 * 2.0 -
               t2 * t5 * t10 * t29 * t35 * t241 * 2.0 -
               t3 * t10 * t20 * t29 * t35 * t241 * 2.0 -
               t2 * t5 * t10 * t35 * t37 * t117 * t224 * 2.0 -
               t2 * t5 * t10 * t29 * t38 * t123 * t224 * 2.0 -
               t3 * t10 * t20 * t35 * t37 * t117 * t224 * 2.0 -
               t3 * t10 * t20 * t29 * t38 * t123 * t224 * 2.0 -
               t6 * t10 * t29 * t32 * t35 * t117 * t224 * 2.0 -
               t7 * t10 * t29 * t32 * t35 * t117 * t224 * 2.0;
  hess(6, 0) =
      -t248 - t249 -
      t5 * t35 * (t244 - t2 * t10 * t32 + t6 * t36 * t37 * t246) * 2.0 +
      t34 * t35 * t247 * 2.0 + t5 * t38 * t42 * t247 * 2.0 -
      t2 * t3 * t20 * t35 * t36 * t37 * t246 * 2.0 -
      t2 * t4 * t21 * t35 * t36 * t37 * t246 * 2.0 -
      t2 * t7 * t29 * t32 * t35 * t36 * t246 * 2.0 -
      t2 * t8 * t29 * t32 * t35 * t36 * t246 * 2.0 -
      t3 * t10 * t20 * t29 * t38 * t42 * t246 * 2.0 -
      t4 * t10 * t21 * t29 * t38 * t42 * t246 * 2.0;
  hess(6, 1) =
      -t207 - t252 -
      t5 * t35 * (t250 - t3 * t10 * t32 + t2 * t3 * t36 * t37 * t246) * 2.0 +
      t5 * t38 * t48 * t247 * 2.0 + t2 * t3 * t10 * t32 * t35 * t247 * 2.0 -
      t3 * t10 * t29 * t35 * t44 * t246 * 2.0 -
      t7 * t20 * t35 * t36 * t37 * t246 * 2.0 -
      t3 * t4 * t21 * t35 * t36 * t37 * t246 * 2.0 -
      t3 * t8 * t29 * t32 * t35 * t36 * t246 * 2.0 -
      t3 * t10 * t20 * t29 * t38 * t48 * t246 * 2.0 -
      t4 * t10 * t21 * t29 * t38 * t48 * t246 * 2.0;
  hess(6, 2) =
      -t227 - t254 -
      t5 * t35 * (t253 - t4 * t10 * t32 + t2 * t4 * t36 * t37 * t246) * 2.0 +
      t5 * t38 * t54 * t247 * 2.0 + t2 * t4 * t10 * t32 * t35 * t247 * 2.0 -
      t4 * t10 * t29 * t35 * t50 * t246 * 2.0 -
      t8 * t21 * t35 * t36 * t37 * t246 * 2.0 -
      t3 * t4 * t20 * t35 * t36 * t37 * t246 * 2.0 -
      t4 * t7 * t29 * t32 * t35 * t36 * t246 * 2.0 -
      t3 * t10 * t20 * t29 * t38 * t54 * t246 * 2.0 -
      t4 * t10 * t21 * t29 * t38 * t54 * t246 * 2.0;
  hess(6, 3) = t35 * t63 * t247 * 2.0 -
               t5 * t35 *
                   (t264 - t32 * t61 - t10 * t29 * t246 +
                    t2 * t10 * t29 * t257 + t2 * t10 * t37 * t61 * t246) *
                   2.0 +
               t5 * t38 * t67 * t247 * 2.0 -
               t3 * t10 * t20 * t29 * t35 * t257 * 2.0 -
               t4 * t10 * t21 * t29 * t35 * t257 * 2.0 -
               t3 * t10 * t20 * t35 * t37 * t61 * t246 * 2.0 -
               t3 * t10 * t20 * t29 * t38 * t67 * t246 * 2.0 -
               t4 * t10 * t21 * t35 * t37 * t61 * t246 * 2.0 -
               t4 * t10 * t21 * t29 * t38 * t67 * t246 * 2.0 -
               t7 * t10 * t29 * t32 * t35 * t61 * t246 * 2.0 -
               t8 * t10 * t29 * t32 * t35 * t61 * t246 * 2.0 -
               t3 * t20 * t29 * t35 * t36 * t57 * t246 * 2.0 -
               t4 * t21 * t29 * t35 * t36 * t57 * t246 * 2.0;
  hess(6, 4) = t268 -
               t5 * t35 *
                   (t267 - t32 * t74 + t2 * t10 * t29 * t260 +
                    t2 * t10 * t37 * t74 * t246) *
                   2.0 +
               t5 * t38 * t80 * t247 * 2.0 + t2 * t32 * t35 * t74 * t247 * 2.0 -
               t3 * t10 * t20 * t29 * t35 * t260 * 2.0 -
               t4 * t10 * t21 * t29 * t35 * t260 * 2.0 -
               t3 * t10 * t29 * t35 * t76 * t246 * 2.0 -
               t3 * t10 * t20 * t35 * t37 * t74 * t246 * 2.0 -
               t3 * t10 * t20 * t29 * t38 * t80 * t246 * 2.0 -
               t4 * t10 * t21 * t35 * t37 * t74 * t246 * 2.0 -
               t4 * t10 * t21 * t29 * t38 * t80 * t246 * 2.0 -
               t8 * t10 * t29 * t32 * t35 * t74 * t246 * 2.0 -
               t3 * t20 * t29 * t35 * t36 * t70 * t246 * 2.0 -
               t4 * t21 * t29 * t35 * t36 * t70 * t246 * 2.0;
  hess(6, 5) = t270 -
               t5 * t35 *
                   (t269 - t32 * t87 + t2 * t10 * t29 * t263 +
                    t2 * t10 * t37 * t87 * t246) *
                   2.0 +
               t5 * t38 * t93 * t247 * 2.0 + t2 * t32 * t35 * t87 * t247 * 2.0 -
               t3 * t10 * t20 * t29 * t35 * t263 * 2.0 -
               t4 * t10 * t21 * t29 * t35 * t263 * 2.0 -
               t4 * t10 * t29 * t35 * t89 * t246 * 2.0 -
               t3 * t10 * t20 * t35 * t37 * t87 * t246 * 2.0 -
               t3 * t10 * t20 * t29 * t38 * t93 * t246 * 2.0 -
               t4 * t10 * t21 * t35 * t37 * t87 * t246 * 2.0 -
               t4 * t10 * t21 * t29 * t38 * t93 * t246 * 2.0 -
               t7 * t10 * t29 * t32 * t35 * t87 * t246 * 2.0 -
               t3 * t20 * t29 * t35 * t36 * t83 * t246 * 2.0 -
               t4 * t21 * t29 * t35 * t36 * t83 * t246 * 2.0;
  hess(6, 6) = t5 * t35 *
                   (-t264 + t291 + t10 * t29 * t246 + t2 * t10 * t29 * t266 -
                    t2 * t10 * t37 * t95 * t246) *
                   -2.0 +
               t35 * t97 * t247 * 2.0 - t5 * t38 * t101 * t247 * 2.0 -
               t3 * t10 * t20 * t29 * t35 * t266 * 2.0 -
               t4 * t10 * t21 * t29 * t35 * t266 * 2.0 +
               t3 * t10 * t20 * t29 * t38 * t246 * (t99 + t100 - t102) * 2.0 +
               t4 * t10 * t21 * t29 * t38 * t246 * (t99 + t100 - t102) * 2.0 +
               t3 * t20 * t29 * t35 * t36 * t57 * t246 * 2.0 +
               t4 * t21 * t29 * t35 * t36 * t57 * t246 * 2.0 +
               t3 * t10 * t20 * t35 * t37 * t246 * (t60 - t96) * 2.0 +
               t4 * t10 * t21 * t35 * t37 * t246 * (t60 - t96) * 2.0 +
               t7 * t10 * t29 * t32 * t35 * t246 * (t60 - t96) * 2.0 +
               t8 * t10 * t29 * t32 * t35 * t246 * (t60 - t96) * 2.0;
  hess(6, 7) =
      -t268 +
      t5 * t35 *
          (t267 - t32 * t106 + t2 * t10 * t29 * (t259 - t2 * t10 * t13 * 2.0) +
           t2 * t10 * t37 * t246 * (t71 - t107)) *
          2.0 -
      t5 * t38 * t112 * t247 * 2.0 - t2 * t32 * t35 * t106 * t247 * 2.0 -
      t3 * t10 * t29 * t35 * t108 * t246 * 2.0 +
      t3 * t10 * t20 * t29 * t35 * (t259 - t2 * t10 * t13 * 2.0) * 2.0 +
      t4 * t10 * t21 * t29 * t35 * (t259 - t2 * t10 * t13 * 2.0) * 2.0 +
      t3 * t10 * t20 * t29 * t38 * t246 * (t110 + t111 - t113) * 2.0 +
      t4 * t10 * t21 * t29 * t38 * t246 * (t110 + t111 - t113) * 2.0 +
      t3 * t20 * t29 * t35 * t36 * t70 * t246 * 2.0 +
      t4 * t21 * t29 * t35 * t36 * t70 * t246 * 2.0 +
      t3 * t10 * t20 * t35 * t37 * t246 * (t71 - t107) * 2.0 +
      t4 * t10 * t21 * t35 * t37 * t246 * (t71 - t107) * 2.0 +
      t8 * t10 * t29 * t32 * t35 * t246 * (t71 - t107) * 2.0;
  hess(6, 8) =
      -t270 +
      t5 * t35 *
          (t269 - t32 * t117 + t2 * t10 * t29 * (t262 - t2 * t10 * t15 * 2.0) +
           t2 * t10 * t37 * t246 * (t84 - t118)) *
          2.0 -
      t5 * t38 * t123 * t247 * 2.0 - t2 * t32 * t35 * t117 * t247 * 2.0 -
      t4 * t10 * t29 * t35 * t119 * t246 * 2.0 +
      t3 * t10 * t20 * t29 * t35 * (t262 - t2 * t10 * t15 * 2.0) * 2.0 +
      t4 * t10 * t21 * t29 * t35 * (t262 - t2 * t10 * t15 * 2.0) * 2.0 +
      t3 * t10 * t20 * t29 * t38 * t246 * (t121 + t122 - t124) * 2.0 +
      t4 * t10 * t21 * t29 * t38 * t246 * (t121 + t122 - t124) * 2.0 +
      t3 * t20 * t29 * t35 * t36 * t83 * t246 * 2.0 +
      t4 * t21 * t29 * t35 * t36 * t83 * t246 * 2.0 +
      t3 * t10 * t20 * t35 * t37 * t246 * (t84 - t118) * 2.0 +
      t4 * t10 * t21 * t35 * t37 * t246 * (t84 - t118) * 2.0 +
      t7 * t10 * t29 * t32 * t35 * t246 * (t84 - t118) * 2.0;
  hess(7, 0) =
      -t207 - t274 -
      t20 * t35 * (t271 - t2 * t10 * t32 + t2 * t3 * t36 * t37 * t273) * 2.0 +
      t20 * t38 * t42 * t275 * 2.0 + t2 * t3 * t10 * t32 * t35 * t275 * 2.0 -
      t2 * t10 * t29 * t34 * t35 * t273 * 2.0 -
      t5 * t6 * t35 * t36 * t37 * t273 * 2.0 -
      t2 * t5 * t10 * t29 * t38 * t42 * t273 * 2.0 -
      t2 * t4 * t21 * t35 * t36 * t37 * t273 * 2.0 -
      t2 * t8 * t29 * t32 * t35 * t36 * t273 * 2.0 -
      t4 * t10 * t21 * t29 * t38 * t42 * t273 * 2.0;
  hess(7, 1) =
      -t278 - t279 -
      t20 * t35 * (t277 - t3 * t10 * t32 + t7 * t36 * t37 * t273) * 2.0 +
      t35 * t44 * t275 * 2.0 + t20 * t38 * t48 * t275 * 2.0 -
      t2 * t3 * t5 * t35 * t36 * t37 * t273 * 2.0 -
      t2 * t5 * t10 * t29 * t38 * t48 * t273 * 2.0 -
      t3 * t4 * t21 * t35 * t36 * t37 * t273 * 2.0 -
      t3 * t6 * t29 * t32 * t35 * t36 * t273 * 2.0 -
      t3 * t8 * t29 * t32 * t35 * t36 * t273 * 2.0 -
      t4 * t10 * t21 * t29 * t38 * t48 * t273 * 2.0;
  hess(7, 2) =
      -t228 - t281 -
      t20 * t35 * (t280 - t4 * t10 * t32 + t3 * t4 * t36 * t37 * t273) * 2.0 +
      t20 * t38 * t54 * t275 * 2.0 + t3 * t4 * t10 * t32 * t35 * t275 * 2.0 -
      t4 * t10 * t29 * t35 * t50 * t273 * 2.0 -
      t8 * t21 * t35 * t36 * t37 * t273 * 2.0 -
      t2 * t4 * t5 * t35 * t36 * t37 * t273 * 2.0 -
      t2 * t5 * t10 * t29 * t38 * t54 * t273 * 2.0 -
      t4 * t6 * t29 * t32 * t35 * t36 * t273 * 2.0 -
      t4 * t10 * t21 * t29 * t38 * t54 * t273 * 2.0;
  hess(7, 3) = t293 -
               t20 * t35 *
                   (t292 - t32 * t61 + t3 * t10 * t29 * t284 +
                    t3 * t10 * t37 * t61 * t273) *
                   2.0 +
               t20 * t38 * t67 * t275 * 2.0 +
               t3 * t32 * t35 * t61 * t275 * 2.0 -
               t2 * t5 * t10 * t29 * t35 * t284 * 2.0 -
               t4 * t10 * t21 * t29 * t35 * t284 * 2.0 -
               t2 * t10 * t29 * t35 * t63 * t273 * 2.0 -
               t2 * t5 * t10 * t35 * t37 * t61 * t273 * 2.0 -
               t2 * t5 * t10 * t29 * t38 * t67 * t273 * 2.0 -
               t2 * t5 * t29 * t35 * t36 * t57 * t273 * 2.0 -
               t4 * t10 * t21 * t35 * t37 * t61 * t273 * 2.0 -
               t4 * t10 * t21 * t29 * t38 * t67 * t273 * 2.0 -
               t8 * t10 * t29 * t32 * t35 * t61 * t273 * 2.0 -
               t4 * t21 * t29 * t35 * t36 * t57 * t273 * 2.0;
  hess(7, 4) = t35 * t76 * t275 * 2.0 -
               t20 * t35 *
                   (t294 - t32 * t74 - t10 * t29 * t273 +
                    t3 * t10 * t29 * t287 + t3 * t10 * t37 * t74 * t273) *
                   2.0 +
               t20 * t38 * t80 * t275 * 2.0 -
               t2 * t5 * t10 * t29 * t35 * t287 * 2.0 -
               t4 * t10 * t21 * t29 * t35 * t287 * 2.0 -
               t2 * t5 * t10 * t35 * t37 * t74 * t273 * 2.0 -
               t2 * t5 * t10 * t29 * t38 * t80 * t273 * 2.0 -
               t2 * t5 * t29 * t35 * t36 * t70 * t273 * 2.0 -
               t4 * t10 * t21 * t35 * t37 * t74 * t273 * 2.0 -
               t4 * t10 * t21 * t29 * t38 * t80 * t273 * 2.0 -
               t6 * t10 * t29 * t32 * t35 * t74 * t273 * 2.0 -
               t8 * t10 * t29 * t32 * t35 * t74 * t273 * 2.0 -
               t4 * t21 * t29 * t35 * t36 * t70 * t273 * 2.0;
  hess(7, 5) = t298 -
               t20 * t35 *
                   (t297 - t32 * t87 + t3 * t10 * t29 * t290 +
                    t3 * t10 * t37 * t87 * t273) *
                   2.0 +
               t20 * t38 * t93 * t275 * 2.0 +
               t3 * t32 * t35 * t87 * t275 * 2.0 -
               t2 * t5 * t10 * t29 * t35 * t290 * 2.0 -
               t4 * t10 * t21 * t29 * t35 * t290 * 2.0 -
               t4 * t10 * t29 * t35 * t89 * t273 * 2.0 -
               t2 * t5 * t10 * t35 * t37 * t87 * t273 * 2.0 -
               t2 * t5 * t10 * t29 * t38 * t93 * t273 * 2.0 -
               t2 * t5 * t29 * t35 * t36 * t83 * t273 * 2.0 -
               t4 * t10 * t21 * t35 * t37 * t87 * t273 * 2.0 -
               t4 * t10 * t21 * t29 * t38 * t93 * t273 * 2.0 -
               t6 * t10 * t29 * t32 * t35 * t87 * t273 * 2.0 -
               t4 * t21 * t29 * t35 * t36 * t83 * t273 * 2.0;
  hess(7, 6) =
      -t293 +
      t20 * t35 *
          (-t291 + t292 + t3 * t10 * t29 * (t283 - t3 * t10 * t11 * 2.0) +
           t3 * t10 * t37 * t273 * (t60 - t96)) *
          2.0 -
      t20 * t38 * t101 * t275 * 2.0 - t3 * t32 * t35 * t95 * t275 * 2.0 -
      t2 * t10 * t29 * t35 * t97 * t273 * 2.0 +
      t2 * t5 * t10 * t29 * t35 * (t283 - t3 * t10 * t11 * 2.0) * 2.0 +
      t4 * t10 * t21 * t29 * t35 * (t283 - t3 * t10 * t11 * 2.0) * 2.0 +
      t2 * t5 * t10 * t29 * t38 * t273 * (t99 + t100 - t102) * 2.0 +
      t4 * t10 * t21 * t29 * t38 * t273 * (t99 + t100 - t102) * 2.0 +
      t2 * t5 * t29 * t35 * t36 * t57 * t273 * 2.0 +
      t4 * t21 * t29 * t35 * t36 * t57 * t273 * 2.0 +
      t2 * t5 * t10 * t35 * t37 * t273 * (t60 - t96) * 2.0 +
      t4 * t10 * t21 * t35 * t37 * t273 * (t60 - t96) * 2.0 +
      t8 * t10 * t29 * t32 * t35 * t273 * (t60 - t96) * 2.0;
  hess(7, 7) = t20 * t35 *
                   (-t294 + t321 + t10 * t29 * t273 + t3 * t10 * t29 * t296 -
                    t3 * t10 * t37 * t106 * t273) *
                   -2.0 +
               t35 * t108 * t275 * 2.0 - t20 * t38 * t112 * t275 * 2.0 -
               t2 * t5 * t10 * t29 * t35 * t296 * 2.0 -
               t4 * t10 * t21 * t29 * t35 * t296 * 2.0 +
               t2 * t5 * t10 * t29 * t38 * t273 * (t110 + t111 - t113) * 2.0 +
               t4 * t10 * t21 * t29 * t38 * t273 * (t110 + t111 - t113) * 2.0 +
               t2 * t5 * t29 * t35 * t36 * t70 * t273 * 2.0 +
               t4 * t21 * t29 * t35 * t36 * t70 * t273 * 2.0 +
               t2 * t5 * t10 * t35 * t37 * t273 * (t71 - t107) * 2.0 +
               t4 * t10 * t21 * t35 * t37 * t273 * (t71 - t107) * 2.0 +
               t6 * t10 * t29 * t32 * t35 * t273 * (t71 - t107) * 2.0 +
               t8 * t10 * t29 * t32 * t35 * t273 * (t71 - t107) * 2.0;
  hess(7, 8) =
      -t298 +
      t20 * t35 *
          (t297 - t32 * t117 + t3 * t10 * t29 * (t289 - t3 * t10 * t15 * 2.0) +
           t3 * t10 * t37 * t273 * (t84 - t118)) *
          2.0 -
      t20 * t38 * t123 * t275 * 2.0 - t3 * t32 * t35 * t117 * t275 * 2.0 -
      t4 * t10 * t29 * t35 * t119 * t273 * 2.0 +
      t2 * t5 * t10 * t29 * t35 * (t289 - t3 * t10 * t15 * 2.0) * 2.0 +
      t4 * t10 * t21 * t29 * t35 * (t289 - t3 * t10 * t15 * 2.0) * 2.0 +
      t2 * t5 * t10 * t29 * t38 * t273 * (t121 + t122 - t124) * 2.0 +
      t4 * t10 * t21 * t29 * t38 * t273 * (t121 + t122 - t124) * 2.0 +
      t2 * t5 * t29 * t35 * t36 * t83 * t273 * 2.0 +
      t4 * t21 * t29 * t35 * t36 * t83 * t273 * 2.0 +
      t2 * t5 * t10 * t35 * t37 * t273 * (t84 - t118) * 2.0 +
      t4 * t10 * t21 * t35 * t37 * t273 * (t84 - t118) * 2.0 +
      t6 * t10 * t29 * t32 * t35 * t273 * (t84 - t118) * 2.0;
  hess(8, 0) =
      -t227 - t302 -
      t21 * t35 * (t299 - t2 * t10 * t32 + t2 * t4 * t36 * t37 * t301) * 2.0 +
      t21 * t38 * t42 * t303 * 2.0 + t2 * t4 * t10 * t32 * t35 * t303 * 2.0 -
      t2 * t10 * t29 * t34 * t35 * t301 * 2.0 -
      t5 * t6 * t35 * t36 * t37 * t301 * 2.0 -
      t2 * t5 * t10 * t29 * t38 * t42 * t301 * 2.0 -
      t2 * t3 * t20 * t35 * t36 * t37 * t301 * 2.0 -
      t2 * t7 * t29 * t32 * t35 * t36 * t301 * 2.0 -
      t3 * t10 * t20 * t29 * t38 * t42 * t301 * 2.0;
  hess(8, 1) =
      -t228 - t306 -
      t21 * t35 * (t304 - t3 * t10 * t32 + t3 * t4 * t36 * t37 * t301) * 2.0 +
      t21 * t38 * t48 * t303 * 2.0 + t3 * t4 * t10 * t32 * t35 * t303 * 2.0 -
      t3 * t10 * t29 * t35 * t44 * t301 * 2.0 -
      t7 * t20 * t35 * t36 * t37 * t301 * 2.0 -
      t2 * t3 * t5 * t35 * t36 * t37 * t301 * 2.0 -
      t2 * t5 * t10 * t29 * t38 * t48 * t301 * 2.0 -
      t3 * t6 * t29 * t32 * t35 * t36 * t301 * 2.0 -
      t3 * t10 * t20 * t29 * t38 * t48 * t301 * 2.0;
  hess(8, 2) =
      -t308 - t309 -
      t21 * t35 * (t307 - t4 * t10 * t32 + t8 * t36 * t37 * t301) * 2.0 +
      t35 * t50 * t303 * 2.0 + t21 * t38 * t54 * t303 * 2.0 -
      t2 * t4 * t5 * t35 * t36 * t37 * t301 * 2.0 -
      t3 * t4 * t20 * t35 * t36 * t37 * t301 * 2.0 -
      t2 * t5 * t10 * t29 * t38 * t54 * t301 * 2.0 -
      t4 * t6 * t29 * t32 * t35 * t36 * t301 * 2.0 -
      t4 * t7 * t29 * t32 * t35 * t36 * t301 * 2.0 -
      t3 * t10 * t20 * t29 * t38 * t54 * t301 * 2.0;
  hess(8, 3) = t320 -
               t21 * t35 *
                   (t319 - t32 * t61 + t4 * t10 * t29 * t312 +
                    t4 * t10 * t37 * t61 * t301) *
                   2.0 +
               t21 * t38 * t67 * t303 * 2.0 +
               t4 * t32 * t35 * t61 * t303 * 2.0 -
               t2 * t5 * t10 * t29 * t35 * t312 * 2.0 -
               t3 * t10 * t20 * t29 * t35 * t312 * 2.0 -
               t2 * t10 * t29 * t35 * t63 * t301 * 2.0 -
               t2 * t5 * t10 * t35 * t37 * t61 * t301 * 2.0 -
               t2 * t5 * t10 * t29 * t38 * t67 * t301 * 2.0 -
               t2 * t5 * t29 * t35 * t36 * t57 * t301 * 2.0 -
               t3 * t10 * t20 * t35 * t37 * t61 * t301 * 2.0 -
               t3 * t10 * t20 * t29 * t38 * t67 * t301 * 2.0 -
               t7 * t10 * t29 * t32 * t35 * t61 * t301 * 2.0 -
               t3 * t20 * t29 * t35 * t36 * t57 * t301 * 2.0;
  hess(8, 4) = t323 -
               t21 * t35 *
                   (t322 - t32 * t74 + t4 * t10 * t29 * t315 +
                    t4 * t10 * t37 * t74 * t301) *
                   2.0 +
               t21 * t38 * t80 * t303 * 2.0 +
               t4 * t32 * t35 * t74 * t303 * 2.0 -
               t2 * t5 * t10 * t29 * t35 * t315 * 2.0 -
               t3 * t10 * t20 * t29 * t35 * t315 * 2.0 -
               t3 * t10 * t29 * t35 * t76 * t301 * 2.0 -
               t2 * t5 * t10 * t35 * t37 * t74 * t301 * 2.0 -
               t2 * t5 * t10 * t29 * t38 * t80 * t301 * 2.0 -
               t2 * t5 * t29 * t35 * t36 * t70 * t301 * 2.0 -
               t3 * t10 * t20 * t35 * t37 * t74 * t301 * 2.0 -
               t3 * t10 * t20 * t29 * t38 * t80 * t301 * 2.0 -
               t6 * t10 * t29 * t32 * t35 * t74 * t301 * 2.0 -
               t3 * t20 * t29 * t35 * t36 * t70 * t301 * 2.0;
  hess(8, 5) = t35 * t89 * t303 * 2.0 -
               t21 * t35 *
                   (t324 - t32 * t87 - t10 * t29 * t301 +
                    t4 * t10 * t29 * t318 + t4 * t10 * t37 * t87 * t301) *
                   2.0 +
               t21 * t38 * t93 * t303 * 2.0 -
               t2 * t5 * t10 * t29 * t35 * t318 * 2.0 -
               t3 * t10 * t20 * t29 * t35 * t318 * 2.0 -
               t2 * t5 * t10 * t35 * t37 * t87 * t301 * 2.0 -
               t2 * t5 * t10 * t29 * t38 * t93 * t301 * 2.0 -
               t2 * t5 * t29 * t35 * t36 * t83 * t301 * 2.0 -
               t3 * t10 * t20 * t35 * t37 * t87 * t301 * 2.0 -
               t3 * t10 * t20 * t29 * t38 * t93 * t301 * 2.0 -
               t6 * t10 * t29 * t32 * t35 * t87 * t301 * 2.0 -
               t7 * t10 * t29 * t32 * t35 * t87 * t301 * 2.0 -
               t3 * t20 * t29 * t35 * t36 * t83 * t301 * 2.0;
  hess(8, 6) =
      -t320 +
      t21 * t35 *
          (-t291 + t319 + t4 * t10 * t29 * (t311 - t4 * t10 * t11 * 2.0) +
           t4 * t10 * t37 * t301 * (t60 - t96)) *
          2.0 -
      t21 * t38 * t101 * t303 * 2.0 - t4 * t32 * t35 * t95 * t303 * 2.0 -
      t2 * t10 * t29 * t35 * t97 * t301 * 2.0 +
      t2 * t5 * t10 * t29 * t35 * (t311 - t4 * t10 * t11 * 2.0) * 2.0 +
      t3 * t10 * t20 * t29 * t35 * (t311 - t4 * t10 * t11 * 2.0) * 2.0 +
      t2 * t5 * t10 * t29 * t38 * t301 * (t99 + t100 - t102) * 2.0 +
      t3 * t10 * t20 * t29 * t38 * t301 * (t99 + t100 - t102) * 2.0 +
      t2 * t5 * t29 * t35 * t36 * t57 * t301 * 2.0 +
      t3 * t20 * t29 * t35 * t36 * t57 * t301 * 2.0 +
      t2 * t5 * t10 * t35 * t37 * t301 * (t60 - t96) * 2.0 +
      t3 * t10 * t20 * t35 * t37 * t301 * (t60 - t96) * 2.0 +
      t7 * t10 * t29 * t32 * t35 * t301 * (t60 - t96) * 2.0;
  hess(8, 7) =
      -t323 +
      t21 * t35 *
          (-t321 + t322 + t4 * t10 * t29 * (t314 - t4 * t10 * t13 * 2.0) +
           t4 * t10 * t37 * t301 * (t71 - t107)) *
          2.0 -
      t21 * t38 * t112 * t303 * 2.0 - t4 * t32 * t35 * t106 * t303 * 2.0 -
      t3 * t10 * t29 * t35 * t108 * t301 * 2.0 +
      t2 * t5 * t10 * t29 * t35 * (t314 - t4 * t10 * t13 * 2.0) * 2.0 +
      t3 * t10 * t20 * t29 * t35 * (t314 - t4 * t10 * t13 * 2.0) * 2.0 +
      t2 * t5 * t10 * t29 * t38 * t301 * (t110 + t111 - t113) * 2.0 +
      t3 * t10 * t20 * t29 * t38 * t301 * (t110 + t111 - t113) * 2.0 +
      t2 * t5 * t29 * t35 * t36 * t70 * t301 * 2.0 +
      t3 * t20 * t29 * t35 * t36 * t70 * t301 * 2.0 +
      t2 * t5 * t10 * t35 * t37 * t301 * (t71 - t107) * 2.0 +
      t3 * t10 * t20 * t35 * t37 * t301 * (t71 - t107) * 2.0 +
      t6 * t10 * t29 * t32 * t35 * t301 * (t71 - t107) * 2.0;
  hess(8, 8) = t21 * t35 *
                   (-t324 + t32 * t117 + t10 * t29 * t301 +
                    t4 * t10 * t29 * t326 - t4 * t10 * t37 * t117 * t301) *
                   -2.0 +
               t35 * t119 * t303 * 2.0 - t21 * t38 * t123 * t303 * 2.0 -
               t2 * t5 * t10 * t29 * t35 * t326 * 2.0 -
               t3 * t10 * t20 * t29 * t35 * t326 * 2.0 +
               t2 * t5 * t10 * t29 * t38 * t301 * (t121 + t122 - t124) * 2.0 +
               t3 * t10 * t20 * t29 * t38 * t301 * (t121 + t122 - t124) * 2.0 +
               t2 * t5 * t29 * t35 * t36 * t83 * t301 * 2.0 +
               t3 * t20 * t29 * t35 * t36 * t83 * t301 * 2.0 +
               t2 * t5 * t10 * t35 * t37 * t301 * (t84 - t118) * 2.0 +
               t3 * t10 * t20 * t35 * t37 * t301 * (t84 - t118) * 2.0 +
               t6 * t10 * t29 * t32 * t35 * t301 * (t84 - t118) * 2.0 +
               t7 * t10 * t29 * t32 * t35 * t301 * (t84 - t118) * 2.0;
}

template <typename T, int DIM>
inline T softcapsule_phi(const Eigen::Matrix<T, DIM, 1>& p,
                         const Eigen::Matrix<T, DIM, 1>& a,
                         const Eigen::Matrix<T, DIM, 1>& b, const T& radius,
                         const T& h) {
  Eigen::Matrix<T, DIM, 1> pa = p - a;
  Eigen::Matrix<T, DIM, 1> ba = b - a;
...
template <typename T, int DIM>
inline T softcircle_phi(const Eigen::Matrix<T, DIM, 1>& position,
                        const Eigen::Matrix<T, DIM, 1>& centre, const T& radius,
                        const T& h) {
  return softsqrt((position - centre).squaredNorm(), h) - radius;
}

template <typename T, int DIM>
inline void gradsoftcircle_phi(Eigen::Matrix<T, DIM * 2, 1>& grad,
                               const Eigen::Matrix<T, DIM, 1>& position,
                               const Eigen::Matrix<T, DIM, 1>& centre,
                               const T& radius, const T& h) {
  grad.segment(0, DIM) = (position - centre) * 2.0 *
                         gradsoftsqrt((position - centre).squaredNorm(), h);
  grad.segment(DIM, DIM) = -grad.segment(0, DIM);
}

template <typename T, int DIM>
inline void hesssoftcircle_phi(Eigen::Matrix<T, DIM * 2, DIM * 2>& hess,
                               const Eigen::Matrix<T, DIM, 1>& position,
                               const Eigen::Matrix<T, DIM, 1>& centre,
                               const T& radius, const T& h) {
  ...
  Eigen::Matrix<T, DIM, DIM> M =
      2.0 * Eigen::Matrix<T, DIM, DIM>::Identity() * gradsoftsqrt(sndp, h) +
      dp * dp.transpose() * hesssoftsqrt(sndp, h) * 4.0;
...
inline T pointedgedist(const Eigen::Matrix<T, DIM, 1>& x1,
                       const Eigen::Matrix<T, DIM, 1>& x2,
                       const Eigen::Matrix<T, DIM, 1>& x3, T& alpha) {
  alpha = std::min((T)1.0, std::max((T)0.0, (x1 - x2).dot(x3 - x2) /
                                                (x3 - x2).dot(x3 - x2)));
  Eigen::Matrix<T, DIM, 1> closest = x2 + alpha * (x3 - x2);
  Eigen::Matrix<T, DIM, 1> n = closest - x1;
  return n.norm();
...
template <typename T, int DIM>
inline T softpointedgedist(const Eigen::Matrix<T, DIM, 1>& x1,
                           const Eigen::Matrix<T, DIM, 1>& x2,
                           const Eigen::Matrix<T, DIM, 1>& x3, T& alpha,
                           const T& h) {
  alpha = softclamp((x1 - x2).dot(x3 - x2) / (x3 - x2).dot(x3 - x2), (T)0.0,
                    (T)1.0, h);
  Eigen::Matrix<T, DIM, 1> closest = x2 + alpha * (x3 - x2);
  Eigen::Matrix<T, DIM, 1> n = closest - x1;
  return n.norm();
...
template <typename T>
inline void gradsoftpointedgedist(const Eigen::Matrix<T, 2, 1>& x0,
                                  const Eigen::Matrix<T, 2, 1>& x1,
                                  const Eigen::Matrix<T, 2, 1>& x2,
                                  Vector6s& gradE, const T& h) {
  ...
  gradE(0) +=
      t23 *
      (t16 * (t4 * t7 * t13 - 1.0) * 2.0 + t2 * t3 * t7 * t13 * t18 * 2.0) *
      (-1.0 / 2.0);
  gradE(1) +=
      t23 *
      (t18 * (t5 * t7 * t13 - 1.0) * 2.0 + t2 * t3 * t7 * t13 * t16 * 2.0) *
      (-1.0 / 2.0);
  gradE(2) +=
      t23 *
      (t16 * (-t15 + t2 * t13 * t29 + 1.0) * 2.0 + t3 * t13 * t18 * t29 * 2.0) *
      (-1.0 / 2.0);
  gradE(3) +=
      t23 *
      (t18 * (-t15 + t3 * t13 * t34 + 1.0) * 2.0 + t2 * t13 * t16 * t34 * 2.0) *
      (-1.0 / 2.0);
  gradE(4) +=
      t23 * (t16 * (t15 + t2 * t13 * t37) * 2.0 + t3 * t13 * t18 * t37 * 2.0) *
      (-1.0 / 2.0);
  gradE(5) +=
      t23 * (t18 * (t15 + t3 * t13 * t40) * 2.0 + t2 * t13 * t16 * t40 * 2.0) *
      (-1.0 / 2.0);
}

template <typename T>
inline void gradsoftpointedgedist(const Eigen::Matrix<T, 3, 1>& x0,
                                  const Eigen::Matrix<T, 3, 1>& x1,
                                  const Eigen::Matrix<T, 3, 1>& x2,
                                  Vector9s& grad, const T& h) {
  const scalar t2 = x1(0) - x2(0);
  const scalar t3 = x1(1) - x2(1);
  ...
  grad(0) =
      t30 *
      (t5 * (t6 * t10 * t23 - 1.0) * 2.0 + t2 * t3 * t10 * t20 * t23 * 2.0 +
       t2 * t4 * t10 * t21 * t23 * 2.0) *
      (-1.0 / 2.0);
  grad(1) = t30 *
            (t20 * (t7 * t10 * t23 - 1.0) * 2.0 +
             t2 * t3 * t5 * t10 * t23 * 2.0 + t3 * t4 * t10 * t21 * t23 * 2.0) *
            (-1.0 / 2.0);
  grad(2) = t30 *
            (t21 * (t8 * t10 * t23 - 1.0) * 2.0 +
             t2 * t4 * t5 * t10 * t23 * 2.0 + t3 * t4 * t10 * t20 * t23 * 2.0) *
            (-1.0 / 2.0);
  grad(3) = t30 *
            (t5 * (-t19 + t2 * t23 * t37 + 1.0) * 2.0 +
             t3 * t20 * t23 * t37 * 2.0 + t4 * t21 * t23 * t37 * 2.0) *
            (-1.0 / 2.0);
  grad(4) = t30 *
            (t20 * (-t19 + t3 * t23 * t44 + 1.0) * 2.0 +
             t2 * t5 * t23 * t44 * 2.0 + t4 * t21 * t23 * t44 * 2.0) *
            (-1.0 / 2.0);
  grad(5) = t30 *
            (t21 * (-t19 + t4 * t23 * t51 + 1.0) * 2.0 +
             t2 * t5 * t23 * t51 * 2.0 + t3 * t20 * t23 * t51 * 2.0) *
            (-1.0 / 2.0);
  grad(6) = t30 *
            (t5 * (t19 + t2 * t23 * t53) * 2.0 + t3 * t20 * t23 * t53 * 2.0 +
             t4 * t21 * t23 * t53 * 2.0) *
            (-1.0 / 2.0);
  grad(7) = t30 *
            (t20 * (t19 + t3 * t23 * t55) * 2.0 + t2 * t5 * t23 * t55 * 2.0 +
             t4 * t21 * t23 * t55 * 2.0) *
            (-1.0 / 2.0);
  grad(8) = t30 *
            (t21 * (t19 + t4 * t23 * t57) * 2.0 + t2 * t5 * t23 * t57 * 2.0 +
             t3 * t20 * t23 * t57 * 2.0) *
            (-1.0 / 2.0);
}

template <typename T>
inline void hesssoftpointedgedist(const Eigen::Matrix<T, 2, 1>& x0,
                                  const Eigen::Matrix<T, 2, 1>& x1,
                                  const Eigen::Matrix<T, 2, 1>& x2,
                                  Matrix6s& hess, const T& h) {
  const scalar t2 = x1(0) - x2(0);
  const scalar t3 = x1(1) - x2(1);
  ...
  hess(0, 0) = t28 *
                   (t75 + (t17 * t17) * 2.0 + t2 * t4 * t5 * t19 * t21 * 2.0 +
                    t3 * t5 * t16 * t19 * t21 * 2.0) *
                   (1.0 / 2.0) -
               (t24 * t24) * t33 * (1.0 / 4.0);
  hess(0, 1) = t74;
  hess(0, 2) =
      t28 *
          (t56 + t93 + t95 + t96 + t4 * (t53 + t92 - t8 * t18 * t35) * 2.0 -
           t3 * t8 * t16 * t18 * 2.0) *
          (1.0 / 2.0) -
      t24 * t33 * t78 * (1.0 / 4.0);
  hess(0, 3) =
      t28 *
          (t62 + t117 + t118 + t4 * (t59 + t5 * t8 * t21 * t49) * 2.0 -
           t2 * t8 * t16 * t18 * 2.0 + t2 * t3 * t8 * t16 * t21 * t49 * 2.0) *
          (1.0 / 2.0) -
      t24 * t33 * t81 * (1.0 / 4.0);
  hess(0, 4) = t28 *
                   (-t56 + t17 * t58 * 2.0 +
                    t4 * (-t53 + t8 * t18 * t35 + t5 * t8 * t21 * t54) * 2.0 +
                    t3 * t8 * t16 * t18 * 2.0 + t2 * t6 * t8 * t40 * t54 * 2.0 +
                    t2 * t3 * t8 * t16 * t21 * t54 * 2.0) *
                   (1.0 / 2.0) -
               t24 * t33 * t86 * (1.0 / 4.0);
  hess(0, 5) = t28 *
                   (-t62 + t166 - t4 * (t59 - t5 * t8 * t21 * t60) * 2.0 +
                    t2 * t8 * t16 * t18 * 2.0 + t2 * t3 * t8 * t18 * t64 * 2.0 +
                    t2 * t3 * t8 * t16 * t21 * t60 * 2.0) *
                   (1.0 / 2.0) -
               t24 * t33 * t91 * (1.0 / 4.0);
  hess(1, 0) = t74;
  hess(1, 1) = t28 *
                   (t75 + (t32 * t32) * 2.0 + t2 * t4 * t6 * t19 * t21 * 2.0 +
                    t3 * t6 * t16 * t19 * t21 * 2.0) *
                   (1.0 / 2.0) -
               t33 * (t73 * t73) * (1.0 / 4.0);
  hess(1, 2) =
      t28 *
          (t83 + t97 + t98 + t16 * (t82 + t6 * t8 * t21 * t39) * 2.0 -
           t3 * t4 * t8 * t18 * 2.0 + t2 * t3 * t4 * t8 * t21 * t39 * 2.0) *
          (1.0 / 2.0) -
      t33 * t73 * t78 * (1.0 / 4.0);
  hess(1, 3) = t28 *
                   (t88 + t120 + t122 + t123 +
                    t16 * (t87 + t119 - t8 * t18 * t45) * 2.0 -
                    t2 * t4 * t8 * t18 * 2.0) *
                   (1.0 / 2.0) -
               t33 * t73 * t81 * (1.0 / 4.0);
  hess(1, 4) = t28 *
                   (-t83 + t143 - t16 * (t82 - t6 * t8 * t21 * t54) * 2.0 +
                    t3 * t4 * t8 * t18 * 2.0 + t2 * t3 * t8 * t18 * t58 * 2.0 +
                    t2 * t3 * t4 * t8 * t21 * t54 * 2.0) *
                   (1.0 / 2.0) -
               t33 * t73 * t86 * (1.0 / 4.0);
  hess(1, 5) = t28 *
                   (-t88 + t32 * t64 * 2.0 +
                    t16 * (-t87 + t8 * t18 * t45 + t6 * t8 * t21 * t60) * 2.0 +
                    t2 * t4 * t8 * t18 * 2.0 + t3 * t5 * t8 * t40 * t60 * 2.0 +
                    t2 * t3 * t4 * t8 * t21 * t60 * 2.0) *
                   (1.0 / 2.0) -
               t33 * t73 * t91 * (1.0 / 4.0);
  hess(2, 0) = t28 *
                   (t93 + t95 + t96 - t4 * (-t92 + t116 + t138) * 2.0 -
                    t3 * t16 * t18 * t94 * 2.0) *
                   (1.0 / 2.0) -
               t24 * t33 * t78 * (1.0 / 4.0);
  hess(2, 1) =
      t28 *
          (t97 + t98 + t144 +
           t4 * (t142 - t3 * t8 * t18 + t2 * t3 * t8 * t21 * t39) * 2.0 +
           t6 * t8 * t16 * t21 * t39 * 2.0) *
          (1.0 / 2.0) -
      t33 * t73 * t78 * (1.0 / 4.0);
  hess(2, 2) =
      t28 *
          ((t43 * t43) * 2.0 +
           t4 * (t18 * t39 * -2.0 + t2 * t21 * t99 + t2 * t18 * t105) * 2.0 +
           t6 * t40 * t99 * 2.0 + t3 * t16 * t21 * t99 * 2.0 +
           t3 * t16 * t18 * t105 * 2.0) *
          (1.0 / 2.0) -
      t33 * (t78 * t78) * (1.0 / 4.0);
  hess(2, 3) =
      t28 *
          (t125 + t126 +
           t4 * (-t18 * t49 + t2 * t18 * t108 + t2 * t21 * t39 * t49) * 2.0 -
           t16 * t18 * t39 * 2.0 + t3 * t16 * t18 * t108 * 2.0 +
           t3 * t16 * t21 * t39 * t49 * 2.0) *
          (1.0 / 2.0) -
      t33 * t78 * t81 * (1.0 / 4.0);
  hess(2, 4) =
      t28 *
          (t149 + t150 + t151 + t43 * t58 * 2.0 - t3 * t16 * t18 * t112 * 2.0) *
          (1.0 / 2.0) -
      t33 * t78 * t86 * (1.0 / 4.0);
  hess(2, 5) =
      t28 *
          (t172 - t4 * (t160 + t2 * t18 * t115 - t2 * t21 * t39 * t60) * 2.0 +
           t16 * t18 * t39 * 2.0 + t3 * t18 * t39 * t64 * 2.0 -
           t3 * t16 * t18 * t115 * 2.0 + t3 * t16 * t21 * t39 * t60 * 2.0) *
          (1.0 / 2.0) -
      t33 * t78 * t91 * (1.0 / 4.0);
  hess(3, 0) = t28 *
                   (t117 + t118 + t167 +
                    t16 * (-t116 + t165 + t2 * t3 * t8 * t21 * t49) * 2.0 +
                    t4 * t5 * t8 * t21 * t49 * 2.0) *
                   (1.0 / 2.0) -
               t24 * t33 * t81 * (1.0 / 4.0);
  hess(3, 1) = t28 *
                   (t120 + t122 + t123 - t16 * (-t119 + t141 + t171) * 2.0 -
                    t2 * t4 * t18 * t121 * 2.0) *
                   (1.0 / 2.0) -
               t33 * t73 * t81 * (1.0 / 4.0);
  hess(3, 2) =
      t28 *
          (t125 + t126 +
           t16 * (-t124 + t3 * t18 * t108 + t3 * t21 * t39 * t49) * 2.0 -
           t4 * t18 * t49 * 2.0 + t2 * t4 * t18 * t108 * 2.0 +
           t2 * t4 * t21 * t39 * t49 * 2.0) *
          (1.0 / 2.0) -
      t33 * t78 * t81 * (1.0 / 4.0);
  hess(3, 3) =
      t28 *
          ((t52 * t52) * 2.0 +
           t16 * (t18 * t49 * -2.0 + t3 * t18 * t130 + t3 * t21 * t127) * 2.0 +
           t5 * t40 * t127 * 2.0 + t2 * t4 * t18 * t130 * 2.0 +
           t2 * t4 * t21 * t127 * 2.0) *
          (1.0 / 2.0) -
      t33 * (t81 * t81) * (1.0 / 4.0);
  hess(3, 4) =
      t28 *
          (t153 - t16 * (t178 + t3 * t18 * t132 - t3 * t21 * t49 * t54) * 2.0 +
           t4 * t18 * t49 * 2.0 + t2 * t18 * t49 * t58 * 2.0 -
           t2 * t4 * t18 * t132 * 2.0 + t2 * t4 * t21 * t49 * t54 * 2.0) *
          (1.0 / 2.0) -
      t33 * t81 * t86 * (1.0 / 4.0);
  hess(3, 5) =
      t28 *
          (t175 + t176 + t177 + t52 * t64 * 2.0 - t2 * t4 * t18 * t136 * 2.0) *
          (1.0 / 2.0) -
      t33 * t81 * t91 * (1.0 / 4.0);
  hess(4, 0) =
      t28 *
          (t17 * t140 * 2.0 +
           t4 * (t116 + t138 + t5 * t8 * t21 * (t41 - t55)) * 2.0 +
           t3 * t16 * t18 * t94 * 2.0 + t2 * t6 * t8 * t40 * (t41 - t55) * 2.0 +
           t2 * t3 * t8 * t16 * t21 * (t41 - t55) * 2.0) *
          (1.0 / 2.0) -
      t24 * t33 * t146 * (1.0 / 4.0);
  hess(4, 1) =
      t28 *
          (t143 - t144 + t4 * (t141 - t142 + t2 * t3 * t8 * t21 * t54) * 2.0 +
           t6 * t8 * t16 * t21 * t54 * 2.0 + t2 * t3 * t8 * t18 * t140 * 2.0) *
          (1.0 / 2.0) -
      t33 * t73 * t146 * (1.0 / 4.0);
  hess(4, 2) = t28 *
                   (t149 + t150 + t151 + t43 * t140 * 2.0 -
                    t3 * t16 * t18 * t112 * 2.0) *
                   (1.0 / 2.0) -
               t33 * t78 * t146 * (1.0 / 4.0);
  hess(4, 3) =
      t28 *
          (t153 + t4 * (t152 - t2 * t18 * t132 + t2 * t21 * t49 * t54) * 2.0 -
           t16 * t18 * (t41 - t55) * 2.0 - t3 * t16 * t18 * t132 * 2.0 +
           t2 * t18 * t49 * t140 * 2.0 + t3 * t16 * t21 * t49 * t54 * 2.0) *
          (1.0 / 2.0) -
      t33 * t81 * t146 * (1.0 / 4.0);
  hess(4, 4) =
      t33 * (t154 * t154) * (-1.0 / 4.0) +
      t28 *
          (t4 *
               (t18 * (t41 - t55) * 2.0 + t2 * t21 * (t155 * t155) +
                t2 * t18 * t158) *
               2.0 +
           (t140 * t140) * 2.0 + t6 * t40 * (t156 * t156) * 2.0 +
           t3 * t16 * t18 * t158 * 2.0 + t3 * t16 * t21 * (t159 * t159) * 2.0) *
          (1.0 / 2.0);
  hess(4, 5) =
      t28 *
          (t179 + t180 +
           t4 *
               (t160 + t2 * t18 * t161 + t2 * t21 * (t41 - t55) * (t50 - t61)) *
               2.0 +
           t16 * t18 * (t41 - t55) * 2.0 + t3 * t16 * t18 * t161 * 2.0 +
           t3 * t16 * t21 * (t41 - t55) * (t50 - t61) * 2.0) *
          (1.0 / 2.0) -
      t33 * t154 * t169 * (1.0 / 4.0);
  hess(5, 0) =
      t28 *
          (t166 - t167 + t16 * (t116 - t165 + t2 * t3 * t8 * t21 * t60) * 2.0 +
           t4 * t5 * t8 * t21 * t60 * 2.0 + t2 * t3 * t8 * t18 * t164 * 2.0) *
          (1.0 / 2.0) -
      t24 * t33 * t169 * (1.0 / 4.0);
  hess(5, 1) =
      t28 *
          (t32 * t164 * 2.0 +
           t16 * (t141 + t171 + t6 * t8 * t21 * (t50 - t61)) * 2.0 +
           t2 * t4 * t18 * t121 * 2.0 + t3 * t5 * t8 * t40 * (t50 - t61) * 2.0 +
           t2 * t3 * t4 * t8 * t21 * (t50 - t61) * 2.0) *
          (1.0 / 2.0) -
      t33 * t73 * t169 * (1.0 / 4.0);
  hess(5, 2) =
      t28 *
          (t172 + t16 * (t124 - t3 * t18 * t115 + t3 * t21 * t39 * t60) * 2.0 -
           t4 * t18 * (t50 - t61) * 2.0 - t2 * t4 * t18 * t115 * 2.0 +
           t3 * t18 * t39 * t164 * 2.0 + t2 * t4 * t21 * t39 * t60 * 2.0) *
          (1.0 / 2.0) -
      t33 * t78 * t169 * (1.0 / 4.0);
  hess(5, 3) =
      t28 *
          (t175 + t176 + t177 + t52 * t164 * 2.0 - t2 * t4 * t18 * t136 * 2.0) *
          (1.0 / 2.0) -
      t33 * t81 * t169 * (1.0 / 4.0);
  hess(5, 4) =
      t28 *
          (t179 + t180 +
           t16 *
               (t178 + t3 * t18 * t161 + t3 * t21 * (t41 - t55) * (t50 - t61)) *
               2.0 +
           t4 * t18 * (t50 - t61) * 2.0 + t2 * t4 * t18 * t161 * 2.0 +
           t2 * t4 * t21 * (t41 - t55) * (t50 - t61) * 2.0) *
          (1.0 / 2.0) -
      t33 * t154 * t169 * (1.0 / 4.0);
  hess(5, 5) =
      t33 * (t181 * t181) * (-1.0 / 4.0) +
      t28 *
          (t16 *
               (t18 * (t50 - t61) * 2.0 + t3 * t21 * (t182 * t182) +
                t3 * t18 * t185) *
               2.0 +
           (t164 * t164) * 2.0 + t5 * t40 * (t183 * t183) * 2.0 +
           t2 * t4 * t18 * t185 * 2.0 + t2 * t4 * t21 * (t186 * t186) * 2.0) *
          (1.0 / 2.0);
}

template <typename T>
inline void hesssoftpointedgedist(const Eigen::Matrix<T, 3, 1>& x0,
                                  const Eigen::Matrix<T, 3, 1>& x1,
                                  const Eigen::Matrix<T, 3, 1>& x2,
                                  Matrix9s& hess, const T& h) {
  const scalar t2 = x1(0) - x2(0);
  const scalar t3 = x1(1) - x2(1);
  const scalar t4 = x1(2) - x2(2);
  const scalar t6 = t2 * t2;
  ...
  hess(0, 0) =
      t36 *
          (t104 + t163 + (t32 * t32) * 2.0 + t2 * t5 * t6 * t33 * t35 * 2.0 +
           t3 * t6 * t20 * t33 * t35 * 2.0 + t4 * t6 * t21 * t33 * t35 * 2.0) *
          (1.0 / 2.0) -
      (t26 * t26) * t37 * (1.0 / 4.0);
  hess(0, 1) = t103;
  hess(0, 2) = t153;
  hess(0, 3) =
      t36 *
          (t73 + t74 + t179 + t180 + t181 + t182 +
           t32 *
               (-t19 +
                t2 * t23 *
                    (t10 * (x0(0) - x1(0) * 2.0 + x2(0)) - t17 * t33 * t46) +
                1.0) *
               2.0 +
           t5 * (t70 + t176 - t10 * t23 * t46) * 2.0 -
           t3 * t10 * t20 * t23 * 2.0 - t4 * t10 * t21 * t23 * 2.0) *
          (1.0 / 2.0) -
      t26 * t37 * t114 * (1.0 / 4.0);
  hess(0, 4) =
      t36 *
          (t80 + t81 + t215 + t216 + t217 + t218 +
           t5 *
               (t77 +
                t6 * t10 * t35 *
                    (t10 * (x0(1) - x1(1) * 2.0 + x2(1)) - t17 * t33 * t53)) *
               2.0 -
           t2 * t10 * t20 * t23 * 2.0 + t2 * t3 * t10 * t20 * t35 * t57 * 2.0) *
          (1.0 / 2.0) -
      t26 * t37 * t119 * (1.0 / 4.0);
  hess(0, 5) =
      t36 *
          (t87 + t88 + t254 + t255 + t256 + t257 +
           t5 *
               (t84 +
                t6 * t10 * t35 *
                    (t10 * (x0(2) - x1(2) * 2.0 + x2(2)) - t17 * t33 * t62)) *
               2.0 -
           t2 * t10 * t21 * t23 * 2.0 + t2 * t4 * t10 * t21 * t35 * t66 * 2.0) *
          (1.0 / 2.0) -
      t26 * t37 * t123 * (1.0 / 4.0);
  hess(0, 6) =
      t36 *
          (-t73 - t74 + t115 + t166 + t32 * t76 * 2.0 +
           t5 * (-t70 + t10 * t23 * t46 + t6 * t10 * t35 * t71) * 2.0 +
           t2 * t7 * t10 * t34 * t71 * 2.0 + t2 * t8 * t10 * t34 * t71 * 2.0 +
           t2 * t3 * t10 * t20 * t35 * t71 * 2.0 +
           t2 * t4 * t10 * t21 * t35 * t71 * 2.0) *
          (1.0 / 2.0) -
      t26 * t37 * t130 * (1.0 / 4.0);
  hess(0, 7) =
      t36 *
          (-t80 - t81 + t341 + t343 + t344 -
           t5 * (t77 - t6 * t10 * t35 * t78) * 2.0 +
           t2 * t10 * t20 * t23 * 2.0 + t2 * t3 * t10 * t23 * t83 * 2.0 +
           t2 * t3 * t10 * t20 * t35 * t78 * 2.0) *
          (1.0 / 2.0) -
      t26 * t37 * t137 * (1.0 / 4.0);
  hess(0, 8) =
      t36 *
          (-t87 - t88 + t383 + t385 + t386 -
           t5 * (t84 - t6 * t10 * t35 * t85) * 2.0 +
           t2 * t10 * t21 * t23 * 2.0 + t2 * t4 * t10 * t23 * t90 * 2.0 +
           t2 * t4 * t10 * t21 * t35 * t85 * 2.0) *
          (1.0 / 2.0) -
      t26 * t37 * t144 * (1.0 / 4.0);
  hess(1, 0) = t103;
  hess(1, 1) =
      t36 *
          (t104 + t164 + (t42 * t42) * 2.0 + t2 * t5 * t7 * t33 * t35 * 2.0 +
           t3 * t7 * t20 * t33 * t35 * 2.0 + t4 * t7 * t21 * t33 * t35 * 2.0) *
          (1.0 / 2.0) -
      t37 * (t102 * t102) * (1.0 / 4.0);
  hess(1, 2) = t162;
  hess(1, 3) =
      t36 *
          (t125 + t126 + t183 + t184 + t185 + t186 +
           t20 * (t124 + t7 * t10 * t35 * t50) * 2.0 -
           t3 * t5 * t10 * t23 * 2.0 + t2 * t3 * t5 * t10 * t35 * t50 * 2.0) *
          (1.0 / 2.0) -
      t37 * t102 * t114 * (1.0 / 4.0);
  hess(1, 4) = t36 *
                   (-t115 + t132 + t133 + t219 + t223 + t224 + t225 + t226 +
                    t20 * (t131 + t220 - t10 * t23 * t53) * 2.0 -
                    t2 * t5 * t10 * t23 * 2.0) *
                   (1.0 / 2.0) -
               t37 * t102 * t119 * (1.0 / 4.0);
  hess(1, 5) =
      t36 *
          (t139 + t140 + t259 + t260 + t261 + t262 +
           t20 * (t138 + t7 * t10 * t35 * t66) * 2.0 -
           t3 * t10 * t21 * t23 * 2.0 + t3 * t4 * t10 * t21 * t35 * t66 * 2.0) *
          (1.0 / 2.0) -
      t37 * t102 * t123 * (1.0 / 4.0);
  hess(1, 6) =
      t36 *
          (-t125 - t126 + t300 + t302 + t303 -
           t20 * (t124 - t7 * t10 * t35 * t71) * 2.0 +
           t3 * t5 * t10 * t23 * 2.0 + t2 * t3 * t10 * t23 * t76 * 2.0 +
           t2 * t3 * t5 * t10 * t35 * t71 * 2.0) *
          (1.0 / 2.0) -
      t37 * t102 * t130 * (1.0 / 4.0);
  hess(1, 7) =
      t36 *
          (t115 - t132 - t133 + t165 + t42 * t83 * 2.0 +
           t20 * (-t131 + t10 * t23 * t53 + t7 * t10 * t35 * t78) * 2.0 +
           t3 * t6 * t10 * t34 * t78 * 2.0 + t3 * t8 * t10 * t34 * t78 * 2.0 +
           t2 * t3 * t5 * t10 * t35 * t78 * 2.0 +
           t3 * t4 * t10 * t21 * t35 * t78 * 2.0) *
          (1.0 / 2.0) -
      t37 * t102 * t137 * (1.0 / 4.0);
  hess(1, 8) =
      t36 *
          (-t139 - t140 + t388 + t390 + t391 -
           t20 * (t138 - t7 * t10 * t35 * t85) * 2.0 +
           t3 * t10 * t21 * t23 * 2.0 + t3 * t4 * t10 * t23 * t90 * 2.0 +
           t3 * t4 * t10 * t21 * t35 * t85 * 2.0) *
          (1.0 / 2.0) -
      t37 * t102 * t144 * (1.0 / 4.0);
  hess(2, 0) = t153;
  hess(2, 1) = t162;
  hess(2, 2) =
      t36 *
          (t163 + t164 + (t44 * t44) * 2.0 + t2 * t5 * t8 * t33 * t35 * 2.0 +
           t3 * t8 * t20 * t33 * t35 * 2.0 + t4 * t8 * t21 * t33 * t35 * 2.0) *
          (1.0 / 2.0) -
      t37 * (t108 * t108) * (1.0 / 4.0);
  hess(2, 3) =
      t36 *
          (t168 + t169 + t187 + t188 + t189 + t190 +
           t21 * (t167 + t8 * t10 * t35 * t50) * 2.0 -
           t4 * t5 * t10 * t23 * 2.0 + t2 * t4 * t5 * t10 * t35 * t50 * 2.0) *
          (1.0 / 2.0) -
      t37 * t108 * t114 * (1.0 / 4.0);
  hess(2, 4) =
      t36 *
          (t171 + t172 + t227 + t228 + t229 + t230 +
           t21 * (t170 + t8 * t10 * t35 * t57) * 2.0 -
           t4 * t10 * t20 * t23 * 2.0 + t3 * t4 * t10 * t20 * t35 * t57 * 2.0) *
          (1.0 / 2.0) -
      t37 * t108 * t119 * (1.0 / 4.0);
  hess(2, 5) = t36 *
                   (-t165 - t166 + t174 + t175 + t263 + t267 + t268 + t269 +
                    t270 + t21 * (t173 + t264 - t10 * t23 * t62) * 2.0) *
                   (1.0 / 2.0) -
               t37 * t108 * t123 * (1.0 / 4.0);
  hess(2, 6) =
      t36 *
          (-t168 - t169 + t308 + t310 + t311 -
           t21 * (t167 - t8 * t10 * t35 * t71) * 2.0 +
           t4 * t5 * t10 * t23 * 2.0 + t2 * t4 * t10 * t23 * t76 * 2.0 +
           t2 * t4 * t5 * t10 * t35 * t71 * 2.0) *
          (1.0 / 2.0) -
      t37 * t108 * t130 * (1.0 / 4.0);
  hess(2, 7) =
      t36 *
          (-t171 - t172 + t349 + t351 + t352 -
           t21 * (t170 - t8 * t10 * t35 * t78) * 2.0 +
           t4 * t10 * t20 * t23 * 2.0 + t3 * t4 * t10 * t23 * t83 * 2.0 +
           t3 * t4 * t10 * t20 * t35 * t78 * 2.0) *
          (1.0 / 2.0) -
      t37 * t108 * t137 * (1.0 / 4.0);
  hess(2, 8) =
      t36 *
          (t165 + t166 - t174 - t175 + t44 * t90 * 2.0 +
           t21 * (-t173 + t10 * t23 * t62 + t8 * t10 * t35 * t85) * 2.0 +
           t4 * t6 * t10 * t34 * t85 * 2.0 + t4 * t7 * t10 * t34 * t85 * 2.0 +
           t2 * t4 * t5 * t10 * t35 * t85 * 2.0 +
           t3 * t4 * t10 * t20 * t35 * t85 * 2.0) *
          (1.0 / 2.0) -
      t37 * t108 * t144 * (1.0 / 4.0);
  hess(3, 0) = t36 *
                   (t179 + t180 + t181 + t182 + t32 * t110 * 2.0 -
                    t5 * (-t176 + t214 + t296) * 2.0 -
                    t3 * t20 * t23 * t177 * 2.0 - t4 * t21 * t23 * t177 * 2.0) *
                   (1.0 / 2.0) -
               t26 * t37 * t114 * (1.0 / 4.0);
  hess(3, 1) =
      t36 *
          (t126 + t183 + t184 + t185 + t186 + t301 +
           t5 * (t299 - t3 * t10 * t23 + t2 * t3 * t10 * t35 * t50) * 2.0 +
           t7 * t10 * t20 * t35 * t50 * 2.0) *
          (1.0 / 2.0) -
      t37 * t102 * t114 * (1.0 / 4.0);
  hess(3, 2) =
      t36 *
          (t169 + t187 + t188 + t189 + t190 + t309 +
           t5 * (t307 - t4 * t10 * t23 + t2 * t4 * t10 * t35 * t50) * 2.0 +
           t8 * t10 * t21 * t35 * t50 * 2.0) *
          (1.0 / 2.0) -
      t37 * t108 * t114 * (1.0 / 4.0);
  hess(3, 3) =
      t36 *
          ((t110 * t110) * 2.0 +
           t5 * (t23 * t50 * -2.0 + t2 * t23 * t197 + t2 * t35 * t191) * 2.0 +
           t7 * t34 * t191 * 2.0 + t8 * t34 * t191 * 2.0 +
           t3 * t20 * t23 * t197 * 2.0 + t4 * t21 * t23 * t197 * 2.0 +
           t3 * t20 * t35 * t191 * 2.0 + t4 * t21 * t35 * t191 * 2.0) *
          (1.0 / 2.0) -
      t37 * (t114 * t114) * (1.0 / 4.0);
  hess(3, 4) =
      t36 *
          (t232 + t233 + t234 + t235 + t236 +
           t5 * (-t23 * t57 + t2 * t23 * t201 + t2 * t35 * t50 * t57) * 2.0 -
           t20 * t23 * t50 * 2.0 + t3 * t20 * t23 * t201 * 2.0 +
           t3 * t20 * t35 * t50 * t57 * 2.0) *
          (1.0 / 2.0) -
      t37 * t114 * t119 * (1.0 / 4.0);
  hess(3, 5) =
      t36 *
          (t271 + t272 + t273 + t274 + t275 +
           t5 * (-t23 * t66 + t2 * t23 * t205 + t2 * t35 * t50 * t66) * 2.0 -
           t21 * t23 * t50 * 2.0 + t4 * t21 * t23 * t205 * 2.0 +
           t4 * t21 * t35 * t50 * t66 * 2.0) *
          (1.0 / 2.0) -
      t37 * t114 * t123 * (1.0 / 4.0);
  hess(3, 6) = t36 *
                   (t314 + t315 + t316 + t317 + t318 + t76 * t110 * 2.0 -
                    t3 * t20 * t23 * t209 * 2.0 - t4 * t21 * t23 * t209 * 2.0) *
                   (1.0 / 2.0) -
               t37 * t114 * t130 * (1.0 / 4.0);
  hess(3, 7) = t36 *
                   (t353 + t354 + t355 -
                    t5 * (t290 + t2 * t23 * t211 - t2 * t35 * t50 * t78) * 2.0 +
                    t20 * t23 * t50 * 2.0 + t3 * t23 * t50 * t83 * 2.0 -
                    t3 * t20 * t23 * t211 * 2.0 - t4 * t21 * t23 * t211 * 2.0 +
                    t3 * t20 * t35 * t50 * t78 * 2.0) *
                   (1.0 / 2.0) -
               t37 * t114 * t137 * (1.0 / 4.0);
  hess(3, 8) = t36 *
                   (t393 + t394 + t395 -
                    t5 * (t251 + t2 * t23 * t213 - t2 * t35 * t50 * t85) * 2.0 +
                    t21 * t23 * t50 * 2.0 + t4 * t23 * t50 * t90 * 2.0 -
                    t3 * t20 * t23 * t213 * 2.0 - t4 * t21 * t23 * t213 * 2.0 +
                    t4 * t21 * t35 * t50 * t85 * 2.0) *
                   (1.0 / 2.0) -
               t37 * t114 * t144 * (1.0 / 4.0);
  hess(4, 0) = t36 *
                   (t81 + t215 + t216 + t217 + t218 + t342 +
                    t20 * (-t214 + t340 + t2 * t3 * t10 * t35 * t57) * 2.0 +
                    t5 * t6 * t10 * t35 * t57 * 2.0) *
                   (1.0 / 2.0) -
               t26 * t37 * t119 * (1.0 / 4.0);
  hess(4, 1) = t36 *
                   (t219 + t223 + t224 + t225 + t226 -
                    t20 * (-t220 + t258 + t347) * 2.0 -
                    t2 * t5 * t23 * t221 * 2.0 - t4 * t21 * t23 * t221 * 2.0) *
                   (1.0 / 2.0) -
               t37 * t102 * t119 * (1.0 / 4.0);
  hess(4, 2) =
      t36 *
          (t171 + t227 + t228 + t229 + t230 + t350 +
           t20 * (t348 - t4 * t10 * t23 + t3 * t4 * t10 * t35 * t57) * 2.0 +
           t8 * t10 * t21 * t35 * t57 * 2.0) *
          (1.0 / 2.0) -
      t37 * t108 * t119 * (1.0 / 4.0);
  hess(4, 3) =
      t36 *
          (t232 + t233 + t234 + t235 + t236 +
           t20 * (-t231 + t3 * t23 * t201 + t3 * t35 * t50 * t57) * 2.0 -
           t5 * t23 * t57 * 2.0 + t2 * t5 * t23 * t201 * 2.0 +
           t2 * t5 * t35 * t50 * t57 * 2.0) *
          (1.0 / 2.0) -
      t37 * t114 * t119 * (1.0 / 4.0);
  hess(4, 4) =
      t36 *
          ((t60 * t60) * 2.0 +
           t20 * (t23 * t57 * -2.0 + t3 * t23 * t240 + t3 * t35 * t237) * 2.0 +
           t6 * t34 * t237 * 2.0 + t8 * t34 * t237 * 2.0 +
           t2 * t5 * t23 * t240 * 2.0 + t2 * t5 * t35 * t237 * 2.0 +
           t4 * t21 * t23 * t240 * 2.0 + t4 * t21 * t35 * t237 * 2.0) *
          (1.0 / 2.0) -
      t37 * (t119 * t119) * (1.0 / 4.0);
  hess(4, 5) =
      t36 *
          (t277 + t278 + t279 + t280 + t281 +
           t20 * (-t23 * t66 + t3 * t23 * t244 + t3 * t35 * t57 * t66) * 2.0 -
           t21 * t23 * t57 * 2.0 + t4 * t21 * t23 * t244 * 2.0 +
           t4 * t21 * t35 * t57 * t66 * 2.0) *
          (1.0 / 2.0) -
      t37 * t119 * t123 * (1.0 / 4.0);
  hess(4, 6) =
      t36 *
          (t319 + t320 + t321 -
           t20 * (t287 + t3 * t23 * t247 - t3 * t35 * t57 * t71) * 2.0 +
           t5 * t23 * t57 * 2.0 + t2 * t23 * t57 * t76 * 2.0 -
           t2 * t5 * t23 * t247 * 2.0 - t4 * t21 * t23 * t247 * 2.0 +
           t2 * t5 * t35 * t57 * t71 * 2.0) *
          (1.0 / 2.0) -
      t37 * t119 * t130 * (1.0 / 4.0);
  hess(4, 7) = t36 *
                   (t358 + t359 + t360 + t361 + t362 + t60 * t83 * 2.0 -
                    t2 * t5 * t23 * t250 * 2.0 - t4 * t21 * t23 * t250 * 2.0) *
                   (1.0 / 2.0) -
               t37 * t119 * t137 * (1.0 / 4.0);
  hess(4, 8) =
      t36 *
          (t396 + t397 + t398 -
           t20 * (t251 + t3 * t23 * t253 - t3 * t35 * t57 * t85) * 2.0 +
           t21 * t23 * t57 * 2.0 + t4 * t23 * t57 * t90 * 2.0 -
           t2 * t5 * t23 * t253 * 2.0 - t4 * t21 * t23 * t253 * 2.0 +
           t4 * t21 * t35 * t57 * t85 * 2.0) *
          (1.0 / 2.0) -
      t37 * t119 * t144 * (1.0 / 4.0);
  hess(5, 0) = t36 *
                   (t87 + t254 + t255 + t256 + t257 + t384 +
                    t21 * (-t214 + t382 + t2 * t4 * t10 * t35 * t66) * 2.0 +
                    t5 * t6 * t10 * t35 * t66 * 2.0) *
                   (1.0 / 2.0) -
               t26 * t37 * t123 * (1.0 / 4.0);
  hess(5, 1) = t36 *
                   (t139 + t259 + t260 + t261 + t262 + t389 +
                    t21 * (-t258 + t387 + t3 * t4 * t10 * t35 * t66) * 2.0 +
                    t7 * t10 * t20 * t35 * t66 * 2.0) *
                   (1.0 / 2.0) -
               t37 * t102 * t123 * (1.0 / 4.0);
  hess(5, 2) = t36 *
                   (t263 + t267 + t268 + t269 + t270 -
                    t21 * (-t264 + t306 + t392) * 2.0 -
                    t2 * t5 * t23 * t265 * 2.0 - t3 * t20 * t23 * t265 * 2.0) *
                   (1.0 / 2.0) -
               t37 * t108 * t123 * (1.0 / 4.0);
  hess(5, 3) =
      t36 *
          (t271 + t272 + t273 + t274 + t275 +
           t21 * (-t231 + t4 * t23 * t205 + t4 * t35 * t50 * t66) * 2.0 -
           t5 * t23 * t66 * 2.0 + t2 * t5 * t23 * t205 * 2.0 +
           t2 * t5 * t35 * t50 * t66 * 2.0) *
          (1.0 / 2.0) -
      t37 * t114 * t123 * (1.0 / 4.0);
  hess(5, 4) =
      t36 *
          (t277 + t278 + t279 + t280 + t281 +
           t21 * (-t276 + t4 * t23 * t244 + t4 * t35 * t57 * t66) * 2.0 -
           t20 * t23 * t66 * 2.0 + t3 * t20 * t23 * t244 * 2.0 +
           t3 * t20 * t35 * t57 * t66 * 2.0) *
          (1.0 / 2.0) -
      t37 * t119 * t123 * (1.0 / 4.0);
  hess(5, 5) =
      t36 *
          ((t69 * t69) * 2.0 +
           t21 * (t23 * t66 * -2.0 + t4 * t23 * t285 + t4 * t35 * t282) * 2.0 +
           t6 * t34 * t282 * 2.0 + t7 * t34 * t282 * 2.0 +
           t2 * t5 * t23 * t285 * 2.0 + t2 * t5 * t35 * t282 * 2.0 +
           t3 * t20 * t23 * t285 * 2.0 + t3 * t20 * t35 * t282 * 2.0) *
          (1.0 / 2.0) -
      t37 * (t123 * t123) * (1.0 / 4.0);
  hess(5, 6) =
      t36 *
          (t323 + t324 + t325 -
           t21 * (t287 + t4 * t23 * t289 - t4 * t35 * t66 * t71) * 2.0 +
           t5 * t23 * t66 * 2.0 + t2 * t23 * t66 * t76 * 2.0 -
           t2 * t5 * t23 * t289 * 2.0 - t3 * t20 * t23 * t289 * 2.0 +
           t2 * t5 * t35 * t66 * t71 * 2.0) *
          (1.0 / 2.0) -
      t37 * t123 * t130 * (1.0 / 4.0);
  hess(5, 7) =
      t36 *
          (t363 + t364 + t365 -
           t21 * (t290 + t4 * t23 * t292 - t4 * t35 * t66 * t78) * 2.0 +
           t20 * t23 * t66 * 2.0 + t3 * t23 * t66 * t83 * 2.0 -
           t2 * t5 * t23 * t292 * 2.0 - t3 * t20 * t23 * t292 * 2.0 +
           t3 * t20 * t35 * t66 * t78 * 2.0) *
          (1.0 / 2.0) -
      t37 * t123 * t137 * (1.0 / 4.0);
  hess(5, 8) = t36 *
                   (t401 + t402 + t403 + t404 + t405 + t69 * t90 * 2.0 -
                    t2 * t5 * t23 * t295 * 2.0 - t3 * t20 * t23 * t295 * 2.0) *
                   (1.0 / 2.0) -
               t37 * t123 * t144 * (1.0 / 4.0);
  hess(6, 0) = t36 *
                   (t32 * t298 * 2.0 +
                    t5 * (t214 + t296 + t6 * t10 * t35 * (t51 - t72)) * 2.0 +
                    t3 * t20 * t23 * t177 * 2.0 + t4 * t21 * t23 * t177 * 2.0 +
                    t2 * t7 * t10 * t34 * (t51 - t72) * 2.0 +
                    t2 * t8 * t10 * t34 * (t51 - t72) * 2.0 +
                    t2 * t3 * t10 * t20 * t35 * (t51 - t72) * 2.0 +
                    t2 * t4 * t10 * t21 * t35 * (t51 - t72) * 2.0) *
                   (1.0 / 2.0) -
               t26 * t37 * t305 * (1.0 / 4.0);
  hess(6, 1) = t36 *
                   (-t126 + t300 - t301 + t302 + t303 +
                    t5 * (t258 - t299 + t2 * t3 * t10 * t35 * t71) * 2.0 +
                    t7 * t10 * t20 * t35 * t71 * 2.0 +
                    t2 * t3 * t10 * t23 * t298 * 2.0) *
                   (1.0 / 2.0) -
               t37 * t102 * t305 * (1.0 / 4.0);
  hess(6, 2) = t36 *
                   (-t169 + t308 - t309 + t310 + t311 +
                    t5 * (t306 - t307 + t2 * t4 * t10 * t35 * t71) * 2.0 +
                    t8 * t10 * t21 * t35 * t71 * 2.0 +
                    t2 * t4 * t10 * t23 * t298 * 2.0) *
                   (1.0 / 2.0) -
               t37 * t108 * t305 * (1.0 / 4.0);
  hess(6, 3) = t36 *
                   (t314 + t315 + t316 + t317 + t318 + t110 * t298 * 2.0 -
                    t3 * t20 * t23 * t209 * 2.0 - t4 * t21 * t23 * t209 * 2.0) *
                   (1.0 / 2.0) -
               t37 * t114 * t305 * (1.0 / 4.0);
  hess(6, 4) =
      t36 *
          (t319 + t320 + t321 +
           t5 * (t276 - t2 * t23 * t247 + t2 * t35 * t57 * t71) * 2.0 -
           t20 * t23 * (t51 - t72) * 2.0 - t3 * t20 * t23 * t247 * 2.0 -
           t4 * t21 * t23 * t247 * 2.0 + t2 * t23 * t57 * t298 * 2.0 +
           t3 * t20 * t35 * t57 * t71 * 2.0) *
          (1.0 / 2.0) -
      t37 * t119 * t305 * (1.0 / 4.0);
  hess(6, 5) =
      t36 *
          (t323 + t324 + t325 +
           t5 * (t322 - t2 * t23 * t289 + t2 * t35 * t66 * t71) * 2.0 -
           t21 * t23 * (t51 - t72) * 2.0 - t3 * t20 * t23 * t289 * 2.0 -
           t4 * t21 * t23 * t289 * 2.0 + t2 * t23 * t66 * t298 * 2.0 +
           t4 * t21 * t35 * t66 * t71 * 2.0) *
          (1.0 / 2.0) -
      t37 * t123 * t305 * (1.0 / 4.0);
  hess(6, 6) =
      t36 *
          (t5 *
               (t23 * (t51 - t72) * 2.0 + t2 * t35 * (t327 * t327) +
                t2 * t23 * t331) *
               2.0 +
           (t298 * t298) * 2.0 + t7 * t34 * (t328 * t328) * 2.0 +
           t8 * t34 * (t329 * t329) * 2.0 + t3 * t20 * t23 * t331 * 2.0 +
           t4 * t21 * t23 * t331 * 2.0 + t3 * t20 * t35 * (t332 * t332) * 2.0 +
           t4 * t21 * t35 * (t333 * t333) * 2.0) *
          (1.0 / 2.0) -
      t37 * (t326 * t326) * (1.0 / 4.0);
  hess(6, 7) =
      t36 *
          (t366 + t367 + t368 + t369 + t370 +
           t5 *
               (t290 + t2 * t23 * t334 + t2 * t35 * (t51 - t72) * (t58 - t79)) *
               2.0 +
           t20 * t23 * (t51 - t72) * 2.0 + t3 * t20 * t23 * t334 * 2.0 +
           t3 * t20 * t35 * (t51 - t72) * (t58 - t79) * 2.0) *
          (1.0 / 2.0) -
      t37 * t305 * t346 * (1.0 / 4.0);
  hess(6, 8) =
      t36 *
          (t406 + t407 + t408 + t409 + t410 +
           t5 *
               (t251 + t2 * t23 * t337 + t2 * t35 * (t51 - t72) * (t67 - t86)) *
               2.0 +
           t21 * t23 * (t51 - t72) * 2.0 + t4 * t21 * t23 * t337 * 2.0 +
           t4 * t21 * t35 * (t51 - t72) * (t67 - t86) * 2.0) *
          (1.0 / 2.0) -
      t37 * t305 * t381 * (1.0 / 4.0);
  hess(7, 0) =
      t36 *
          (-t81 + t341 - t342 + t343 + t344 +
           t20 * (t214 - t340 + t2 * t3 * t10 * t35 * t78) * 2.0 +
           t5 * t6 * t10 * t35 * t78 * 2.0 + t2 * t3 * t10 * t23 * t336 * 2.0) *
          (1.0 / 2.0) -
      t26 * t37 * t346 * (1.0 / 4.0);
  hess(7, 1) = t36 *
                   (t42 * t336 * 2.0 +
                    t20 * (t258 + t347 + t7 * t10 * t35 * (t58 - t79)) * 2.0 +
                    t2 * t5 * t23 * t221 * 2.0 + t4 * t21 * t23 * t221 * 2.0 +
                    t3 * t6 * t10 * t34 * (t58 - t79) * 2.0 +
                    t3 * t8 * t10 * t34 * (t58 - t79) * 2.0 +
                    t2 * t3 * t5 * t10 * t35 * (t58 - t79) * 2.0 +
                    t3 * t4 * t10 * t21 * t35 * (t58 - t79) * 2.0) *
                   (1.0 / 2.0) -
               t37 * t102 * t346 * (1.0 / 4.0);
  hess(7, 2) = t36 *
                   (-t171 + t349 - t350 + t351 + t352 +
                    t20 * (t306 - t348 + t3 * t4 * t10 * t35 * t78) * 2.0 +
                    t8 * t10 * t21 * t35 * t78 * 2.0 +
                    t3 * t4 * t10 * t23 * t336 * 2.0) *
                   (1.0 / 2.0) -
               t37 * t108 * t346 * (1.0 / 4.0);
  hess(7, 3) =
      t36 *
          (t353 + t354 + t355 +
           t20 * (t231 - t3 * t23 * t211 + t3 * t35 * t50 * t78) * 2.0 -
           t5 * t23 * (t58 - t79) * 2.0 - t2 * t5 * t23 * t211 * 2.0 -
           t4 * t21 * t23 * t211 * 2.0 + t3 * t23 * t50 * t336 * 2.0 +
           t2 * t5 * t35 * t50 * t78 * 2.0) *
          (1.0 / 2.0) -
      t37 * t114 * t346 * (1.0 / 4.0);
  hess(7, 4) = t36 *
                   (t358 + t359 + t360 + t361 + t362 + t60 * t336 * 2.0 -
                    t2 * t5 * t23 * t250 * 2.0 - t4 * t21 * t23 * t250 * 2.0) *
                   (1.0 / 2.0) -
               t37 * t119 * t346 * (1.0 / 4.0);
  hess(7, 5) =
      t36 *
          (t363 + t364 + t365 +
           t20 * (t322 - t3 * t23 * t292 + t3 * t35 * t66 * t78) * 2.0 -
           t21 * t23 * (t58 - t79) * 2.0 - t2 * t5 * t23 * t292 * 2.0 -
           t4 * t21 * t23 * t292 * 2.0 + t3 * t23 * t66 * t336 * 2.0 +
           t4 * t21 * t35 * t66 * t78 * 2.0) *
          (1.0 / 2.0) -
      t37 * t123 * t346 * (1.0 / 4.0);
  hess(7, 6) =
      t36 *
          (t366 + t367 + t368 + t369 + t370 +
           t20 *
               (t287 + t3 * t23 * t334 + t3 * t35 * (t51 - t72) * (t58 - t79)) *
               2.0 +
           t5 * t23 * (t58 - t79) * 2.0 + t2 * t5 * t23 * t334 * 2.0 +
           t2 * t5 * t35 * (t51 - t72) * (t58 - t79) * 2.0) *
          (1.0 / 2.0) -
      t37 * t305 * t346 * (1.0 / 4.0);
  hess(7, 7) =
      t36 *
          (t20 *
               (t23 * (t58 - t79) * 2.0 + t3 * t35 * (t372 * t372) +
                t3 * t23 * t376) *
               2.0 +
           (t336 * t336) * 2.0 + t6 * t34 * (t373 * t373) * 2.0 +
           t8 * t34 * (t374 * t374) * 2.0 + t2 * t5 * t23 * t376 * 2.0 +
           t4 * t21 * t23 * t376 * 2.0 + t2 * t5 * t35 * (t377 * t377) * 2.0 +
           t4 * t21 * t35 * (t378 * t378) * 2.0) *
          (1.0 / 2.0) -
      t37 * (t371 * t371) * (1.0 / 4.0);
  hess(7, 8) =
      t36 *
          (t411 + t412 + t413 + t414 + t415 +
           t20 *
               (t251 + t3 * t23 * t379 + t3 * t35 * (t58 - t79) * (t67 - t86)) *
               2.0 +
           t21 * t23 * (t58 - t79) * 2.0 + t4 * t21 * t23 * t379 * 2.0 +
           t4 * t21 * t35 * (t58 - t79) * (t67 - t86) * 2.0) *
          (1.0 / 2.0) -
      t37 * t346 * t381 * (1.0 / 4.0);
  hess(8, 0) =
      t36 *
          (-t87 + t383 - t384 + t385 + t386 +
           t21 * (t214 - t382 + t2 * t4 * t10 * t35 * t85) * 2.0 +
           t5 * t6 * t10 * t35 * t85 * 2.0 + t2 * t4 * t10 * t23 * t339 * 2.0) *
          (1.0 / 2.0) -
      t26 * t37 * t381 * (1.0 / 4.0);
  hess(8, 1) = t36 *
                   (-t139 + t388 - t389 + t390 + t391 +
                    t21 * (t258 - t387 + t3 * t4 * t10 * t35 * t85) * 2.0 +
                    t7 * t10 * t20 * t35 * t85 * 2.0 +
                    t3 * t4 * t10 * t23 * t339 * 2.0) *
                   (1.0 / 2.0) -
               t37 * t102 * t381 * (1.0 / 4.0);
  hess(8, 2) = t36 *
                   (t44 * t339 * 2.0 +
                    t21 * (t306 + t392 + t8 * t10 * t35 * (t67 - t86)) * 2.0 +
                    t2 * t5 * t23 * t265 * 2.0 + t3 * t20 * t23 * t265 * 2.0 +
                    t4 * t6 * t10 * t34 * (t67 - t86) * 2.0 +
                    t4 * t7 * t10 * t34 * (t67 - t86) * 2.0 +
                    t2 * t4 * t5 * t10 * t35 * (t67 - t86) * 2.0 +
                    t3 * t4 * t10 * t20 * t35 * (t67 - t86) * 2.0) *
                   (1.0 / 2.0) -
               t37 * t108 * t381 * (1.0 / 4.0);
  hess(8, 3) =
      t36 *
          (t393 + t394 + t395 +
           t21 * (t231 - t4 * t23 * t213 + t4 * t35 * t50 * t85) * 2.0 -
           t5 * t23 * (t67 - t86) * 2.0 - t2 * t5 * t23 * t213 * 2.0 -
           t3 * t20 * t23 * t213 * 2.0 + t4 * t23 * t50 * t339 * 2.0 +
           t2 * t5 * t35 * t50 * t85 * 2.0) *
          (1.0 / 2.0) -
      t37 * t114 * t381 * (1.0 / 4.0);
  hess(8, 4) =
      t36 *
          (t396 + t397 + t398 +
           t21 * (t276 - t4 * t23 * t253 + t4 * t35 * t57 * t85) * 2.0 -
           t20 * t23 * (t67 - t86) * 2.0 - t2 * t5 * t23 * t253 * 2.0 -
           t3 * t20 * t23 * t253 * 2.0 + t4 * t23 * t57 * t339 * 2.0 +
           t3 * t20 * t35 * t57 * t85 * 2.0) *
          (1.0 / 2.0) -
      t37 * t119 * t381 * (1.0 / 4.0);
  hess(8, 5) = t36 *
                   (t401 + t402 + t403 + t404 + t405 + t69 * t339 * 2.0 -
                    t2 * t5 * t23 * t295 * 2.0 - t3 * t20 * t23 * t295 * 2.0) *
                   (1.0 / 2.0) -
               t37 * t123 * t381 * (1.0 / 4.0);
  hess(8, 6) =
      t36 *
          (t406 + t407 + t408 + t409 + t410 +
           t21 *
               (t287 + t4 * t23 * t337 + t4 * t35 * (t51 - t72) * (t67 - t86)) *
               2.0 +
           t5 * t23 * (t67 - t86) * 2.0 + t2 * t5 * t23 * t337 * 2.0 +
           t2 * t5 * t35 * (t51 - t72) * (t67 - t86) * 2.0) *
          (1.0 / 2.0) -
      t37 * t305 * t381 * (1.0 / 4.0);
  hess(8, 7) =
      t36 *
          (t411 + t412 + t413 + t414 + t415 +
           t21 *
               (t290 + t4 * t23 * t379 + t4 * t35 * (t58 - t79) * (t67 - t86)) *
               2.0 +
           t20 * t23 * (t67 - t86) * 2.0 + t3 * t20 * t23 * t379 * 2.0 +
           t3 * t20 * t35 * (t58 - t79) * (t67 - t86) * 2.0) *
          (1.0 / 2.0) -
      t37 * t346 * t381 * (1.0 / 4.0);
  hess(8, 8) =
      t36 *
          (t21 *
               (t23 * (t67 - t86) * 2.0 + t4 * t35 * (t417 * t417) +
                t4 * t23 * t421) *
               2.0 +
           (t339 * t339) * 2.0 + t6 * t34 * (t418 * t418) * 2.0 +
           t7 * t34 * (t419 * t419) * 2.0 + t2 * t5 * t23 * t421 * 2.0 +
           t3 * t20 * t23 * t421 * 2.0 + t2 * t5 * t35 * (t422 * t422) * 2.0 +
           t3 * t20 * t35 * (t423 * t423) * 2.0) *
          (1.0 / 2.0) -
      t37 * (t416 * t416) * (1.0 / 4.0);
}

template <typename T, int DIM>
inline void pointedgevec(const Eigen::Matrix<T, DIM, 1>& x1,
                         const Eigen::Matrix<T, DIM, 1>& x2,
                         const Eigen::Matrix<T, DIM, 1>& x3,
                         Eigen::Matrix<T, DIM, 1>& n, T& alpha) {
  alpha = std::min((T)1.0, std::max((T)0.0, (x1 - x2).dot(x3 - x2) /
                                                (x3 - x2).dot(x3 - x2)));
  Eigen::Matrix<T, DIM, 1> closest = x2 + alpha * (x3 - x2);
  n = closest - x1;
...
template <typename T, int DIM>
inline T pointlinealpha(const Eigen::Matrix<T, DIM, 1>& x1,
                        const Eigen::Matrix<T, DIM, 1>& x2,
                        const Eigen::Matrix<T, DIM, 1>& x3) {
...
template <typename T, int DIM>
inline void pointlinevec(const Eigen::Matrix<T, DIM, 1>& x1,
                         const Eigen::Matrix<T, DIM, 1>& x2,
                         const Eigen::Matrix<T, DIM, 1>& x3,
                         Eigen::Matrix<T, DIM, 1>& n, T& alpha) {
...
inline void grad_pointline_dist_fixed(const VectorXs& x0, const VectorXs& x1,
                                      const VectorXs& x2, const VectorXs& x3,
                                      const scalar& alpha_0,
                                      const scalar& alpha_1, VectorXs& grad) {
...
template <typename T, int DIM>
inline void grad_pointline_dist_fixed_constant(
    const Eigen::Matrix<T, DIM, 1>& x1, const Eigen::Matrix<T, DIM, 1>& x2,
    const Eigen::Matrix<T, DIM, 1>& x3, const scalar& alpha, const scalar& K,
...
template <typename T, int DIM>
inline void hess_pointline_dist_fixed(
    const Eigen::Matrix<T, DIM, 1>& x1, const Eigen::Matrix<T, DIM, 1>& x2,
    const Eigen::Matrix<T, DIM, 1>& x3, const scalar& alpha,
    Eigen::Matrix<T, DIM * 3, DIM * 3>& hess) {
  Eigen::Matrix<T, DIM, 1> closest = x2 + alpha * (x3 - x2);
  Eigen::Matrix<T, DIM, 1> n = closest - x1;
  scalar d = n.norm();
  Eigen::Matrix<T, DIM, 1> nhat = n / d;
  Eigen::Matrix<T, DIM, DIM> innt =
      (Eigen::Matrix<T, DIM, DIM>::Identity() - nhat * nhat.transpose()) / d;
...
template <typename T, int DIM>
inline void hess_pointline_dist_fixed(
    const Eigen::Matrix<T, DIM, 1>& x0, const Eigen::Matrix<T, DIM, 1>& x1,
    const Eigen::Matrix<T, DIM, 1>& x2, const Eigen::Matrix<T, DIM, 1>& x3,
    const scalar& alpha_0, const scalar& alpha_1,
    Eigen::Matrix<T, DIM * 4, DIM * 4>& hess) {
  ...
  hess.template block<DIM, DIM>(0, 0) =
      (1.0 - alpha_0) * (1.0 - alpha_0) * innt;
  hess.template block<DIM, DIM>(0, DIM) = (1.0 - alpha_0) * alpha_0 * innt;
  hess.template block<DIM, DIM>(0, DIM * 2) =
      -(1.0 - alpha_0) * (1.0 - alpha_1) * innt;
  ...
  hess.template block<DIM, DIM>(DIM, DIM * 2) =
      -alpha_0 * (1.0 - alpha_1) * innt;
  hess.template block<DIM, DIM>(DIM, DIM * 3) = -alpha_0 * alpha_1 * innt;

  hess.template block<DIM, DIM>(DIM * 2, 0) =
      -(1.0 - alpha_0) * (1.0 - alpha_1) * innt;
  hess.template block<DIM, DIM>(DIM * 2, DIM) =
      -alpha_0 * (1.0 - alpha_1) * innt;
  hess.template block<DIM, DIM>(DIM * 2, DIM * 2) =
      (1.0 - alpha_1) * (1.0 - alpha_1) * innt;
  hess.template block<DIM, DIM>(DIM * 2, DIM * 3) =
      alpha_1 * (1.0 - alpha_1) * innt;

  hess.template block<DIM, DIM>(DIM * 3, 0) = -(1.0 - alpha_0) * alpha_1 * innt;
  hess.template block<DIM, DIM>(DIM * 3, DIM) = -alpha_0 * alpha_1 * innt;
  hess.template block<DIM, DIM>(DIM * 3, DIM * 2) =
      alpha_1 * (1.0 - alpha_1) * innt;
...
inline void hess_pointline_dist_fixed(
    ...
    const Eigen::Matrix<T, DIM, 1>& x2, const Eigen::Matrix<T, DIM, 1>& x3,
    const scalar& alpha_0, const scalar& alpha_1,
    Eigen::Matrix<T, 1, DIM * 4>& hess, const int local_pidx, const int idir) {
  Eigen::Matrix<T, DIM, 1> cur = x0 + alpha_0 * (x1 - x0);
  Eigen::Matrix<T, DIM, 1> closest = x2 + alpha_1 * (x3 - x2);
  Eigen::Matrix<T, DIM, 1> n = closest - cur;
  ...
  if (local_pidx == 0) {
    hess.template block<1, DIM>(0, 0) =
        (1.0 - alpha_0) * (1.0 - alpha_0) * innt;
    hess.template block<1, DIM>(0, DIM) = (1.0 - alpha_0) * alpha_0 * innt;
    hess.template block<1, DIM>(0, DIM * 2) =
        -(1.0 - alpha_0) * (1.0 - alpha_1) * innt;
...
template <typename T, int DIM>
inline void hess_pointline_dist_fixed_constant(
    const scalar& alpha, const scalar& K,
    Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>& hess) {
  hess.template block<DIM, DIM>(0, 0) = K * Matrixs<DIM>::Identity();
  hess.template block<DIM, DIM>(0, DIM) =
      K * Matrixs<DIM>::Identity() * (alpha - 1.);
  hess.template block<DIM, DIM>(0, DIM * 2) =
      K * Matrixs<DIM>::Identity() * (-alpha);

  hess.template block<DIM, DIM>(DIM, 0) =
      K * Matrixs<DIM>::Identity() * (alpha - 1.);
  hess.template block<DIM, DIM>(DIM, DIM) =
      K * Matrixs<DIM>::Identity() * (1. - alpha) * (1. - alpha);
  hess.template block<DIM, DIM>(DIM, DIM * 2) =
      K * Matrixs<DIM>::Identity() * (1. - alpha) * alpha;
  ...
  hess.template block<DIM, DIM>(DIM * 2, DIM) =
      K * Matrixs<DIM>::Identity() * (1. - alpha) * alpha;
  hess.template block<DIM, DIM>(DIM * 2, DIM * 2) =
      K * Matrixs<DIM>::Identity() * alpha * alpha;
}
...
template <typename T>
inline void grad_pointline_dist(const Eigen::Matrix<T, 2, 1>& x0,
                                const Eigen::Matrix<T, 2, 1>& x1,
                                const Eigen::Matrix<T, 2, 1>& x2,
  ...
  grad(0) = t20 * (t4 * (t5 * t8 - 1.0) * 2.0 + t2 * t3 * t8 * t14 * 2.0) *
            (1.0 / 2.0);
  grad(1) = t20 * (t14 * (t6 * t8 - 1.0) * 2.0 + t2 * t3 * t4 * t8 * 2.0) *
            (1.0 / 2.0);
  grad(2) =
      t20 *
      (t4 *
           (t24 + t2 * t8 * (-t21 + x0(0) + x2(0)) - t2 * t13 * t22 * t23 +
            1.0) *
           2.0 +
       t14 * (t3 * t8 * (x0(0) - x1(0) * 2.0 + x2(0)) - t3 * t13 * t22 * t23) *
           2.0) *
      (1.0 / 2.0);
  grad(3) =
      t20 *
      (t14 *
           (t24 + t3 * t8 * (-t25 + x0(1) + x2(1)) - t3 * t13 * t23 * t26 +
            1.0) *
           2.0 +
       t4 * (t2 * t8 * (x0(1) - x1(1) * 2.0 + x2(1)) - t2 * t13 * t23 * t26) *
           2.0) *
      (1.0 / 2.0);
  grad(4) = t20 *
            (t14 * (t3 * t8 * t9 - t3 * t13 * t22 * t23) * 2.0 +
             t4 * (t24 + t2 * t8 * t9 - t2 * t13 * t22 * t23) * 2.0) *
            (-1.0 / 2.0);
  grad(5) = t20 *
            (t4 * (t2 * t8 * t11 - t2 * t13 * t23 * t26) * 2.0 +
             t14 * (t24 + t3 * t8 * t11 - t3 * t13 * t23 * t26) * 2.0) *
            (-1.0 / 2.0);
}

template <typename T>
inline void grad_pointline_dist(const Eigen::Matrix<T, 3, 1>& x0,
                                const Eigen::Matrix<T, 3, 1>& x1,
                                const Eigen::Matrix<T, 3, 1>& x2,
                                Eigen::Matrix<T, 9, 1>& gradE) {
  const scalar t2 = x1(0) - x2(0);
  const scalar t3 = x1(1) - x2(1);
  const scalar t4 = x1(2) - x2(2);
  const scalar t6 = t2 * t2;
  ...
  gradE(0) = t27 *
             (t5 * (t6 * t10 - 1.0) * 2.0 + t2 * t3 * t10 * t18 * 2.0 +
              t2 * t4 * t10 * t19 * 2.0) *
             (1.0 / 2.0);
  gradE(1) = t27 *
             (t18 * (t7 * t10 - 1.0) * 2.0 + t2 * t3 * t5 * t10 * 2.0 +
              t3 * t4 * t10 * t19 * 2.0) *
             (1.0 / 2.0);
  gradE(2) = t27 *
             (t19 * (t8 * t10 - 1.0) * 2.0 + t2 * t4 * t5 * t10 * 2.0 +
              t3 * t4 * t10 * t18 * 2.0) *
             (1.0 / 2.0);
  gradE(3) = t27 *
             (t18 * (t3 * t10 * t31 - t3 * t17 * t29 * t30) * 2.0 +
              t19 * (t4 * t10 * t31 - t4 * t17 * t29 * t30) * 2.0 +
              t5 *
                  (t33 + t2 * t10 * (x0(0) - x1(0) * 2.0 + x2(0)) -
                   t2 * t17 * t29 * t30 + 1.0) *
                  2.0) *
             (1.0 / 2.0);
  gradE(4) = t27 *
             (t5 * (t2 * t10 * t36 - t2 * t17 * t30 * t35) * 2.0 +
              t19 * (t4 * t10 * t36 - t4 * t17 * t30 * t35) * 2.0 +
              t18 *
                  (t33 + t3 * t10 * (x0(1) - x1(1) * 2.0 + x2(1)) -
                   t3 * t17 * t30 * t35 + 1.0) *
                  2.0) *
             (1.0 / 2.0);
  gradE(5) = t27 *
             (t5 * (t2 * t10 * t40 - t2 * t17 * t30 * t39) * 2.0 +
              t18 * (t3 * t10 * t40 - t3 * t17 * t30 * t39) * 2.0 +
              t19 *
                  (t33 + t4 * t10 * (x0(2) - x1(2) * 2.0 + x2(2)) -
                   t4 * t17 * t30 * t39 + 1.0) *
                  2.0) *
             (1.0 / 2.0);
  gradE(6) = t27 *
             (t18 * (t3 * t10 * t11 - t3 * t17 * t29 * t30) * 2.0 +
              t19 * (t4 * t10 * t11 - t4 * t17 * t29 * t30) * 2.0 +
              t5 * (t33 + t2 * t10 * t11 - t2 * t17 * t29 * t30) * 2.0) *
             (-1.0 / 2.0);
  gradE(7) = t27 *
             (t5 * (t2 * t10 * t13 - t2 * t17 * t30 * t35) * 2.0 +
              t19 * (t4 * t10 * t13 - t4 * t17 * t30 * t35) * 2.0 +
              t18 * (t33 + t3 * t10 * t13 - t3 * t17 * t30 * t35) * 2.0) *
             (-1.0 / 2.0);
  gradE(8) = t27 *
             (t5 * (t2 * t10 * t15 - t2 * t17 * t30 * t39) * 2.0 +
              t18 * (t3 * t10 * t15 - t3 * t17 * t30 * t39) * 2.0 +
              t19 * (t33 + t4 * t10 * t15 - t4 * t17 * t30 * t39) * 2.0) *
             (-1.0 / 2.0);
}

template <typename T>
inline void hess_pointline_dist(const Eigen::Matrix<T, 2, 1>& x0,
                                const Eigen::Matrix<T, 2, 1>& x1,
                                const Eigen::Matrix<T, 2, 1>& x2,
                                Eigen::Matrix<T, 6, 6>& hessE) {
  const scalar t2 = x1(0) - x2(0);
  const scalar t3 = x1(1) - x2(1);
  const scalar t5 = t2 * t2;
  const scalar t6 = t3 * t3;
  ...
  hessE(0, 0) = (t17 * t17) * t24 * (-1.0 / 4.0) +
                t23 * (t65 + (t22 * t22) * 2.0) * (1.0 / 2.0);
  hessE(0, 1) = t64;
  hessE(0, 2) =
      t23 *
          (t45 + t46 + t86 +
           t22 *
               (t36 + t2 * t8 * (x0(0) - x1(0) * 2.0 + x2(0)) -
                t2 * t13 * t29 * t31 + 1.0) *
               2.0 -
           t2 * t3 * t14 * t29 * t31 * 2.0) *
          (1.0 / 2.0) -
      t17 * t24 * (t68 + t4 * (t36 + t66 - t2 * t13 * t29 * t31 + 1.0) * 2.0) *
          (1.0 / 4.0);
  hessE(0, 3) =
      t23 *
          (t51 + t107 +
           t22 *
               (t2 * t8 * (x0(1) - x1(1) * 2.0 + x2(1)) -
                t2 * t13 * t29 * t38) *
               2.0 -
           t4 * t5 * t29 * t38 * 2.0 - t2 * t3 * t14 * t29 * t38 * 2.0) *
          (1.0 / 2.0) -
      t17 * t24 * (t73 + t4 * (t70 - t2 * t13 * t29 * t38) * 2.0) * (1.0 / 4.0);
  hessE(0, 4) = t130 - t23 *
                           (t45 + t46 + t129 - t2 * t3 * t8 * t48 * 2.0 -
                            t2 * t3 * t14 * t29 * t31 * 2.0) *
                           (1.0 / 2.0);
  hessE(0, 5) = t163 - t23 *
                           (t51 + t161 + t162 - t4 * t5 * t29 * t38 * 2.0 -
                            t2 * t3 * t14 * t29 * t38 * 2.0) *
                           (1.0 / 2.0);
  hessE(1, 0) = t64;
  hessE(1, 1) = t24 * (t63 * t63) * (-1.0 / 4.0) +
                t23 * (t65 + (t28 * t28) * 2.0) * (1.0 / 2.0);
  hessE(1, 2) = t23 *
                    (t75 + t89 + t93 - t6 * t14 * t29 * t31 * 2.0 -
                     t2 * t3 * t4 * t29 * t31 * 2.0) *
                    (1.0 / 2.0) -
                t24 * t63 * t88 * (1.0 / 4.0);
  hessE(1, 3) = t23 *
                    (t80 + t81 + t108 + t112 - t2 * t3 * t4 * t29 * t38 * 2.0) *
                    (1.0 / 2.0) -
                t24 * t63 * t97 * (1.0 / 4.0);
  hessE(1, 4) = t133 + t23 *
                           (-t75 + t92 + t132 - t2 * t3 * t8 * t50 * 2.0 +
                            t2 * t3 * t4 * t29 * t31 * 2.0) *
                           (1.0 / 2.0);
  hessE(1, 5) =
      t169 - t23 * (t80 + t81 + t167 + t168 - t2 * t3 * t4 * t29 * t38 * 2.0) *
                 (1.0 / 2.0);
  hessE(2, 0) =
      t23 * (t86 + t128 + t22 * t69 * 2.0 - t4 * (t85 - t2 * t8 * 2.0) * 2.0) *
          (1.0 / 2.0) -
      t17 * t24 * t88 * (1.0 / 4.0);
  hessE(2, 1) = t23 * (t89 - t92 + t93 + t131) * (1.0 / 2.0) -
                t24 * t63 * t88 * (1.0 / 4.0);
  hessE(2, 2) =
      t23 *
          (t4 *
               (t98 + t99 + t121 - t8 * t33 * 2.0 + t2 * t29 * t31 * t33 * 2.0 -
                t2 * t13 * t94 * t95 * 2.0) *
               2.0 +
           t14 *
               (t100 + t109 + t3 * t29 * t31 * t33 * 2.0 -
                t3 * t13 * t94 * t95 * 2.0) *
               2.0 -
           (t35 * t35) * 2.0 - (t69 * t69) * 2.0) *
          (-1.0 / 2.0) -
      t24 * (t88 * t88) * (1.0 / 4.0);
  hessE(2, 3) = t120;
  hessE(2, 4) = t142 + t23 *
                           (t137 + t141 +
                            t4 *
                                (t98 + t99 + t106 + t135 + t136 - t8 * t9 -
                                 t8 * t33 - t2 * t13 * t94 * t95 * 2.0) *
                                2.0 -
                            t50 * t69 * 2.0) *
                           (1.0 / 2.0);
  hessE(2, 5) = t172 - t23 *
                           (t170 + t171 -
                            t4 *
                                (t104 + t105 + t156 - t8 * t11 -
                                 t2 * t13 * t31 * t38 * t95 * 2.0) *
                                2.0 -
                            t14 *
                                (t102 + t103 + t158 - t8 * t33 -
                                 t3 * t13 * t31 * t38 * t95 * 2.0) *
                                2.0) *
                           (1.0 / 2.0);
  hessE(3, 0) =
      t23 * (t107 + t160 + t22 * t72 * 2.0 - t4 * t5 * t29 * t38 * 2.0) *
          (1.0 / 2.0) -
      t17 * t24 * t97 * (1.0 / 4.0);
  hessE(3, 1) = t23 * (t108 + t112 + t165 + t166) * (1.0 / 2.0) -
                t24 * t63 * t97 * (1.0 / 4.0);
  hessE(3, 2) = t120;
  hessE(3, 3) =
      t23 *
          (t14 *
               (t100 + t109 + t126 - t8 * t40 * 2.0 +
                t3 * t29 * t38 * t40 * 2.0 - t3 * t13 * t95 * t122 * 2.0) *
               2.0 +
           t4 *
               (t99 + t121 + t2 * t29 * t38 * t40 * 2.0 -
                t2 * t13 * t95 * t122 * 2.0) *
               2.0 -
           (t42 * t42) * 2.0 - (t72 * t72) * 2.0) *
          (-1.0 / 2.0) -
      t24 * (t97 * t97) * (1.0 / 4.0);
  hessE(3, 4) = t153;
  hessE(3, 5) = t183;
  hessE(4, 0) =
      t130 -
      t23 * (t128 + t129 - t4 * (t85 - t121) * 2.0 - t2 * t3 * t8 * t48 * 2.0) *
          (1.0 / 2.0);
  hessE(4, 1) =
      t133 + t23 * (t92 - t131 + t132 - t2 * t3 * t8 * t50 * 2.0) * (1.0 / 2.0);
  hessE(4, 2) = t142 + t23 *
                           (t137 + t141 - t50 * t69 * 2.0 +
                            t4 *
                                (t98 + t99 + t106 - t134 + t135 + t136 -
                                 t8 * t9 - t2 * t13 * t94 * t95 * 2.0) *
                                2.0) *
                           (1.0 / 2.0);
  hessE(4, 3) = t153;
  hessE(4, 4) =
      t23 *
          (t14 * (t100 - t155 + t3 * t9 * t29 * t31 * 2.0) * 2.0 +
           t4 *
               (t98 + t99 - t8 * t9 * 2.0 + t2 * t9 * t29 * t31 * 2.0 -
                t2 * t13 * t94 * t95 * 2.0) *
               2.0 -
           (t50 * t50) * 2.0 - (t154 * t154) * 2.0) *
          (-1.0 / 2.0) -
      t24 * (t77 * t77) * (1.0 / 4.0);
  hessE(4, 5) = t23 *
                    (t184 + t187 - t50 * t54 * 2.0 +
                     t4 * (t104 - t124 + t143 + t156 - t8 * t11) * 2.0) *
                    (-1.0 / 2.0) -
                t24 * t77 * t84 * (1.0 / 4.0);
  hessE(5, 0) = t163 - t23 * (t160 + t161 + t162 - t4 * t5 * t29 * t38 * 2.0) *
                           (1.0 / 2.0);
  hessE(5, 1) = t169 - t23 * (t165 + t166 + t167 + t168) * (1.0 / 2.0);
  hessE(5, 2) =
      t172 - t23 *
                 (t170 + t171 - t14 * (t102 + t103 - t125 - t134 + t158) * 2.0 -
                  t4 * (t104 + t105 - t124 + t156 - t8 * t11) * 2.0) *
                 (1.0 / 2.0);
  hessE(5, 3) = t183;
  hessE(5, 4) =
      t23 *
          (t184 + t187 + t4 * (t104 - t124 + t143 + t156 - t185) * 2.0 -
           t50 * t54 * 2.0) *
          (-1.0 / 2.0) -
      t24 * t77 * t84 * (1.0 / 4.0);
  hessE(5, 5) = t24 * (t84 * t84) * (-1.0 / 4.0) -
                t23 *
                    (t14 *
                         (t100 + t126 - t189 - t8 * t11 * 2.0 +
                          t3 * t11 * t29 * t38 * 2.0) *
                         2.0 +
                     t4 * (t99 - t188 + t2 * t11 * t29 * t38 * 2.0) * 2.0 -
                     (t54 * t54) * 2.0 - (t56 * t56) * 2.0) *
                    (1.0 / 2.0);
}

template <typename T>
inline void hess_pointline_dist(const Eigen::Matrix<T, 3, 1>& x0,
                                const Eigen::Matrix<T, 3, 1>& x1,
                                const Eigen::Matrix<T, 3, 1>& x2,
                                Eigen::Matrix<T, 9, 9>& hessE) {
  const scalar t2 = x1(0) - x2(0);
  const scalar t3 = x1(1) - x2(1);
  const scalar t4 = x1(2) - x2(2);
  const scalar t6 = t2 * t2;
  ...
  const scalar t227 = t220 + t223 + t226 - t46 * t54 * 2.0 - t48 * t56 * 2.0 -
                      t109 * t115 * 2.0;
  const scalar t228 =
      t31 * t227 * (-1.0 / 2.0) - t32 * t156 * t158 * (1.0 / 4.0);
  ...
  const scalar t311 =
      t31 * (t300 + t303 + t306 + t309 + t310 - t73 * t115 * 2.0) * (1.0 / 2.0);
  ...
  const scalar t364 =
      t31 * (t351 + t355 + t359 + t363 - t54 * t80 * 2.0 - t82 * t115 * 2.0) *
      (1.0 / 2.0);
  ...
  const scalar t423 =
      t31 * (t410 + t414 + t418 + t422 - t62 * t88 * 2.0 - t90 * t120 * 2.0) *
      (1.0 / 2.0);
  ...
  const scalar t429 =
      t188 - t314 + t315 + t333 - t4 * t17 * t41 * t58 * t187 * 2.0;
  ...
  hessE(0, 0) = t31 * (t102 + t153 + (t20 * t20) * 2.0) * (1.0 / 2.0) -
                (t30 * t30) * t32 * (1.0 / 4.0);
  hessE(0, 1) = t101;
  hessE(0, 2) = t146;
  hessE(0, 3) =
      t31 *
          (t67 + t68 + t69 + t170 + t171 +
           t20 *
               (t44 + t2 * t10 * (x0(0) - x1(0) * 2.0 + x2(0)) -
                t2 * t17 * t21 * t41 + 1.0) *
               2.0 -
           t2 * t3 * t18 * t21 * t41 * 2.0 - t2 * t4 * t19 * t21 * t41 * 2.0) *
          (1.0 / 2.0) -
      t30 * t32 *
          (t110 + t111 + t5 * (t44 + t107 - t2 * t17 * t21 * t41 + 1.0) * 2.0) *
          (1.0 / 4.0);
  hessE(0, 4) =
      t31 *
          (t76 + t202 + t204 +
           t20 *
               (t2 * t10 * (x0(1) - x1(1) * 2.0 + x2(1)) -
                t2 * t17 * t21 * t50) *
               2.0 -
           t5 * t6 * t21 * t50 * 2.0 - t2 * t3 * t18 * t21 * t50 * 2.0 -
           t2 * t4 * t19 * t21 * t50 * 2.0) *
          (1.0 / 2.0) -
      t30 * t32 * (t114 + t116 + t5 * (t112 - t2 * t17 * t21 * t50) * 2.0) *
          (1.0 / 4.0);
  hessE(0, 5) =
      t31 *
          (t84 + t240 + t242 +
           t20 *
               (t2 * t10 * (x0(2) - x1(2) * 2.0 + x2(2)) -
                t2 * t17 * t21 * t58) *
               2.0 -
           t5 * t6 * t21 * t58 * 2.0 - t2 * t3 * t18 * t21 * t58 * 2.0 -
           t2 * t4 * t19 * t21 * t58 * 2.0) *
          (1.0 / 2.0) -
      t30 * t32 * (t119 + t121 + t5 * (t117 - t2 * t17 * t21 * t58) * 2.0) *
          (1.0 / 4.0);
  hessE(0, 6) =
      t31 *
          (t67 + t68 + t69 + t277 - t2 * t3 * t10 * t74 * 2.0 -
           t2 * t4 * t10 * t75 * 2.0 - t2 * t3 * t18 * t21 * t41 * 2.0 -
           t2 * t4 * t19 * t21 * t41 * 2.0) *
          (-1.0 / 2.0) -
      t30 * t32 * t127 * (1.0 / 4.0);
  hessE(0, 7) =
      t337 + t31 *
                 (-t76 + t203 + t205 + t336 - t20 * t82 * 2.0 -
                  t2 * t3 * t10 * t80 * 2.0 + t2 * t3 * t18 * t21 * t50 * 2.0) *
                 (1.0 / 2.0);
  hessE(0, 8) =
      t394 + t31 *
                 (-t84 + t241 + t243 + t393 - t20 * t90 * 2.0 -
                  t2 * t4 * t10 * t88 * 2.0 + t2 * t4 * t19 * t21 * t58 * 2.0) *
                 (1.0 / 2.0);
  hessE(1, 0) = t101;
  hessE(1, 1) = t31 * (t102 + t154 + (t37 * t37) * 2.0) * (1.0 / 2.0) -
                t32 * (t100 * t100) * (1.0 / 4.0);
  hessE(1, 2) = t152;
  hessE(1, 3) =
      t31 *
          (t123 + t172 + t175 + t177 - t7 * t18 * t21 * t41 * 2.0 -
           t2 * t3 * t5 * t21 * t41 * 2.0 - t3 * t4 * t19 * t21 * t41 * 2.0) *
          (1.0 / 2.0) -
      t32 * t100 * t156 * (1.0 / 4.0);
  hessE(1, 4) =
      t31 *
          (t69 + t130 + t131 + t206 + t210 + t211 -
           t2 * t3 * t5 * t21 * t50 * 2.0 - t3 * t4 * t19 * t21 * t50 * 2.0) *
          (1.0 / 2.0) -
      t32 * t100 * t158 * (1.0 / 4.0);
  hessE(1, 5) =
      t31 *
          (t137 + t244 + t245 + t247 - t7 * t18 * t21 * t58 * 2.0 -
           t2 * t3 * t5 * t21 * t58 * 2.0 - t3 * t4 * t19 * t21 * t58 * 2.0) *
          (1.0 / 2.0) -
      t32 * t100 * t160 * (1.0 / 4.0);
  hessE(1, 6) =
      t31 *
          (-t123 + t176 + t178 + t283 + t284 - t2 * t3 * t10 * t73 * 2.0 +
           t2 * t3 * t5 * t21 * t41 * 2.0) *
          (1.0 / 2.0) -
      t32 * t100 * t127 * (1.0 / 4.0);
  hessE(1, 7) =
      t345 -
      t31 *
          (t69 + t130 + t131 + t340 + t344 - t3 * t4 * t10 * t83 * 2.0 -
           t2 * t3 * t5 * t21 * t50 * 2.0 - t3 * t4 * t19 * t21 * t50 * 2.0) *
          (1.0 / 2.0);
  hessE(1, 8) =
      t398 + t31 *
                 (-t137 + t246 + t248 + t397 - t2 * t3 * t10 * t90 * 2.0 -
                  t3 * t4 * t10 * t88 * 2.0 + t3 * t4 * t19 * t21 * t58 * 2.0) *
                 (1.0 / 2.0);
  hessE(2, 0) = t146;
  hessE(2, 1) = t152;
  hessE(2, 2) = t31 * (t153 + t154 + (t39 * t39) * 2.0) * (1.0 / 2.0) -
                t32 * (t106 * t106) * (1.0 / 4.0);
  hessE(2, 3) =
      t31 *
          (t161 + t179 + t182 + t184 - t8 * t19 * t21 * t41 * 2.0 -
           t2 * t4 * t5 * t21 * t41 * 2.0 - t3 * t4 * t18 * t21 * t41 * 2.0) *
          (1.0 / 2.0) -
      t32 * t106 * t156 * (1.0 / 4.0);
  hessE(2, 4) =
      t31 *
          (t163 + t212 + t214 + t216 - t8 * t19 * t21 * t50 * 2.0 -
           t2 * t4 * t5 * t21 * t50 * 2.0 - t3 * t4 * t18 * t21 * t50 * 2.0) *
          (1.0 / 2.0) -
      t32 * t106 * t158 * (1.0 / 4.0);
  hessE(2, 5) =
      t31 *
          (t68 + t131 + t167 + t249 + t253 + t254 -
           t2 * t4 * t5 * t21 * t58 * 2.0 - t3 * t4 * t18 * t21 * t58 * 2.0) *
          (1.0 / 2.0) -
      t32 * t106 * t160 * (1.0 / 4.0);
  hessE(2, 6) =
      t31 *
          (-t161 + t183 + t185 + t286 + t287 - t2 * t4 * t10 * t73 * 2.0 +
           t2 * t4 * t5 * t21 * t41 * 2.0) *
          (1.0 / 2.0) -
      t32 * t106 * t127 * (1.0 / 4.0);
  hessE(2, 7) =
      t348 + t31 *
                 (-t163 + t215 + t217 + t347 - t3 * t4 * t10 * t80 * 2.0 -
                  t2 * t4 * t10 * t82 * 2.0 + t3 * t4 * t18 * t21 * t50 * 2.0) *
                 (1.0 / 2.0);
  hessE(2, 8) =
      t405 -
      t31 *
          (t68 + t131 + t167 + t401 + t404 - t3 * t4 * t10 * t91 * 2.0 -
           t2 * t4 * t5 * t21 * t58 * 2.0 - t3 * t4 * t18 * t21 * t58 * 2.0) *
          (1.0 / 2.0);
  hessE(3, 0) = t31 *
                    (t170 + t171 + t279 + t281 + t20 * t109 * 2.0 -
                     t5 * (t169 - t2 * t10 * 2.0) * 2.0) *
                    (1.0 / 2.0) -
                t30 * t32 * t156 * (1.0 / 4.0);
  hessE(3, 1) = t31 * (t172 + t175 - t176 + t177 - t178 + t282) * (1.0 / 2.0) -
                t32 * t100 * t156 * (1.0 / 4.0);
  hessE(3, 2) = t31 * (t179 + t182 - t183 + t184 - t185 + t285) * (1.0 / 2.0) -
                t32 * t106 * t156 * (1.0 / 4.0);
  hessE(3, 3) =
      t32 * (t156 * t156) * (-1.0 / 4.0) -
      t31 *
          (t5 *
               (t191 + t192 + t229 - t10 * t43 * 2.0 +
                t2 * t21 * t41 * t43 * 2.0 - t2 * t17 * t186 * t187 * 2.0) *
               2.0 +
           t18 *
               (t189 + t207 + t3 * t21 * t41 * t43 * 2.0 -
                t3 * t17 * t186 * t187 * 2.0) *
               2.0 +
           t19 *
               (t190 + t231 + t4 * t21 * t41 * t43 * 2.0 -
                t4 * t17 * t186 * t187 * 2.0) *
               2.0 -
           (t46 * t46) * 2.0 - (t48 * t48) * 2.0 - (t109 * t109) * 2.0) *
          (1.0 / 2.0);
  hessE(3, 4) = t228;
  hessE(3, 5) =
      t31 *
          (t257 + t260 - t46 * t64 * 2.0 - t48 * t62 * 2.0 - t109 * t120 * 2.0 +
           t19 *
               (t188 + t200 + t262 - t10 * t43 -
                t4 * t17 * t41 * t58 * t187 * 2.0) *
               2.0) *
          (-1.0 / 2.0) -
      t32 * t156 * t160 * (1.0 / 4.0);
  hessE(3, 6) = t31 *
                    (t288 + t289 + t293 + t297 +
                     t5 *
                         (t191 + t192 + t201 + t298 + t299 - t10 * t11 -
                          t10 * t43 - t2 * t17 * t186 * t187 * 2.0) *
                         2.0 -
                     t73 * t109 * 2.0) *
                    (1.0 / 2.0) -
                t32 * t127 * t156 * (1.0 / 4.0);
  hessE(3, 7) =
      t350 +
      t31 *
          (t349 - t46 * t80 * 2.0 - t82 * t109 * 2.0 +
           t5 *
               (t193 + t194 + t329 - t10 * t13 -
                t2 * t17 * t41 * t50 * t187 * 2.0) *
               2.0 +
           t18 *
               (t188 + t196 + t330 - t10 * t43 -
                t3 * t17 * t41 * t50 * t187 * 2.0) *
               2.0 +
           t19 * (t195 + t328 - t4 * t17 * t41 * t50 * t187 * 2.0) * 2.0) *
          (1.0 / 2.0);
  hessE(3, 8) =
      t407 +
      t31 *
          (t406 - t48 * t88 * 2.0 - t90 * t109 * 2.0 +
           t5 *
               (t197 + t198 + t332 - t10 * t15 -
                t2 * t17 * t41 * t58 * t187 * 2.0) *
               2.0 +
           t19 *
               (t188 + t200 + t333 - t10 * t43 -
                t4 * t17 * t41 * t58 * t187 * 2.0) *
               2.0 +
           t18 * (t199 + t331 - t3 * t17 * t41 * t58 * t187 * 2.0) * 2.0) *
          (1.0 / 2.0);
  hessE(4, 0) = t31 * (t202 - t203 + t204 - t205 + t335 + t20 * t115 * 2.0) *
                    (1.0 / 2.0) -
                t30 * t32 * t158 * (1.0 / 4.0);
  hessE(4, 1) = t31 * (t206 + t210 + t211 + t339 + t341 + t343) * (1.0 / 2.0) -
                t32 * t100 * t158 * (1.0 / 4.0);
  hessE(4, 2) = t31 * (t212 + t214 - t215 + t216 - t217 + t346) * (1.0 / 2.0) -
                t32 * t106 * t158 * (1.0 / 4.0);
  hessE(4, 3) = t228;
  hessE(4, 4) =
      t32 * (t158 * t158) * (-1.0 / 4.0) -
      t31 *
          (t18 *
               (t189 + t207 + t236 - t10 * t52 * 2.0 +
                t3 * t21 * t50 * t52 * 2.0 - t3 * t17 * t187 * t230 * 2.0) *
               2.0 +
           t5 *
               (t191 + t229 + t2 * t21 * t50 * t52 * 2.0 -
                t2 * t17 * t187 * t230 * 2.0) *
               2.0 +
           t19 *
               (t190 + t231 + t4 * t21 * t50 * t52 * 2.0 -
                t4 * t17 * t187 * t230 * 2.0) *
               2.0 -
           (t54 * t54) * 2.0 - (t56 * t56) * 2.0 - (t115 * t115) * 2.0) *
          (1.0 / 2.0);
  hessE(4, 5) =
      t31 *
          (t265 + t270 - t54 * t64 * 2.0 - t56 * t62 * 2.0 - t115 * t120 * 2.0 +
           t18 *
               (t197 + t237 + t267 - t10 * t60 -
                t3 * t17 * t50 * t58 * t187 * 2.0) *
               2.0) *
          (-1.0 / 2.0) -
      t32 * t158 * t160 * (1.0 / 4.0);
  hessE(4, 6) = t312;
  hessE(4, 7) = t366;
  hessE(4, 8) =
      t409 +
      t31 *
          (t408 +
           t19 *
               (t193 - t232 + t239 + t390 - t4 * t17 * t50 * t58 * t187 * 2.0) *
               2.0 -
           t56 * t88 * 2.0 - t90 * t115 * 2.0 +
           t18 *
               (t197 + t237 + t388 - t10 * t15 -
                t3 * t17 * t50 * t58 * t187 * 2.0) *
               2.0 +
           t5 * (t238 + t387 - t2 * t17 * t50 * t58 * t187 * 2.0) * 2.0) *
          (1.0 / 2.0);
  hessE(5, 0) = t31 * (t240 - t241 + t242 - t243 + t392 + t20 * t120 * 2.0) *
                    (1.0 / 2.0) -
                t30 * t32 * t160 * (1.0 / 4.0);
  hessE(5, 1) = t31 * (t244 + t245 - t246 + t247 - t248 + t396) * (1.0 / 2.0) -
                t32 * t100 * t160 * (1.0 / 4.0);
  hessE(5, 2) = t31 * (t249 + t253 + t254 + t400 + t402 + t403) * (1.0 / 2.0) -
                t32 * t106 * t160 * (1.0 / 4.0);
  hessE(5, 3) =
      t31 *
          (t257 + t260 +
           t19 *
               (t188 + t200 - t261 + t262 - t4 * t17 * t41 * t58 * t187 * 2.0) *
               2.0 -
           t46 * t64 * 2.0 - t48 * t62 * 2.0 - t109 * t120 * 2.0) *
          (-1.0 / 2.0) -
      t32 * t156 * t160 * (1.0 / 4.0);
  hessE(5, 4) =
      t31 *
          (t265 + t270 +
           t18 *
               (t197 + t237 - t266 + t267 - t3 * t17 * t50 * t58 * t187 * 2.0) *
               2.0 -
           t54 * t64 * 2.0 - t56 * t62 * 2.0 - t115 * t120 * 2.0) *
          (-1.0 / 2.0) -
      t32 * t158 * t160 * (1.0 / 4.0);
  hessE(5, 5) =
      t32 * (t160 * t160) * (-1.0 / 4.0) -
      t31 *
          (t19 *
               (t190 + t231 + t276 - t10 * t60 * 2.0 +
                t4 * t21 * t58 * t60 * 2.0 - t4 * t17 * t187 * t271 * 2.0) *
               2.0 +
           t18 *
               (t189 + t207 + t3 * t21 * t58 * t60 * 2.0 -
                t3 * t17 * t187 * t271 * 2.0) *
               2.0 +
           t5 *
               (t191 + t229 + t2 * t21 * t58 * t60 * 2.0 -
                t2 * t17 * t187 * t271 * 2.0) *
               2.0 -
           (t62 * t62) * 2.0 - (t64 * t64) * 2.0 - (t120 * t120) * 2.0) *
          (1.0 / 2.0);
  hessE(5, 6) = t31 *
                    (t313 + t318 + t321 + t322 - t73 * t120 * 2.0 +
                     t19 *
                         (t188 + t262 + t315 - t10 * t11 -
                          t4 * t17 * t41 * t58 * t187 * 2.0) *
                         2.0) *
                    (1.0 / 2.0) -
                t32 * t127 * t160 * (1.0 / 4.0);
  hessE(5, 7) =
      t376 + t31 *
                 (t371 + t374 + t375 - t64 * t80 * 2.0 - t82 * t120 * 2.0 +
                  t19 * (t193 + t268 - t274 + t368 - t10 * t13) * 2.0) *
                 (1.0 / 2.0);
  hessE(5, 8) = t425;
  hessE(6, 0) = t31 *
                    (t277 + t279 + t281 - t5 * (t169 - t229) * 2.0 -
                     t2 * t3 * t10 * t74 * 2.0 - t2 * t4 * t10 * t75 * 2.0) *
                    (-1.0 / 2.0) -
                t30 * t32 * t127 * (1.0 / 4.0);
  hessE(6, 1) =
      t31 * (t176 + t178 - t282 + t283 + t284 - t2 * t3 * t10 * t73 * 2.0) *
          (1.0 / 2.0) -
      t32 * t100 * t127 * (1.0 / 4.0);
  hessE(6, 2) =
      t31 * (t183 + t185 - t285 + t286 + t287 - t2 * t4 * t10 * t73 * 2.0) *
          (1.0 / 2.0) -
      t32 * t106 * t127 * (1.0 / 4.0);
  hessE(6, 3) = t31 *
                    (t288 + t289 + t293 + t297 - t73 * t109 * 2.0 +
                     t5 *
                         (t191 + t192 + t201 - t261 + t298 + t299 - t10 * t11 -
                          t2 * t17 * t186 * t187 * 2.0) *
                         2.0) *
                    (1.0 / 2.0) -
                t32 * t127 * t156 * (1.0 / 4.0);
  hessE(6, 4) = t312;
  hessE(6, 5) =
      t31 *
          (t313 + t318 + t321 + t322 +
           t19 *
               (t188 + t262 - t314 + t315 - t4 * t17 * t41 * t58 * t187 * 2.0) *
               2.0 -
           t73 * t120 * 2.0) *
          (1.0 / 2.0) -
      t32 * t127 * t160 * (1.0 / 4.0);
  hessE(6, 6) =
      t32 * (t323 * t323) * (-1.0 / 4.0) -
      t31 *
          (t18 * (t189 - t326 + t3 * t11 * t21 * t41 * 2.0) * 2.0 +
           t19 * (t190 - t327 + t4 * t11 * t21 * t41 * 2.0) * 2.0 +
           t5 *
               (t191 + t192 - t10 * t11 * 2.0 + t2 * t11 * t21 * t41 * 2.0 -
                t2 * t17 * t186 * t187 * 2.0) *
               2.0 -
           (t73 * t73) * 2.0 - (t324 * t324) * 2.0 - (t325 * t325) * 2.0) *
          (1.0 / 2.0);
  hessE(6, 7) =
      t382 - t31 *
                 (t378 + t380 + t381 - t73 * t82 * 2.0 - t75 * t83 * 2.0 +
                  t5 * (t193 - t235 + t307 + t329 - t10 * t13) * 2.0) *
                 (1.0 / 2.0);
  hessE(6, 8) =
      t432 - t31 *
                 (t427 + t430 + t431 - t73 * t90 * 2.0 - t74 * t91 * 2.0 +
                  t5 * (t197 - t273 + t319 + t332 - t10 * t15) * 2.0) *
                 (1.0 / 2.0);
  hessE(7, 0) = t337 + t31 *
                           (t203 + t205 - t335 + t336 - t20 * t82 * 2.0 -
                            t2 * t3 * t10 * t80 * 2.0) *
                           (1.0 / 2.0);
  hessE(7, 1) =
      t345 -
      t31 * (t339 + t340 + t341 + t343 + t344 - t3 * t4 * t10 * t83 * 2.0) *
          (1.0 / 2.0);
  hessE(7, 2) =
      t348 + t31 *
                 (t215 + t217 - t346 + t347 - t3 * t4 * t10 * t80 * 2.0 -
                  t2 * t4 * t10 * t82 * 2.0) *
                 (1.0 / 2.0);
  hessE(7, 3) =
      t350 + t31 *
                 (t349 + t18 * (t188 + t196 - t233 - t261 + t330) * 2.0 -
                  t46 * t80 * 2.0 - t82 * t109 * 2.0 +
                  t5 * (t193 + t194 - t235 + t329 - t10 * t13) * 2.0 +
                  t19 * (t195 - t234 + t328) * 2.0) *
                 (1.0 / 2.0);
  hessE(7, 4) = t366;
  hessE(7, 5) = t376 + t31 *
                           (t371 + t374 + t375 +
                            t19 * (t193 + t268 - t274 - t367 + t368) * 2.0 -
                            t64 * t80 * 2.0 - t82 * t120 * 2.0) *
                           (1.0 / 2.0);
  hessE(7, 6) = t382 - t31 *
                           (t378 + t380 + t381 +
                            t5 * (t193 - t235 + t307 + t329 - t367) * 2.0 -
                            t73 * t82 * 2.0 - t75 * t83 * 2.0) *
                           (1.0 / 2.0);
  hessE(7, 7) =
      t31 *
          (t18 *
               (t189 + t236 - t384 - t10 * t13 * 2.0 +
                t3 * t13 * t21 * t50 * 2.0) *
               2.0 +
           t5 * (t191 - t385 + t2 * t13 * t21 * t50 * 2.0) * 2.0 +
           t19 * (t190 - t386 + t4 * t13 * t21 * t50 * 2.0) * 2.0 -
           (t80 * t80) * 2.0 - (t82 * t82) * 2.0 - (t383 * t383) * 2.0) *
          (-1.0 / 2.0) -
      t32 * (t135 * t135) * (1.0 / 4.0);
  hessE(7, 8) = t31 *
                    (t434 + t436 + t437 + t438 - t82 * t90 * 2.0 +
                     t18 * (t197 + t372 + t388 - t389 - t10 * t15) * 2.0) *
                    (-1.0 / 2.0) -
                t32 * t135 * t140 * (1.0 / 4.0);
  hessE(8, 0) = t394 + t31 *
                           (t241 + t243 - t392 + t393 - t20 * t90 * 2.0 -
                            t2 * t4 * t10 * t88 * 2.0) *
                           (1.0 / 2.0);
  hessE(8, 1) =
      t398 + t31 *
                 (t246 + t248 - t396 + t397 - t2 * t3 * t10 * t90 * 2.0 -
                  t3 * t4 * t10 * t88 * 2.0) *
                 (1.0 / 2.0);
  hessE(8, 2) =
      t405 -
      t31 * (t400 + t401 + t402 + t403 + t404 - t3 * t4 * t10 * t91 * 2.0) *
          (1.0 / 2.0);
  hessE(8, 3) =
      t407 +
      t31 *
          (t406 +
           t19 *
               (t188 + t200 - t261 + t333 - t4 * t17 * t41 * t58 * t187 * 2.0) *
               2.0 -
           t48 * t88 * 2.0 - t90 * t109 * 2.0 +
           t5 * (t197 + t198 - t273 + t332 - t10 * t15) * 2.0 +
           t18 * (t199 - t272 + t331) * 2.0) *
          (1.0 / 2.0);
  hessE(8, 4) =
      t409 + t31 *
                 (t408 + t19 * (t193 - t232 + t239 - t274 + t390) * 2.0 -
                  t56 * t88 * 2.0 - t90 * t115 * 2.0 +
                  t18 * (t197 + t237 + t388 - t389 - t10 * t15) * 2.0 +
                  t5 * (t238 - t275 + t387) * 2.0) *
                 (1.0 / 2.0);
  hessE(8, 5) = t425;
  hessE(8, 6) = t432 - t31 *
                           (t427 + t430 + t431 +
                            t5 * (t197 - t273 + t319 + t332 - t428) * 2.0 -
                            t73 * t90 * 2.0 - t74 * t91 * 2.0) *
                           (1.0 / 2.0);
  hessE(8, 7) =
      t31 *
          (t434 + t436 + t437 + t438 +
           t18 * (t197 + t372 + t388 - t389 - t428) * 2.0 - t82 * t90 * 2.0) *
          (-1.0 / 2.0) -
      t32 * t135 * t140 * (1.0 / 4.0);
  hessE(8, 8) =
      t31 *
          (t19 *
               (t190 + t276 - t440 - t10 * t15 * 2.0 +
                t4 * t15 * t21 * t58 * 2.0) *
               2.0 +
           t5 * (t191 - t441 + t2 * t15 * t21 * t58 * 2.0) * 2.0 +
           t18 * (t189 - t442 + t3 * t15 * t21 * t58 * 2.0) * 2.0 -
           (t88 * t88) * 2.0 - (t90 * t90) * 2.0 - (t439 * t439) * 2.0) *
          (-1.0 / 2.0) -
      t32 * (t140 * t140) * (1.0 / 4.0);
}
...
template <typename T>
inline T det3(const Eigen::Matrix<T, 3, 1>& a, const Eigen::Matrix<T, 3, 1>& v1,
...
template <typename T>
inline void closestDistanceBetweenLines(
    const Eigen::Matrix<T, 3, 1>& a0, const Eigen::Matrix<T, 3, 1>& a1,
    const Eigen::Matrix<T, 3, 1>& b0, const Eigen::Matrix<T, 3, 1>& b1,
    Eigen::Matrix<T, 3, 1>& pA, Eigen::Matrix<T, 3, 1>& pB, T& d,
    bool clampAll = true, bool clampA0 = true, bool clampA1 = true,
    bool clampB0 = true, bool clampB1 = true) {
  ...
  // If denominator is 0, lines are parallel: Calculate distance with a
  // projection and evaluate clamp edge cases
  if (denom == 0) {
    auto d0 = _A.dot(b0 - a0);

    d = (_A * d0 + a0 - b0).norm();

    // If clamping: the only time we'll get closest points will be when lines
    // don't overlap at all. Find if segments overlap using dot products.
    ...
    // If clamping is off, or segments overlapped, we have infinite results,
...
// DDG operators
template <int DIM>
inline scalar compute_edge_area(const Vectors<DIM>& e0,
                                const Vectors<DIM>& e1) {
  return (e1 - e0).norm();
}
...
inline void compute_edge_area(const VectorXs& x,
  ...
  for (int i = 0; i < ne; ++i) {
    area(i) =
        compute_edge_area<DIM>(x.segment<DIM>(scene->getDof(edges[i].first)),
                               x.segment<DIM>(scene->getDof(edges[i].second)));
  }
}
...
inline void compute_radius_vector(const VectorXs& radii,
...
inline void compute_flat_bracket_matrix(const VectorXs& vertex_value,
                                        SparseXs& Gv) {
  int np = vertex_value.size();
...
inline void compute_bracket_matrix(const VectorXs& vertex_value, SparseXs& Gf,
                                   int dim) {
...
inline void compute_inv_bracket_matrix(const VectorXs& vertex_value,
                                       SparseXs& iGv, scalar min_val = -1e+20) {
  int np = vertex_value.size();
...
inline void parallel_compute_inv_bracket_matrix(const VectorXs& vertex_value,
                                                SparseXs& iGv,
                                                scalar min_val = -1e+20) {
  int np = vertex_value.size();
...
inline void compute_vertex_divergence_matrix(const SparseXs& iGv,
                                             const SparseXs& Gf,
                                             const SparseXs& grad,
                                             SparseXs& div) {
...
inline void compute_vertex_laplacian_matrix(const SparseXs& grad,
                                            const SparseXs& div, SparseXs& L) {
  L = div * grad;
}
...
inline void compute_edge_to_vertex_matrix(
  ...
  for (int j = 0; j < ne; ++j) {
    ...
    if (N_Vi0 > 0)
      tri.push_back(Triplets(i0, j, A_Fj / (A_Vi0 * (scalar)N_Vi0)));
    if (N_Vi1 > 0)
      tri.push_back(Triplets(i1, j, A_Fj / (A_Vi1 * (scalar)N_Vi1)));
  }
...
inline void compute_mass_vector_solid(const VectorXs& rho_hair,
                                      ...
                                      const VectorXs& radii, VectorXs& m,
                                      TwoDScene<DIM>* scene) {
  int np = indices.size();
  for (int i = 0; i < np; ++i) {
    int idx = indices[i];
    scalar masshair =
        rho_hair(idx) * M_PI * radii[idx] * radii[idx] * area_v(i);
...
inline void compute_next_pressure(const SparseXs& L, const VectorXs& eta,
                                  const scalar& sigma, VectorXs& pressure) {
  pressure = -sigma * (L * eta);
}

inline void compute_D_matrix(const SparseXs& W_fv, const VectorXs& u_next,
                             const SparseXs& gradF, const SparseXs& dir_f_exp,
                             SparseXs& D) {
...
inline void compute_A_matrix(const SparseXs& D, const SparseXs& divV,
                             const VectorXs& u_next, const SparseXs& dir_f_exp,
                             const SparseXs& L, const scalar& mu, SparseXs& A) {
...
inline void compute_mass_vector_totalliquid(
    const VectorXs& eta, const std::vector<int>& indices,
    const VectorXs& area_v, const VectorXs& radii, const scalar& rho,
    const scalar& H, VectorXs& m, TwoDScene<DIM>* scene) {
  int np = indices.size();
  for (int i = 0; i < np; ++i) {
    int idx = indices[i];
    ...
    scalar massliquid =
        rho * M_PI *
        std::max(0.0, realeta * realeta - radii[idx] * radii[idx]) * area_v(i);
...
template <int DIM>
inline scalar compute_hair_length_geodesic(const std::vector<int>& hair,
                                           const VectorXs& x, int start_i,
                                           const scalar& start_alpha, int end_i,
                                           const scalar& end_alpha,
                                           TwoDScene<DIM>* scene) {
  ...
  scalar cut_start_len = (x.segment<DIM>(scene->getDof(ipnext)) -
                          x.segment<DIM>(scene->getDof(ipstart)))
                             .norm() *
                         start_alpha;
  ...
  if (end_i < (int)hair.size() - 1) {
    int inext = end_i + 1;
    cut_end_len = (x.segment<DIM>(scene->getDof(hair[inext])) -
                   x.segment<DIM>(scene->getDof(ipend)))
                      .norm() *
                  (1.0 - end_alpha);
  }
...
inline scalar compute_point_edge_closest_alpha(const Vector2s& x1,
                                               const Vector2s& x2,
                                               const Vector2s& x3) {
...
inline Vector2s compute_point_edge_closest_vector(const Vector2s& x1,
                                                  const Vector2s& x2,
                                                  const Vector2s& x3) {
  scalar alpha = (x1 - x2).dot(x3 - x2) / (x3 - x2).dot(x3 - x2);
...
inline void compute_cwiseProduct(VectorXs& b, const VectorXs& x,
                                 const VectorXs& y) {
  scalar* b_ptr = b.data();
  ...
  threadutils::thread_pool::ParallelFor(
      0, (int)x.size(), [&](int k) { b_ptr[k] = x_ptr[k] * y_ptr[k]; });
}
...
inline void accumulate_cwiseProduct(VectorXs& b, const VectorXs& x) {
  ...
  threadutils::thread_pool::ParallelFor(0, (int)x.size(),
                                        [&](int k) { b_ptr[k] *= x_ptr[k]; });
}

inline void computeJTPhi_coeff_add(VectorXs& b, const VectorXs& a,
                                   const scalar& c, const VectorXs& x,
                                   const SparseXs& JT) {
...
inline void computeJTPhi_coeff_add(VectorXs& b, const VectorXs& a,
                                   const VectorXs& c, const VectorXs& x,
                                   const SparseXs& JT) {
  const scalar* a_ptr = a.data();
...
inline void computeJTPhi_coeff(VectorXs& b, const VectorXs& c,
                               const VectorXs& x, const SparseXs& JT) {
  scalar* b_ptr = b.data();
  const scalar* c_ptr = c.data();
  threadutils::thread_pool::ParallelFor(0, (int)JT.outerSize(), [&](int k) {
    scalar sum = 0.0;
...
inline void computeJTPhi_coeff(MatrixXs& b, const VectorXs& c,
                               const VectorXs& x, const SparseXs& JT,
                               int col_idx) {
...
inline void accumulateJTPhi_coeff(VectorXs& b, const VectorXs& c,
                                  const VectorXs& x, const SparseXs& JT) {
  scalar* b_ptr = b.data();
  const scalar* c_ptr = c.data();
  threadutils::thread_pool::ParallelFor(0, (int)JT.outerSize(), [&](int k) {
...
inline void computeJTPhi_coeff(VectorXs& b, const scalar& c, const VectorXs& x,
                               const SparseXs& JT) {
  scalar* b_ptr = b.data();
  threadutils::thread_pool::ParallelFor(0, (int)JT.outerSize(), [&](int k) {
    scalar sum = 0.0;
...
inline void computeJTPhi_coeff(MatrixXs& b, const scalar& c, const VectorXs& x,
                               const SparseXs& JT, int col_idx) {
  scalar* b_ptr = b.data() + col_idx * b.rows();
  threadutils::thread_pool::ParallelFor(0, (int)JT.outerSize(), [&](int k) {
    scalar sum = 0.0;
    for (SparseXs::InnerIterator it(JT, k); it; ++it) {
...
inline void accumulateJTPhi_coeff(VectorXs& b, const scalar& c,
                                  const VectorXs& x, const SparseXs& JT) {
  scalar* b_ptr = b.data();
  threadutils::thread_pool::ParallelFor(0, (int)JT.outerSize(), [&](int k) {
    scalar sum = 0.0;
