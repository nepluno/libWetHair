...
#ifndef LIBWETHAIR_CORE_FLUIDSIM2D_H_
#define LIBWETHAIR_CORE_FLUIDSIM2D_H_
...
class FluidSim2D : public FluidSim {
 public:
  ...
  FluidSim2D(const Vector2s& origin_, scalar width, int ni_, int nj_,
             const scalar& dz_, const std::vector<Boundary<2>*>& boundaries_,
  ...
  Vector2s get_velocity(const Vector2s& position, const Array2s& u,
                        const Array2s& v) const;
