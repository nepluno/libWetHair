...
#ifndef LIBWETHAIR_CORE_HAIR_FLOW_H_
#define LIBWETHAIR_CORE_HAIR_FLOW_H_
...
class HairFlow {
 ...
 public:
  enum STATE { NORMAL, ENDPOINT, STAR };

  HairFlow(TwoDScene<DIM>* parent, const std::vector<int>& involved_particles,
           const VectorXs& eta,
  ...
  virtual void updateFromFilteredGrid(const VectorXs& x, VectorXs& v,
                                      FluidSim* fluidsim, const scalar& dt) = 0;
  ...
  virtual void updateReservoir(FluidSim* fluidsim, const VectorXs& x,
                               const VectorXs& v, const scalar& dt) = 0;
  ...
  virtual Vector3s computeHairLiquidAngularMomentum(
      const VectorXs& x, const VectorXs& v, FluidSim* fluidsim) const = 0;
  ...
  virtual void setConstraintParameters(const Vector6i& start,
                                       const Vector6i& num);
