...
template <int DIM>
CylindricalShallowFlow<DIM>::CylindricalShallowFlow(
    TwoDScene<DIM>* parent, const std::vector<int>& involved_particles,
    const VectorXs& eta, const std::vector<unsigned char>& particle_state)
  ...
  for (int i = 0; i < ne; ++i) {
    ...
    if (itr0 != HairFlow<DIM>::m_global_to_local.end() &&
        itr1 != HairFlow<DIM>::m_global_to_local.end()) {
      HairFlow<DIM>::m_edge_indices.push_back(i);
      HairFlow<DIM>::m_global_edges.push_back(e);
      HairFlow<DIM>::m_internal_edges.push_back(
          std::pair<int, int>(itr0->second, itr1->second));
  ...
  copy_rest_mass<DIM>(HairFlow<DIM>::m_parent->getHairRestMass(),
                      HairFlow<DIM>::m_particle_indices, m_mass_v,
                      HairFlow<DIM>::m_parent);

  compute_edge_val(HairFlow<DIM>::m_eta, HairFlow<DIM>::m_internal_edges,
                   HairFlow<DIM>::m_edge_eta);

  compute_rest_porosity(HairFlow<DIM>::m_eta,
                        HairFlow<DIM>::m_parent->getRadii(), involved_particles,
                        HairFlow<DIM>::m_porosity);
  m_pool_liquid = 0.0;
}

template <int DIM>
void CylindricalShallowFlow<DIM>::updateGeometricState(const VectorXs& x,
                                                       const VectorXs& v,
                                                       FluidSim* fluidsim) {
  // compute geometry
  m_old_rad_vec = HairFlow<DIM>::m_rad_vec;
  m_old_area_v = HairFlow<DIM>::m_area_v;
  const PolygonalCohesion<DIM>* cohesion =
      HairFlow<DIM>::m_parent->getPolygonalCohesion();
  ...
  compute_radius_vector(HairFlow<DIM>::m_parent->getEdgeRadii(),
                        HairFlow<DIM>::m_edge_indices,
                        HairFlow<DIM>::m_edge_rad_vec);

  if (HairFlow<DIM>::m_parent->isIndividualTransfer()) {
    compute_edge_area<DIM>(x, HairFlow<DIM>::m_global_edges,
                           HairFlow<DIM>::m_area_e, HairFlow<DIM>::m_parent);

    compute_vertex_area<DIM>(HairFlow<DIM>::m_internal_edges,
                             HairFlow<DIM>::m_area_e,
                             HairFlow<DIM>::m_area_v_hair);
    ...
    for (int i = 0; i < ne; ++i)
      HairFlow<DIM>::m_area_e(i) =
          area_e_global(HairFlow<DIM>::m_edge_indices[i]);

    for (int i = 0; i < np; ++i)
      HairFlow<DIM>::m_area_v(i) =
          area_v_global(HairFlow<DIM>::m_particle_indices[i]);
  ...
  compute_edge_to_vertex_matrix(HairFlow<DIM>::m_internal_edges,
                                HairFlow<DIM>::m_area_v,
                                HairFlow<DIM>::m_area_e, m_W_fv);
  compute_edge_to_vertex_matrix(HairFlow<DIM>::m_internal_edges,
                                HairFlow<DIM>::m_area_v_hair,
                                HairFlow<DIM>::m_area_e, m_W_fv_hair);

  // compute dirs
  compute_edge_dir<DIM>(x, HairFlow<DIM>::m_global_edges,
                        HairFlow<DIM>::m_dir_f, HairFlow<DIM>::m_parent);
  expand_dir_f(HairFlow<DIM>::m_dir_f, m_dir_f_expand);
  compute_vertex_val(HairFlow<DIM>::m_dir_f, m_W_fv_hair,
                     HairFlow<DIM>::m_dir_v);
  compute_normalized_vector<DIM>(HairFlow<DIM>::m_dir_v);

  compute_vertex_val(HairFlow<DIM>::m_edge_rad_vec, m_W_fv_hair,
                     HairFlow<DIM>::m_rad_vec);
  // compute local gradient matrix
  compute_edge_gradient_matrix<DIM>(
      x, HairFlow<DIM>::m_global_edges, HairFlow<DIM>::m_internal_edges,
      HairFlow<DIM>::m_area_e, m_gradF, HairFlow<DIM>::m_parent);
  ...
  compute_inverted_pos_mapping(m_area_e_accu, m_sum_area_e,
                               m_area_e_inv_mapping);
  compute_edge_val(m_area_e_accu, HairFlow<DIM>::m_internal_edges, m_edge_x);

  // compute local porosity
  // compute_rest_porosity(HairFlow<DIM>::m_eta,
  // HairFlow<DIM>::m_parent->getRadii(), HairFlow<DIM>::m_particle_indices,
  // HairFlow<DIM>::m_porosity);
  compute_edge_val(HairFlow<DIM>::m_porosity, HairFlow<DIM>::m_internal_edges,
                   m_porosity_e);
  HairFlow<DIM>::m_avg_area_e = compute_avg(HairFlow<DIM>::m_area_e);
...
template <int DIM>
void CylindricalShallowFlow<DIM>::updateReservoir(FluidSim* fluidsim,
                                                  const VectorXs& x,
                                                  const VectorXs& v,
                                                  const scalar& dt) {
  if (fluidsim) {
    if (HairFlow<DIM>::m_parent->isIndividualTransfer()) {
      ...
      for (int i = 0; i < ne; ++i) {
        ...
        m_pool_liquid +=
            M_PI * (Q0 + Q1 - dA0 - dA1) * HairFlow<DIM>::m_area_e(i) * 0.5;
...
template <>
void CylindricalShallowFlow<2>::updateToFilteredGrid(const VectorXs& x,
                                                     const VectorXs& v,
                                                     FluidSim* fluidsim,
                                                     const scalar& dt,
                                                     int ibuffer) {
  ...
  for (int eidx = 0; eidx < ne; ++eidx) {
    ...
    int xmin_u = std::max(
        0, std::min(fluid2d->get_u_ni() - 1,
                    (int)ceil((xmin - fluid2d->get_origin()(0)) / cellsize)));
    int xmin_v = std::max(
        0, std::min(
               fluid2d->get_v_ni() - 1,
               (int)ceil((xmin - fluid2d->get_origin()(0)) / cellsize - 0.5)));
    int xmax_u = std::max(
        0, std::min(fluid2d->get_u_ni() - 1,
                    (int)floor((xmax - fluid2d->get_origin()(0)) / cellsize)));
    int xmax_v = std::max(
        0, std::min(
               fluid2d->get_v_ni() - 1,
               (int)floor((xmax - fluid2d->get_origin()(0)) / cellsize - 0.5)));

    int ymin_u = std::max(
        0, std::min(
               fluid2d->get_u_nj() - 1,
               (int)ceil((ymin - fluid2d->get_origin()(1)) / cellsize - 0.5)));
    int ymin_v = std::max(
        0, std::min(fluid2d->get_v_nj() - 1,
                    (int)ceil((ymin - fluid2d->get_origin()(1)) / cellsize)));
    int ymax_u = std::max(
        0, std::min(
               fluid2d->get_u_nj() - 1,
               (int)floor((ymax - fluid2d->get_origin()(1)) / cellsize - 0.5)));
    int ymax_v = std::max(
        0, std::min(fluid2d->get_v_nj() - 1,
                    (int)floor((ymax - fluid2d->get_origin()(1)) / cellsize)));

    std::vector<Vector2i> line_buffer;
    for (int j = ymin_u; j <= ymax_u; ++j)
      for (int i = xmin_u; i <= xmax_u; ++i) {
        Vector2s pos = Vector2s(i * cellsize, (j + 0.5) * cellsize) +
                       fluid2d->get_origin();
        ...
        if (!liangbarsky::clip_line(clipping_window, q0, q1, alpha0, alpha1))
          continue;
        ...
        for (int k = 0; k < N_GAUSS; ++k) {
          ...
          Vector2i ixcb = Vector2i(
              (int)((xcb(0) - fluid2d->get_origin()(0)) / cellsize),
              (int)((xcb(1) - fluid2d->get_origin()(1)) / cellsize - 0.5));
          ...
          scalar vol_c =
              M_PI *
              mathutils::lerp(
                  er_first * er_first - radii_first * radii_first,
                  er_second * er_second - radii_second * radii_second, alpha) *
              length_segment;
          scalar mass_c = rho * vol_c;
          scalar rho_base_c = mathutils::lerp(rho_h_first, rho_h_second, alpha);
          scalar vol_base_c =
              M_PI *
              mathutils::lerp(radii_first * radii_first,
                              radii_second * radii_second, alpha) *
              length_segment;
          scalar mass_base_c = rho_base_c * vol_base_c;
          scalar dragc = d0(0) * (1.0 - alpha) + d1(0) * alpha;
          Vector2s c_c =
              c0.segment<2>(0) * (1.0 - alpha) + c1.segment<2>(0) * alpha;
          ...
          for (const Vector2i& cd : line_buffer) {
            ...
            tmp_inter.vel_weighted =
                (u_base_world * mass_base_c + u_flow_world * mass_c) *
                w_linear * w;
            tmp_inter.drag_weighted =
                dragc * weight * w / (scalar)line_buffer.size();
    ...
    for (int j = ymin_v; j <= ymax_v; ++j)
      for (int i = xmin_v; i <= xmax_v; ++i) {
        Vector2s pos = Vector2s((i + 0.5) * cellsize, j * cellsize) +
                       fluid2d->get_origin();
        Vector4s clipping_window(pos(0) - cellsize, pos(1) - cellsize,
                                 pos(0) + cellsize, pos(1) + cellsize);
        Vector2s q0 = x0, q1 = x1;
        ...
        for (int k = 0; k < N_GAUSS; ++k) {
          ...
          Vector2s xcb = xc - vel_c * dt;
          Vector2i ixcb = Vector2i(
              (int)((xcb(0) - fluid2d->get_origin()(0)) / cellsize - 0.5),
              (int)((xcb(1) - fluid2d->get_origin()(1)) / cellsize));
          ...
          Vector2s c_c =
              c0.segment<2>(2) * (1.0 - alpha) + c1.segment<2>(2) * alpha;
          ...
          for (const Vector2i& cd : line_buffer) {
            EdgeVelDragIntersection<2> tmp_inter;
            tmp_inter.vel_weighted =
                (v_base_world * mass_base_c + v_flow_world * mass_c) *
                w_linear * w;
            tmp_inter.vol_weighted = (vol_c + vol_base_c) * w_linear * w;
            tmp_inter.weight = weight * w;
            tmp_inter.linear_weight = w_linear * w;
...
template <>
Vector3s CylindricalShallowFlow<2>::computeHairLiquidAngularMomentum(
    const VectorXs& x, const VectorXs& v, FluidSim* fluidsim) const {
  FluidSim2D* fluid2d = (FluidSim2D*)fluidsim;
  scalar cellsize = fluid2d->cellsize();
    ...
    for (int j = ymin_u; j <= ymax_u; ++j)
      for (int i = xmin_u; i <= xmax_u; ++i) {
        ...
        for (int k = 0; k < N_GAUSS; ++k) {
          ...
          Vector2s pu =
              Vector2s((u_base_world * mass_base_c + u_flow_world * mass_c) *
                           w_linear * w,
                       0.0);
          sum += cross2(pos, pu);
        }
    ...
    for (int j = ymin_v; j <= ymax_v; ++j)
      for (int i = xmin_v; i <= xmax_v; ++i) {
        ...
        for (int k = 0; k < N_GAUSS; ++k) {
          ...
          Vector2s pv = Vector2s(
              0.0, (v_base_world * mass_base_c + v_flow_world * mass_c) *
                       w_linear * w);
          sum += cross2(pos, pv);
        }
      }
...
template <>
void CylindricalShallowFlow<2>::updateFromFilteredGrid(const VectorXs& x,
                                                       VectorXs& v,
                                                       FluidSim* fluidsim,
                                                       const scalar& dt) {
  FluidSim2D* fluid2d = (FluidSim2D*)fluidsim;
...
template <>
void CylindricalShallowFlow<3>::updateToFilteredGrid(const VectorXs& x,
                                                     const VectorXs& v,
                                                     FluidSim* fluidsim,
                                                     const scalar& dt,
                                                     int ibuffer) {
  ...
  for (int eidx = 0; eidx < ne; ++eidx) {
    ...
    int xmin_u = std::max(
        0, std::min(fluid3d->get_u_ni() - 1,
                    (int)ceil((xmin - fluid3d->get_origin()(0)) / cellsize)));
    int xmin_v = std::max(
        0, std::min(
               fluid3d->get_v_ni() - 1,
               (int)ceil((xmin - fluid3d->get_origin()(0)) / cellsize - 0.5)));
    int xmin_w = std::max(
        0, std::min(
               fluid3d->get_w_ni() - 1,
               (int)ceil((xmin - fluid3d->get_origin()(0)) / cellsize - 0.5)));
    int xmax_u = std::max(
        0, std::min(fluid3d->get_u_ni() - 1,
                    (int)floor((xmax - fluid3d->get_origin()(0)) / cellsize)));
    int xmax_v = std::max(
        0, std::min(
               fluid3d->get_v_ni() - 1,
               (int)floor((xmax - fluid3d->get_origin()(0)) / cellsize - 0.5)));
    int xmax_w = std::max(
        0, std::min(
               fluid3d->get_w_ni() - 1,
               (int)floor((xmax - fluid3d->get_origin()(0)) / cellsize - 0.5)));
    ...
    int ymin_u = std::max(
        0, std::min(
               fluid3d->get_u_nj() - 1,
               (int)ceil((ymin - fluid3d->get_origin()(1)) / cellsize - 0.5)));
    int ymin_v = std::max(
        0, std::min(fluid3d->get_v_nj() - 1,
                    (int)ceil((ymin - fluid3d->get_origin()(1)) / cellsize)));
    int ymin_w = std::max(
        0, std::min(
               fluid3d->get_w_nj() - 1,
               (int)ceil((ymin - fluid3d->get_origin()(1)) / cellsize - 0.5)));
    int ymax_u = std::max(
        0, std::min(
               fluid3d->get_u_nj() - 1,
               (int)floor((ymax - fluid3d->get_origin()(1)) / cellsize - 0.5)));
    int ymax_v = std::max(
        0, std::min(fluid3d->get_v_nj() - 1,
                    (int)floor((ymax - fluid3d->get_origin()(1)) / cellsize)));
    int ymax_w = std::max(
        0, std::min(
               fluid3d->get_w_nj() - 1,
               (int)floor((ymax - fluid3d->get_origin()(1)) / cellsize - 0.5)));
    ...
    int zmin_u = std::max(
        0, std::min(
               fluid3d->get_u_nk() - 1,
               (int)ceil((zmin - fluid3d->get_origin()(2)) / cellsize - 0.5)));
    int zmin_v = std::max(
        0, std::min(
               fluid3d->get_v_nk() - 1,
               (int)ceil((zmin - fluid3d->get_origin()(2)) / cellsize - 0.5)));
    int zmin_w = std::max(
        0, std::min(fluid3d->get_w_nk() - 1,
                    (int)ceil((zmin - fluid3d->get_origin()(2)) / cellsize)));
    int zmax_u = std::max(
        0, std::min(
               fluid3d->get_u_nk() - 1,
               (int)floor((zmax - fluid3d->get_origin()(2)) / cellsize - 0.5)));
    int zmax_v = std::max(
        0, std::min(
               fluid3d->get_v_nk() - 1,
               (int)floor((zmax - fluid3d->get_origin()(2)) / cellsize - 0.5)));
    int zmax_w = std::max(
        0, std::min(fluid3d->get_w_nk() - 1,
                    (int)floor((zmax - fluid3d->get_origin()(2)) / cellsize)));

    std::vector<Vector3i> line_buffer;
    for (int k = zmin_u; k <= zmax_u; ++k)
      for (int j = ymin_u; j <= ymax_u; ++j)
        for (int i = xmin_u; i <= xmax_u; ++i) {
          Vector3s pos = Vector3s(i * cellsize, (j + 0.5) * cellsize,
                                  (k + 0.5) * cellsize) +
                         fluid3d->get_origin();
          Vector6s clipping_window;
          clipping_window << (pos(0) - cellsize), (pos(1) - cellsize),
              (pos(2) - cellsize), (pos(0) + cellsize), (pos(1) + cellsize),
              (pos(2) + cellsize);
          ...
          for (int r = 0; r < N_GAUSS; ++r) {
            ...
            Vector3i ixcb = Vector3i(
                (int)((xcb(0) - fluid3d->get_origin()(0)) / cellsize),
                (int)((xcb(1) - fluid3d->get_origin()(1)) / cellsize - 0.5),
                (int)((xcb(2) - fluid3d->get_origin()(2)) / cellsize - 0.5));

            line_buffer.resize(0);
            bresemham::Bresenham3D(
                ixcb(0), ixcb(1), ixcb(2), i, j, k,
                [&](int ii, int jj, int kk) {
                  line_buffer.push_back(Vector3i(ii, jj, kk));
                });
            if (line_buffer.size() == 0)
              line_buffer.push_back(Vector3i(i, j, k));
            ...
            scalar vol_c =
                ...
                mathutils::lerp(
                    er_first * er_first - radii_first * radii_first,
                    er_second * er_second - radii_second * radii_second,
                    alpha) *
                length_segment;
            scalar mass_c = rho * vol_c;
            scalar rho_base_c =
                mathutils::lerp(rho_h_first, rho_h_second, alpha);
            scalar vol_base_c =
                M_PI *
                mathutils::lerp(radii_first * radii_first,
                                radii_second * radii_second, alpha) *
            ...
            scalar dragc = d0(0) * (1.0 - alpha) + d1(0) * alpha;
            Vector3s c_c =
                c0.segment<3>(0) * (1.0 - alpha) + c1.segment<3>(0) * alpha;
    ...
    for (int k = zmin_v; k <= zmax_v; ++k)
      for (int j = ymin_v; j <= ymax_v; ++j)
        for (int i = xmin_v; i <= xmax_v; ++i) {
          Vector3s pos = Vector3s((i + 0.5) * cellsize, j * cellsize,
                                  (k + 0.5) * cellsize) +
                         fluid3d->get_origin();
          Vector6s clipping_window;
          clipping_window << (pos(0) - cellsize), (pos(1) - cellsize),
          ...
          for (int r = 0; r < N_GAUSS; ++r) {
            ...
            Vector3s xcb = xc - vel_c * dt;
            Vector3i ixcb = Vector3i(
                (int)((xcb(0) - fluid3d->get_origin()(0)) / cellsize - 0.5),
                (int)((xcb(1) - fluid3d->get_origin()(1)) / cellsize),
                (int)((xcb(2) - fluid3d->get_origin()(2)) / cellsize - 0.5));
            ...
            scalar dragc = d0(1) * (1.0 - alpha) + d1(1) * alpha;
            Vector3s c_c =
                c0.segment<3>(3) * (1.0 - alpha) + c1.segment<3>(3) * alpha;
    ...
    for (int k = zmin_w; k <= zmax_w; ++k)
      for (int j = ymin_w; j <= ymax_w; ++j)
        for (int i = xmin_w; i <= xmax_w; ++i) {
          Vector3s pos = Vector3s((i + 0.5) * cellsize, (j + 0.5) * cellsize,
                                  k * cellsize) +
                         fluid3d->get_origin();
          Vector6s clipping_window;
          clipping_window << (pos(0) - cellsize), (pos(1) - cellsize),
          ...
          for (int r = 0; r < N_GAUSS; ++r) {
            ...
            Vector3i ixcb = Vector3i(
                (int)((xcb(0) - fluid3d->get_origin()(0)) / cellsize - 0.5),
                (int)((xcb(1) - fluid3d->get_origin()(1)) / cellsize - 0.5),
                (int)((xcb(2) - fluid3d->get_origin()(2)) / cellsize));
            ...
            Vector3s c_c =
                c0.segment<3>(6) * (1.0 - alpha) + c1.segment<3>(6) * alpha;
            ...
            for (const Vector3i& cd : line_buffer) {
              EdgeVelDragIntersection<3> tmp_inter;
              tmp_inter.vel_weighted =
                  (w_base_world * mass_base_c + w_flow_world * mass_c) *
                  w_linear * w;
              tmp_inter.drag_weighted =
                  dragc * weight * w / (scalar)line_buffer.size();
              tmp_inter.vol_weighted = (vol_c + vol_base_c) * w_linear * w;
...
template <>
void CylindricalShallowFlow<3>::updateFromFilteredGrid(const VectorXs& x,
                                                       VectorXs& v,
                                                       FluidSim* fluidsim,
                                                       const scalar& dt) {
  FluidSim3D* fluid3d = (FluidSim3D*)fluidsim;
  ...
  for (int i = 0; i < ne; ++i) {
    ...
    if (visc_solve) {
      HairFlow<3>::m_u(i) +=
          fluid3d->get_visc_impulse(edge_center).dot(dir) * dt;
    }
  }
...
template <>
Vector3s CylindricalShallowFlow<3>::computeHairLiquidAngularMomentum(
    const VectorXs& x, const VectorXs& v, FluidSim* fluidsim) const {
  FluidSim3D* fluid3d = (FluidSim3D*)fluidsim;
  scalar cellsize = fluid3d->cellsize();
      ...
      for (int j = ymin_u; j <= ymax_u; ++j)
        for (int i = xmin_u; i <= xmax_u; ++i) {
          ...
          for (int r = 0; r < N_GAUSS; ++r) {
            ...
            Vector3s pu =
                Vector3s((u_base_world * mass_base_c + u_flow_world * mass_c) *
                             w_linear * w,
                         0.0, 0.0);
            sum += pos.cross(pu);
          }
...
void CylindricalShallowFlow<DIM>::advance(const VectorXs& x, const scalar& dt) {
  ...
  // compute_stretched_liquid_height(m_old_rad_vec, HairFlow<DIM>::m_rad_vec,
  // m_old_area_v, HairFlow<DIM>::m_area_v, HairFlow<DIM>::m_eta);
  // stringutils::print(HairFlow<DIM>::m_eta);

  // advection
  compute_vertex_val(HairFlow<DIM>::m_u, m_W_fv_hair, m_uvert);

  compute_backtracing<DIM>(m_edge_x, dt, m_sum_area_e, m_area_e_accu,
                           m_area_e_inv_mapping, m_uvert, HairFlow<DIM>::m_c_v,
                           m_ustar, HairFlow<DIM>::m_c_star);

  compute_vertex_val(HairFlow<DIM>::m_c_star, m_W_fv_hair,
                     HairFlow<DIM>::m_c_v);
}

template <int DIM>
void CylindricalShallowFlow<DIM>::geodesic_to_local(const scalar& geopos,
                                                    int& pidx_low,
                                                    scalar& alpha) const {
  scalar pos_hair = find_pos_on_hair(geopos, m_sum_area_e, m_area_e_accu,
                                     m_area_e_inv_mapping);
...
scalar CylindricalShallowFlow<DIM>::computeTotalLiquidVol() const {
  ...
  for (int i = 0; i < ne; ++i) {
    ...
    sum +=
        (H1 * H1 - HairFlow<DIM>::m_rad_vec(p1) * HairFlow<DIM>::m_rad_vec(p1) +
         H0 * H0 -
         HairFlow<DIM>::m_rad_vec(p0) * HairFlow<DIM>::m_rad_vec(p0)) *
        M_PI * HairFlow<DIM>::m_area_e(i) * 0.5;
  }
  return sum;
}
...
template <int DIM>
void CylindricalShallowFlow<DIM>::add_force(const VectorXs& x,
                                            const VectorXs& accel,
                                            FluidSim* fluidsim,
                                            const scalar& dt) {
  const int np = HairFlow<DIM>::m_particle_indices.size();
  const PolygonalCohesion<DIM>* cohesion =
  ...
  const scalar& cap_multiplier =
      HairFlow<DIM>::m_parent->getCapillaryAccelMultiplier();
  const scalar& gamma = HairFlow<DIM>::m_parent->getLiquidTension();

  if (HairFlow<DIM>::m_parent->isIndividualTransfer()) {
    compute_next_pressure(m_L, HairFlow<DIM>::m_eta,
                          HairFlow<DIM>::m_parent->getLiquidTension(),
                          m_pressure);
  } else {
  ...
  compute_edge_val(HairFlow<DIM>::m_accel_v, HairFlow<DIM>::m_internal_edges,
                   m_accel_f);
  compute_accel_e(m_accel_f, HairFlow<DIM>::m_dir_f, m_accel_e);
  if (cap_multiplier > 0.0) {
    compute_capillary_accel_e(HairFlow<DIM>::m_rad_vec, HairFlow<DIM>::m_eta,
                              m_area_e_accu, gamma, m_sum_area_e,
                              cap_multiplier, m_cap_e);

    m_accel_e += m_cap_e;
  }

  compute_next_velocity(
      m_ustar, m_gradF, m_dir_f_expand, m_pressure, m_porosity_e,
      HairFlow<DIM>::m_edge_eta, HairFlow<DIM>::m_edge_rad_vec, dt, m_accel_e,
      HairFlow<DIM>::m_parent->getLiquidDensity(),
      HairFlow<DIM>::m_parent->getViscosity(),
      HairFlow<DIM>::m_parent->getHairFrictionCoeff(), HairFlow<DIM>::m_u);
  ...
  if (HairFlow<DIM>::m_parent->drippingFar())
    HairFlow<DIM>::m_u(HairFlow<DIM>::m_u.size() - 1) =
        std::max(0.0, HairFlow<DIM>::m_u(HairFlow<DIM>::m_u.size() - 1));
  else
    HairFlow<DIM>::m_u(HairFlow<DIM>::m_u.size() - 1) = 0.0;

  for (int i = 0; i < HairFlow<DIM>::m_u.size(); ++i) {
    if (HairFlow<DIM>::m_edge_eta(i) - HairFlow<DIM>::m_edge_rad_vec(i) < 1e-6)
      HairFlow<DIM>::m_u(i) = 0.0;
  }

  compute_actual_u(HairFlow<DIM>::m_u, HairFlow<DIM>::m_dir_f,
                   HairFlow<DIM>::m_actual_u_f);
  compute_vertex_val(HairFlow<DIM>::m_actual_u_f, m_W_fv_hair,
                     HairFlow<DIM>::m_actual_u_v);
}
...
void CylindricalShallowFlow<DIM>::preUpdateHairFlowHeight(const scalar& dt) {
  ...
  compute_A_matrix(m_D, m_divV, HairFlow<DIM>::m_u, m_dir_f_expand, m_L,
                   HairFlow<DIM>::m_parent->getHeightSmooth(), m_A);
...
void CylindricalShallowFlow<DIM>::updateHairFlowHeight(const scalar& dt) {
  ...
  if (HairFlow<DIM>::m_parent->isIndividualTransfer()) {
    ...
    for (int i = 1; i < np - 1; ++i)
      m_rhs_plus.row(i) =
          m_rhs.row(i) + inter_rhs.row(HairFlow<DIM>::m_particle_indices[i]);
  }
  ...
  if (m_solver.info() == Eigen::Success) {
    ...
    if (mum == MUM_NONE) {
      for (int i = 0; i < np; ++i) {
        ...
        HairFlow<DIM>::m_eta(i) =
            sqrt(std::max(
                m_sol(i, 0) +
                    HairFlow<DIM>::m_rad_vec(i) * HairFlow<DIM>::m_rad_vec(i),
                HairFlow<DIM>::m_rad_vec(i) * HairFlow<DIM>::m_rad_vec(i))) -
            HairFlow<DIM>::m_rad_vec(i);
      }
      ...
      for (int i = 0; i < np; ++i) {
        ...
        scalar H =
            std::max(HairFlow<DIM>::m_eta(i) + HairFlow<DIM>::m_rad_vec(i),
                     liquid_shell * HairFlow<DIM>::m_rad_vec(i));
        scalar modified_A =
            H * H - HairFlow<DIM>::m_rad_vec(i) * HairFlow<DIM>::m_rad_vec(i);

        const scalar& rho_hair = HairFlow<DIM>::m_parent->getHairDensity(pidx);
        const scalar w = HairFlow<DIM>::m_rad_vec(i) *
                             HairFlow<DIM>::m_rad_vec(i) * rho_hair +
                         modified_A * rho_liq;
        if (w != 0.0) {
          m.segment<DIM>(idof).setConstant(w * M_PI *
                                           HairFlow<DIM>::m_area_v_hair(i));
        }

        if (i != np - 1 && !HairFlow<DIM>::m_parent->isMassSpring()) {
          const scalar m_hair = HairFlow<DIM>::m_rad_vec(i) *
                                HairFlow<DIM>::m_rad_vec(i) * rho_hair;
          const scalar I_hair = m_hair * HairFlow<DIM>::m_rad_vec(i) *
                                HairFlow<DIM>::m_rad_vec(i) * 0.5;
          const scalar m_liq = modified_A * rho_liq;
          const scalar I_liq =
              m_liq * (modified_A * 0.5 + HairFlow<DIM>::m_rad_vec(i) *
                                              HairFlow<DIM>::m_rad_vec(i));
      ...
      for (int i = 0; i < np; ++i) {
        ...
        if (w != 0.0) {
          if (!HairFlow<DIM>::m_parent->isFixed(pidx))
            v.segment<DIM>(idof) *=
                m(idof) / (w * M_PI * HairFlow<DIM>::m_area_v_hair(i));
          m.segment<DIM>(idof).setConstant(w * M_PI *
                                           HairFlow<DIM>::m_area_v_hair(i));
        }
      ...
      for (int i = 0; i < np; ++i) {
        ...
        const scalar& rho_hair = HairFlow<DIM>::m_parent->getHairDensity(pidx);
        const Vectors<DIM> p_hair = v.segment<DIM>(idof) *
                                    HairFlow<DIM>::m_rad_vec(i) *
                                    HairFlow<DIM>::m_rad_vec(i) * rho_hair;
...
void CylindricalShallowFlow<DIM>::updateHairMass() {
  ...
  compute_liquid_mass(HairFlow<DIM>::m_eta, HairFlow<DIM>::m_area_v_hair,
                      HairFlow<DIM>::m_rad_vec,
                      HairFlow<DIM>::m_parent->getLiquidDensity(),
                      m_liquid_mass, HairFlow<DIM>::m_parent->getLiquidShell());

  for (int i = 0; i < np; ++i) {
    HairFlow<DIM>::m_parent->updateMassWithLiquid(
        HairFlow<DIM>::m_particle_indices[i], m_liquid_mass(i),
        HairFlow<DIM>::m_eta(i) + HairFlow<DIM>::m_rad_vec(i));
  }
}
