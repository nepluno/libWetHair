...
#ifndef LIBWETHAIR_CORE_PCGSOLVER_SPARSE_MATRIX_H_
#define LIBWETHAIR_CORE_PCGSOLVER_SPARSE_MATRIX_H_
...
void multiplyMat(const FixedSparseMatrix<T> &A, const FixedSparseMatrix<T> &B,
  ...
  c.zero();
  tbb::parallel_for(
      (unsigned int)0, (unsigned int)c.n, (unsigned int)1,
      [&](unsigned int i)
      // for (unsigned int i=0;i<c.n;++i)
      {
        for (unsigned int j = A.rowstart[i]; j < A.rowstart[i + 1]; ++j) {
          ...
          T A_ik = A.value[j];
          for (unsigned int kkk = B.rowstart[k]; kkk < B.rowstart[k + 1];
               ++kkk) {
