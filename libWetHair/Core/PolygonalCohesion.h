...
#ifndef LIBWETHAIR_CORE_POLYGONAL_COHESION_H_
#define LIBWETHAIR_CORE_POLYGONAL_COHESION_H_
...
struct EdgeEdgePair {
  ...
  std::string toString() const {
    std::stringstream ss;
    ss << "base: " << base_eidx << " neighborhair: " << neighbor_hair_idx
       << " alpha: (" << alpha_0 << ", " << alpha_1 << ") ncoord: ("
       << neighbor_local_coord_0 << ", " << neighbor_local_coord_1
...
struct EdgeEdgePairEEC {
  ...
  std::string toString() const {
    std::stringstream ss;
    ss << "base: " << base_eidx << " neighboredge: " << neighbor_eidx
       << " alpha: (" << alpha_0 << ", " << alpha_1 << ") ncoord: ("
       << neighbor_local_coord_0 << ", " << neighbor_local_coord_1
       << ")";  // count: (" << count_0 << ", " << count_1 << ")";
    return ss.str();
...
struct ParticleEdgePair {
  ...
  std::string toString() const {
    std::stringstream ss;
    ss << "pidx: " << pidx << " eidx: " << eidx << " alpha: " << alpha
       << " dist: " << dist << " edgeradii: " << radii_j
       << " maxd: " << max_dist << " count: " << count
  ...
  static size_t size() {
    return ((sizeof(ParticleEdgePair) + sizeof(scalar) - 1) / sizeof(scalar)) *
           sizeof(scalar);
  }
...
struct PointEdgePair {
  ...
  std::string toString() const {
    std::stringstream ss;
    ss << "base: (" << base_eidx << ", " << alpha_point << ") neighbor: ("
       << neighbor_eidx << ", " << neighbor_alpha_point << ") V: " << V
       << " q-weight: " << quadrature_weight
       << " p-weight: " << pressure_weight;
    return ss.str();
  }
};
...
class PolygonalCohesion : public Force {
 ...
 public:
  ...
  virtual void updatePointEdgePairs(const VectorXs& x, const VectorXs& v,
                                    const scalar& dt);
  ...
  virtual void writeReadable(std::ostream& oss_pepairs,
  ...
  virtual scalar getStiffnessPlanar(const scalar& radius, const scalar& d0,
                                    const scalar& A_target,
                                    const scalar& pressure_weight) const;

  virtual scalar getDStar(const scalar& radius) const;

  virtual scalar getStiffness(const scalar& radius, const scalar& d0,
                              const scalar& A_target,
                              const scalar& pressure_weight) const;
 ...
 protected:
  ...
  void extractLocalCoord(const scalar& local_coord, int hidx, int& eidx,
                         scalar& alpha) const;
  ...
  scalar computeEdgeThickness(const scalar& vol_p,
                              const int_Vectors_scalar<DIM>& ep);
  ...
  void findEdgeEdgeContact(const VectorXs x, const VectorXs v, const scalar& dt,
                           const int& base_eidx);
  void findPointEdgePairsEEC(const VectorXs& x, int base_eidx,
                             std::vector<PointEdgePair>& poepairs);

  void findPointEdgePairs(const VectorXs& x, int base_eidx,
                          std::vector<PointEdgePair>& poepairs);
