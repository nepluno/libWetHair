...
void BendingForce<ViscousT>::accumulateIntegrationVars(
  ...
  for (IndexType vtx = s_first; vtx < strand.getNumVertices() - s_last; ++vtx) {
    ...
    Eigen::Matrix<scalar, 2, 11> utgk =
        (strand.m_strandState->m_gradKappas[vtx]).transpose();
    ...
    for (int r = 0; r < 4; ++r) {
      ...
      if (r < 3)
        J[idx_j + 8 + r] = Triplets(idx_pos, dthird + r, utgk(0, r + 8));

      J[idx_j + r + 11] = Triplets(idx_pos + 1, dfirst + r, utgk(1, r));
      J[idx_j + 4 + r + 11] =
          Triplets(idx_pos + 1, dsecond + r, utgk(1, r + 4));
      if (r < 3)
        J[idx_j + 8 + r + 11] =
            Triplets(idx_pos + 1, dthird + r, utgk(1, r + 8));
    }
        ...
        for (int s = 0; s < 11; ++s) {
          if (r == 3 || r == 7 || s == 3 || s == 7) {
            tildeK[idx_tildek + r * 11 + s] = Triplets(
                dfirst + r, dfirst + s,
                hessKappa.first(r, s) * b * ilen * (kappa(0) - kappaBar(0)));
            tildeK[idx_tildek + r * 11 + s + 121] = Triplets(
                dfirst + r, dfirst + s,
                hessKappa.second(r, s) * b * ilen * (kappa(1) - kappaBar(1)));
          } else {
            tildeK[idx_tildek + r * 11 + s] = Triplets(
                dfirst + r, dfirst + s, hessKappa.first(r, s) * weight0);
            tildeK[idx_tildek + r * 11 + s + 121] = Triplets(
                dfirst + r, dfirst + s, hessKappa.second(r, s) * weight1);
          }
        }
      }
    }
