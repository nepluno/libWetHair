...
#ifndef LIBWETHAIR_CORE_DER_DEPENDENCIES_ELASTICPARAMETERS_H_
#define LIBWETHAIR_CORE_DER_DEPENDENCIES_ELASTICPARAMETERS_H_
...
class PhysicalRadius : public DependencyNode<scalar> {
 public:
  PhysicalRadius(scalar radius) : DependencyNode<scalar>(radius) {
...
class ElasticKt : public DependencyNode<scalar> {
 ...
 protected:
  virtual void compute() {
    ...
    m_value = PI_4 * radius * radius * (radius * radius + radius * radius) *
              shearModulus;
