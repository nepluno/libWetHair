...
void TwoDSceneXMLParser::loadLiquidParameter( rapidxml::xml_node<> *node, TwoDScene<DIM>& twoscene)
{
  ...
  if(nd)
  {
    ...
    if( timend != NULL )
    {
      if( !stringutils::extractFromString(std::string(timend->value()),parameter.quadratic_dragging) )
      {
        std::cerr << "\033[31;1mERROR IN XMLSCENEPARSER:\033[m Failed to parse 'quadraticdragging' attribute for liquid. Value must be numeric. Exiting." << std::endl;
        exit(1);
      }
    }
