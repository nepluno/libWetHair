...
WetHairCore<DIM>::WetHairCore(TwoDScene<DIM>* scene,
                              ...
                              std::function<bool(double)>&& script_callback)
    : m_scene(scene),
...
template <int DIM>
const std::vector<scalar>& WetHairCore<DIM>::getStepperTimingStatistics()
    const {
...
void WetHairCore<DIM>::stepSystem() {
  ...
  while (t < dt) {
    ...
    if (!parameter.no_fluids) {
      ...
      // update particle to grid with hair particles velocity and weight of bulk
      // liquid, the former is for tracing velocity of liquid on the space
      // occupied by hairs, thus we can compute drag later; the latter is for
      // determine if current grid cell is inside bulk liquid (used by cohesion
      // force).
    ...
    if (m_scene->getNumFlows() > 0) {
      // 1. handle the hair dynamics (Sec. 4.1), computing the drag force using
      // velocities sampled from the grid (Sec. 4.8), as well as the
      // adhesive/repulsive force between hairs (Sec. 4.5).
      ...
      if (!parameter.no_fluids) {
        // 2. advect the shallow water on hair, add force (Sec. 4.3 and
        // Sec. 4.6), and compute inter-hair flow velocity (Sec. 4.4).
        ...
        if (!parameter.no_swe) {
          for (int i = 0; i < parameter.swesteps; ++i) {
            ...
            m_scene->addForceHairFlows(m_scene_stepper->getAcceleration(),
                                       swesubstep);
            ...
            m_scene->updateHairFlowsHeight(m_scene_stepper->getAcceleration(),
                                           swesubstep);
        ...
        // 3. transfer the velocity of on-hair liquid onto collocated grid
        // (Sec. 4.7).
    ...
    std::cout << "[update particle flow to grid - no hair particle]"
              << std::endl;
    ...
    if (!parameter.no_fluids) {
      ...
      // 6. solve the Viscous and/or Poisson equation and do pressure projection
      // (Sec. 4.7).
      ...
      // 7. combine the two velocity fields in a momentum-conservative style
      // (Sec. 4.7).
      ...
      // 10. transfer the velocity on grid back to the particles with APIC
      // (Sec. 4.7).
      ...
      if (!(parameter.no_absorb && parameter.no_dripping) &&
          m_scene->getNumFlows() > 0) {
        std::cout << "[absorb/release particle flows]" << std::endl;
        m_scene->absorbReleaseParticleFlows(substep, parameter.no_absorb,
                                            parameter.no_dripping);
      }
