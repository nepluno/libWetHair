...
template <int DIM>
LevelSetForce<DIM>::LevelSetForce(TwoDScene<DIM>* parent, FluidSim* fluidsim,
                                  int hidx)
    : Force(), m_hidx(hidx), m_fluidsim(fluidsim), m_parent(parent) {
...
template <>
void LevelSetForce<3>::preCompute(const VectorXs& x, const VectorXs& v,
  ...
  for (int j = 0; j < nfp; ++j) {
    ...
    scalar max_dist = (1.0 + 0.5 * theta) *
                      sqrt(M_PI * (H * H - radii(particle) * radii(particle)));

    scalar fq =
        fluid3d->get_nodal_solid_phi(x.segment<3>(m_parent->getDof(particle)));

    m_particle_ls_pairs[j].dist = std::max(0.0, fq);
    m_particle_ls_pairs[j].max_dist = max_dist;
    m_particle_ls_pairs[j].valid =
        (m_parent->isFixed(particle)) ? false : (fq < max_dist);

    if (j > 0 && j < nfp - 1) {
      if (m_parent->isFixed(flow_particles[j + 1]) ||
          m_parent->isFixed(flow_particles[j - 1])) {
  ...
  m_point_ls_pairs.resize(flow->getGlobalEdges().size() * m_num_quadrature);
  memset(&m_point_ls_pairs[0], 0,
         m_point_ls_pairs.size() * sizeof(PointLSPair));
  ...
  for (int i = 0; i < ne; ++i) {
    ...
    if (pair_0.valid && pair_1.valid) {
      ...
      scalar ddiv = (pair_0.max_dist - (pair_0.dist - r0i)) -
                    (pair_1.max_dist - (pair_1.dist - r1i));
      if (ddiv == 0.0) {
        // parallel, do nothing
      } else {
    ...
    for (int k_gauss = 0; k_gauss < m_num_quadrature; ++k_gauss) {
      ...
      const scalar V =
          M_PI * mathutils::lerp(H0 * H0 - radii(e.first) * radii(e.first),
                                 H1 * H1 - radii(e.second) * radii(e.second),
                                 alpha_gauss);
      ...
      if (bb > 0. || friction > 0.) {
        ...
        const Vector3s& xsa = x.segment<3>(m_parent->getDof(e.first)) -
                              v.segment<3>(m_parent->getDof(e.first)) * dt;
        const Vector3s& xsb = x.segment<3>(m_parent->getDof(e.second)) -
                              v.segment<3>(m_parent->getDof(e.second)) * dt;
  ...
  m_point_ls_pairs.erase(
      std::remove_if(
          m_point_ls_pairs.begin(), m_point_ls_pairs.end(),
          [](const PointLSPair& p) { return p.quadrature_weight == 0.0; }),
      m_point_ls_pairs.end());
}

template <>
void LevelSetForce<2>::preCompute(const VectorXs& x, const VectorXs& v,
                                  const VectorXs& m, const scalar& dt) {
  ...
  for (int i = 0; i < ne; ++i) {
    ...
    for (int k_gauss = 0; k_gauss < m_num_quadrature; ++k_gauss) {
      ...
      if (bb > 0. || friction > 0.) {
        ...
        const Vector2s& xsa =
            x.segment<2>(e.first * 2) - v.segment<2>(e.first * 2) * dt;
        const Vector2s& xsb =
            x.segment<2>(e.second * 2) - v.segment<2>(e.second * 2) * dt;
...
template <int DIM>
int LevelSetForce<DIM>::getAffectedHair(
...
void LevelSetForce<2>::postStepScene(const scalar& dt) {
  /*  VectorXs& x = m_parent->getX();
    ...
    for(int j = 0; j < nfp; ++j)
    {
      ...
      scalar phi_value = fluid2d->get_nodal_solid_phi(p) -
    radii(flow_particles[j]) * 2.0; if(phi_value < 0) { Vector2s normal =
    fluid2d->get_nodal_solid_phi_gradient(p); normal.normalize();
...
void LevelSetForce<3>::postStepScene(const scalar& dt) {
  /*  VectorXs& x = m_parent->getX();
    ...
    for(int j = 0; j < nfp; ++j)
    {
      ...
      scalar phi_value = fluid3d->get_nodal_solid_phi(p) -
    radii(flow_particles[j]) * 2.0; if(phi_value < 0) { Vector3s normal =
    fluid3d->get_nodal_solid_phi_gradient(p); normal.normalize();
...
template <>
void LevelSetForce<2>::computeIntegrationVars(
  ...
  for (int pair_idx = 0; pair_idx < nplp; ++pair_idx) {
    ...
    scalar r0 =
        mathutils::lerp(radii(e.first), radii(e.second), pair.alpha_point);
    ...
    scalar k0 =
        m_parent->getStiffnessPlanar(
            r0, d0, pair.V, pair.pressure_weight * cohesion_multiplier) *
    ...
    if (friction > 0.) {
      Phi(base) = sqrt((xc - pair.x0).transpose() *
                       (Matrix2s::Identity() - dddx * dddx.transpose()) *
                       (xc - pair.x0));
      if (Phi(base) < 1e-20)
    ...
    for (int i = 0; i < 2; ++i) {
      for (int r = 0; r < 2; ++r) {
        ...
        J[base_J] =
            Triplets(base_idx, indices[i] * 2 + r, dddx(r) * multipliers[i]);
        base_J += nplp * 2 * 2;
        base_idx += nplp;
        ...
        if (friction > 0.) {
          Vector2s dddx_tangent =
              (Matrix2s::Identity() - dddx * dddx.transpose()) * (xc - pair.x0);
          if (dddx_tangent.norm() < 1e-20)
            dddx_tangent = Vector2s(1, 0);
          else {
            dddx_tangent.normalize();
          }
          J[base_J] = Triplets(base_idx, indices[i] * 2 + r,
                               dddx_tangent(r) * multipliers[i]);
          base_J += nplp * 2 * 2;
          base_idx += nplp;
        }
      }
...
template <>
void LevelSetForce<3>::computeIntegrationVars(
    const VectorXs& x, const VectorXs& v, const VectorXs& m, VectorXs& lambda,
    VectorXs& lambda_v, TripletXs& J, TripletXs& Jv, TripletXs& Jxv,
    TripletXs& tildeK, TripletXs& stiffness, TripletXs& damping, VectorXs& Phi,
    VectorXs& Phiv, const scalar& dt) {
  ...
  for (int pair_idx = 0; pair_idx < nplp; ++pair_idx) {
    ...
    if (friction > 0.) {
      // friction
      Phi(base) = sqrt((xc - pair.x0).transpose() *
                       (Matrix3s::Identity() - dddx * dddx.transpose()) *
                       (xc - pair.x0));
      if (Phi(base) < 1e-20) {
        stiffness[base] = Triplets(base, base, 0.0);
      } else {
    ...
    for (int i = 0; i < 2; ++i) {
      for (int r = 0; r < 3; ++r) {
        ...
        J[base_J] = Triplets(base_idx, m_parent->getDof(indices[i]) + r,
                             dddx(r) * multipliers[i]);
        base_J += nplp * 3 * 2;
        base_idx += nplp;
        ...
        if (friction > 0.) {
          Vector3s dddx_tangent =
              (Matrix3s::Identity() - dddx * dddx.transpose()) * (xc - pair.x0);
          if (dddx_tangent.norm() < 1e-20)
            dddx_tangent = Vector3s(1, 0, 0);
          else {
            dddx_tangent.normalize();
          }
          J[base_J] = Triplets(base_idx, m_parent->getDof(indices[i]) + r,
...
int LevelSetForce<DIM>::numTildeK() {
  return 0;  // m_point_ls_pairs.size() * (DIM * 2) * (DIM * 2);   // TODO:
             // Implementation
}
...
template <int DIM>
void LevelSetForce<DIM>::storeLambda(const VectorXs& lambda,
