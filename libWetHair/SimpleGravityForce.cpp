...
template <int DIM>
SimpleGravityForce<DIM>::SimpleGravityForce(const Vector3s& gravity,
                                            TwoDScene<DIM>* s)
    : Force(), m_gravity(gravity), m_scene(s) {
...
template <>
void SimpleGravityForce<2>::preCompute(const VectorXs& x, const VectorXs& v,
...
template <>
void SimpleGravityForce<3>::preCompute(const VectorXs& x, const VectorXs& v,
                                       const VectorXs& m, const scalar& dt) {
  int np = m_scene->getNumParticles();
  ...
  if (m_scene->viscositySolve()) {
    ...
    tbb::parallel_for(0, np, 1, [&](int i) {
      const Vector3s& pos = x.segment<3>(m_scene->getDof(i)) -
                            v.segment<3>(m_scene->getDof(i)) * dt;
      const scalar gamma = fluid3d->getClampedLiquidPhiValue(pos);
      m_buoyancy.segment<3>(i * 3) = (fluid3d->get_pressure_gradient(pos) -
                                      fluid3d->get_visc_impulse(pos) * rho_L) *
                                     gamma;
    });
  } else {
    tbb::parallel_for(0, np, 1, [&](int i) {
      const Vector3s& pos = x.segment<3>(m_scene->getDof(i)) -
                            v.segment<3>(m_scene->getDof(i)) * dt;
      const scalar gamma = fluid3d->getClampedLiquidPhiValue(pos);
      m_buoyancy.segment<3>(i * 3) =
          fluid3d->get_pressure_gradient(pos) * gamma;
    });
  }
}
...
template <int DIM>
void SimpleGravityForce<DIM>::addEnergyToTotal(const VectorXs& x,
                                               const VectorXs& v,
  ...
  for (int i = 0; i < m_scene->getNumParticles(); ++i) {
    E -= m(m_scene->getDof(i)) *
         g.segment<DIM>(0).dot(x.segment<DIM>(m_scene->getDof(i)));
  }
}

template <int DIM>
void SimpleGravityForce<DIM>::addGradEToTotal(const VectorXs& x,
                                              const VectorXs& v,
                                              const VectorXs& m,
                                              VectorXs& gradE) {
  assert(x.size() == v.size());
  ...
  tbb::parallel_for(0, np, 1, [&](int i) {
    ...
    gradE.segment<DIM>(m_scene->getDof(i)) +=
        m(m_scene->getDof(i)) *
        (m_buoyancy.segment<DIM>(i * DIM) / rho - g.segment<DIM>(0));
  });
}

template <int DIM>
void SimpleGravityForce<DIM>::addHessXToTotal(const VectorXs& x,
                                              const VectorXs& v,
                                              const VectorXs& m,
                                              TripletXs& hessE) {
  assert(x.size() == v.size());
...
template <int DIM>
void SimpleGravityForce<DIM>::addHessVToTotal(const VectorXs& x,
                                              const VectorXs& v,
                                              const VectorXs& m,
                                              TripletXs& hessE) {
  assert(x.size() == v.size());
...
template <int DIM>
void SimpleGravityForce<DIM>::getAffectedVars(int pidx,
